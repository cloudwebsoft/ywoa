...
public class DirectoryView {
    ...
    public void ListSimple(HttpServletRequest request, JspWriter out, String target, String link, String tableClass, String tableClassMouseOn) throws Exception {
        String curCode = ParamUtil.get(request, "catalogCode");

        ListTreeSimple(request, out, curCode, rootLeaf, true, target, link, tableClass, tableClassMouseOn);
    }

    // 显示根结点为leaf的树
    void ListTreeSimple(HttpServletRequest request, JspWriter out, String curCode, Leaf leaf,
                  boolean isLastChild, String target, String link, String tableClass, String tableClassMouseOn) throws Exception {
        ShowLeafSimple(request, out, curCode, leaf, isLastChild, target, link, tableClass, tableClassMouseOn);
        ...
        while (ri.hasNext()) {
            ...
            if (size != i)
                isLastChild1 = false;
            ListTreeSimple(request, out, curCode, childlf, isLastChild1, target, link, tableClass, tableClassMouseOn);
        }
        if (size > 0)
     ...
     **/
    void ShowLeafSimple(HttpServletRequest request, JspWriter out, String curCode, Leaf leaf,
                  boolean isLastChild, String target, String link, String tableClass, String tableClassMouseOn) throws Exception {
        String code = leaf.getCode();
        String name = leaf.getName();
        int layer = leaf.getLayer();
        String imgPath = request.getContextPath() + "/forum/admin/images";
        ...
        for (int k = rootLeaf.getLayer(); k <= layer - 1; k++) {
            ...
            while (ir.hasNext()) {
                ...
                //如果在K层上存在一个竖线结点则画出
                if (node.getLayer() == k) {
                    node.show(out, "" + imgPath +"/i_plus-2.gif");
                    if (node.getCount() == 0) {
        ...
        if (leaf.getCode().equals(rootLeaf.getCode())) { // 如果是根结点
            out.println("<img tableRelate='' onClick=\"ShowChild(this, '" + tableid + "')\" src='" + imgPath +"/i_puls-root.gif' align='absmiddle'><img src='" + imgPath +"/folder_01.gif' align='absmiddle'>");
        } else {
            if (isLastChild) { // 是最后一个孩子结点
                if (childcount > 0)
                    out.println("<img tableRelate='" + tableid + "' onClick=\"ShowChild(this, '" + tableid + "')\" src='" + imgPath +"/i_plus2-2.gif' align='absmiddle'><img src='" + imgPath +"/folder_01.gif' align='absmiddle'>");
                else
                    out.println("<img src='" + imgPath +"/i_plus-2-3.gif' align='absmiddle'><img src='" + imgPath +"/folder_01.gif' align='absmiddle'>");
            } else { // 不是最后一个孩子结点
                if (childcount > 0)
                    out.println("<img tableRelate='" + tableid + "' onClick=\"ShowChild(this, '" + tableid + "')\" src='" + imgPath +"/i_plus2-1.gif' align='absmiddle'><img src='" + imgPath +"/folder_01.gif' align='absmiddle'>");
                else
                    out.println("<img src='" + imgPath +"/i_plus-2-2.gif' align='absmiddle'><img src='" + imgPath +"/folder_01.gif' align='absmiddle'>");
            }
        }

        // 三种类型节点，用同一个Link
        if (leaf.getType()==Leaf.TYPE_LIST || leaf.getCode().equals(Leaf.CODE_ROOT)) {
            out.print(
...
*/
    ...
    void ShowLeaf(JspWriter out, Leaf leaf,
        ...
        if (leaf.getType() == leaf.TYPE_LIST) {
            out.print("<a target=_parent href='witkey_task_list.jsp?catalogCode=" +
