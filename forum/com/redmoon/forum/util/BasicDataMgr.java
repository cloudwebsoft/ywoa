...
public class BasicDataMgr {
    String cacheGroup = "config_forum_basic";
    ...
    public BasicDataMgr() {
        String xmlFile = "config_forum_basic.xml";
        URL confURL = getClass().getClassLoader().getResource("/" + xmlFile);
    ...
    /**
     * 取得项目的options，默认项呈被选择状态
     * @param itemCode String
     * @return String
     */
    public String getOptionsStr(String itemCode) {
        return getOptionsStr(itemCode, true);
    }

    /**
     * 取得项目的options
     * @param itemCode String
     * @param isDefaultSelected 默认项是否被选中
     * @return String
     */
    public String getOptionsStr(String itemCode, boolean isDefaultSelected) {
        String str = null;
        try {
            ...
            for (int i = 0; i < len; i++) {
                if (isDefaultSelected) {
                    if (r[i][1].equals(defaultValue))
                        str += "<option value='" + r[i][1] + "' selected>" +
                                r[i][0] +
                                "</option>";
                    else {
