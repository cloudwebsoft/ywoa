...
public class MediaDirMgr {
    String connname = "";
    Logger logger = Logger.getLogger(MediaDirMgr.class.getName());
    ...
    public boolean AddChild(HttpServletRequest request) throws
        ...
        MediaDirDb dd = getMediaDirDb(parent_code);
        return dd.AddChild(lf);
    }

    public void del(String delcode) throws ErrMsgException {
        MediaDirDb lf = getMediaDirDb(delcode);
        lf.del(lf);
    }
    ...
    public synchronized boolean move(HttpServletRequest request) throws
        ...
        MediaDirDb dd = getMediaDirDb(code);
        return dd.move(direction);
    }

    public MediaDirDb getMediaDirDb(String code) {
        MediaDirDb dd = new MediaDirDb();
        return dd.getMediaDirDb(code);
    }

    public MediaDirDb getBrother(String code, String direction) throws
            ErrMsgException {
        MediaDirDb dd = getMediaDirDb(code);
     ...
     */
    public void repairLeaf(MediaDirDb lf) {
        ...
        int orders = 1;
        while (ir.hasNext()) {
            MediaDirDb lfch = (MediaDirDb)ir.next();
            // 重置孩子节点的排列顺序
        ...
        if (lf.getCode().equals(lf.ROOTCODE)) {
            ...
            if (parentCode.equals(lf.ROOTCODE))
                ...
                while (!parentCode.equals(lf.ROOTCODE)) {
                    ...
                    MediaDirDb parentLeaf = getMediaDirDb(parentCode);
    ...
    // 修复根结点为leaf的树
    public void repairTree(MediaDirDb leaf) throws Exception {
        ...
        // 刷新缓存
        MediaDirCache dc = new MediaDirCache();
