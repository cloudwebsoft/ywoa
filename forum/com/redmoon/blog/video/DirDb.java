...
public class DirDb extends ObjectDb implements Serializable {
    ...
    public void setQuerySave() {
        this.QUERY_SAVE = "update blog_video_dir set name=?,description=?,dir_type=?,orders=?,child_count=?,layer=? where code=?";
    }
    ...
    public void setQueryLoad() {
        this.QUERY_LOAD = "select code,name,description,parent_code,root_code,orders,layer,child_count,add_date,dir_type from blog_video_dir where code=?";
    }
    ...
    public Vector getChildren() {
        Vector v = new Vector();
        String sql =
                "select code from blog_video_dir where parent_code=? order by orders";
        Conn conn = new Conn(connname);
        ResultSet rs = null;
        try {
    ...
    public synchronized boolean save(String newParentCode) {
        ...
        String sql = "update blog_video_dir set name=" +
                     StrUtil.sqlstr(name) +
        ...
        try {
            ...
            try {
                if (r == 1) {
                    ...
                    sql =
                            "select code from blog_video_dir where parent_code=" +
    ...
    public boolean AddChild(DirDb childleaf) throws
        ...
        String updatesql = "";
        String insertsql = "insert into blog_video_dir (code,name,parent_code,description,orders,root_code,child_count,layer,dir_type,add_date) values (?,?,?,?,?,?,?,?,?,?)";
        ...
        try {
            // 更改根结点的信息
            updatesql =
                    "update blog_video_dir set child_count=child_count+1" +
                    " where code=" + StrUtil.sqlstr(code);
            conn.beginTrans();
    ...
    public boolean delsingle(DirDb leaf) {
        String sql = "delete from blog_video_dir where code=" +
        ...
        try {
            ...
            sql =
                    "update blog_video_dir set orders=orders-1 where parent_code=" +
                    StrUtil.sqlstr(leaf.getParentCode()) + " and orders>" +
                    leaf.getOrders();
            jt.executeUpdate(sql);
            sql =
                    "update blog_video_dir set child_count=child_count-1 where code=" +
    ...
    public boolean move(String direction) {
        ...
        if (isexist) {
            ...
            try {
                conn.beginTrans();
                if (direction.equals("down")) {
                    sql = "update blog_video_dir set orders=orders+1" +
                          " where code=" + StrUtil.sqlstr(code);
    ...
    public void setQueryList() {
        this.QUERY_LIST = "select code from blog_video_dir";
    }
