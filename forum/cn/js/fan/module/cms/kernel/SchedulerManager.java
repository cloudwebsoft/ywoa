...
public class SchedulerManager {
        ...
        while (ir.hasNext()) {
            ...
            try {
                ...
                if (className.equals("com.redmoon.forum.job.FullTextSearchJob"))
                    ...
                    jobDetail.getJobDataMap().put("id",
                                                  jud.getString("data_map"));
                // LogUtil.getLog(getClass()).info("flowCode=" + jud.getString("data_map") + " cron=" + jud.getString("cron"));
                CronTrigger trigger = new CronTrigger(jud.getString("id"), sched.DEFAULT_GROUP,
    ...
    public void delJob(String jobId) {
        try {
            sched.deleteJob(jobId, sched.DEFAULT_GROUP);
        }
        catch (Exception e) {
    ...
    public void rescheduleJob(String jobId, String cron) {
        try {
            ...
            JobDetail jd = sched.getJobDetail(jobId, sched.DEFAULT_GROUP);
            JobDataMap jdm = jd.getJobDataMap();
            CronTrigger trigger = new CronTrigger("" + jobId, sched.DEFAULT_GROUP,
                        cron); // 创建触发器
