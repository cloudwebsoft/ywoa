...
public class ImgStoreFileDb extends ObjectDb {
    long id;
    String name;
    String diskName;
    String visualPath;

    public ImgStoreFileDb() {
        init();
    }

    public ImgStoreFileDb(long id) {
        init();
        this.id = id;
    ...
    public void initDB() {
        this.tableName = "cws_cms_img_store_file";
        primaryKey = new PrimaryKey("id", PrimaryKey.TYPE_LONG);
        objectCache = new ImgStoreFileCache(this);

        QUERY_CREATE = "insert into cws_cms_img_store_file (id, dir_code, name, disk_name, visual_path, file_size, ext, upload_date) values (?, ?,?,?,?,?,?,?)";
        QUERY_LIST = "select id from cws_cms_img_store_file order by upload_date desc";
        QUERY_DEL = "delete from cws_cms_img_store_file where id=?";

        QUERY_SAVE = "update cws_cms_img_store_file set name=? WHERE id=?";
        QUERY_LOAD = "SELECT dir_code, name, disk_name, visual_path, file_size, upload_date FROM cws_cms_img_store_file WHERE id=?";
        isInitFromConfigDB = false;
    }

    public IObjectDb getObjectRaw(PrimaryKey pk) {
        return new ImgStoreFileDb(pk.getLongValue());
    }

    public ImgStoreFileDb getImgStoreFileDb(long id) {
        return (ImgStoreFileDb)getObjectDb(new Long(id));
    }
