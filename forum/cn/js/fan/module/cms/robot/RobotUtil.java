...
public class RobotUtil
{
  public String gather(String link, String charset)
  {
    ...
    try {
      ...
      Proxy proxy = null;
      boolean isProxy = cfg.getBooleanProperty("robot.isProxy");
      if (!isProxy) {
        huc = (HttpURLConnection)url.openConnection();
      }
      else {
        String server = StrUtil.getNullStr(cfg.getProperty("robot.proxyServer"));
        int port = StrUtil.toInt(cfg.getProperty("robot.proxyPort"));
        String userName = StrUtil.getNullStr(cfg.getProperty("robot.proxyUserName"));
        String pass = StrUtil.getNullStr(cfg.getProperty("robot.proxyPassword"));

        LogUtil.getLog(getClass()).info("server=" + server + " port=" + port + " userName=" + userName + " pass=" + pass);
        ...
        if (!userName.equals(""))
          Authenticator.setDefault(new MyAuthenticator(userName, pass));
  ...
  public static String gatherFile(String fileUrl, String charset, String saveDirectory)
  {
    ...
    try {
      ...
      if (!isProxy) {
        ...
        String server = cfg.getProperty("robot.proxyServer");
        int port = StrUtil.toInt(cfg.getProperty("robot.proxyPort"));
        String userName = StrUtil.getNullStr(cfg.getProperty("robot.proxyUserName"));
        String pass = StrUtil.getNullStr(cfg.getProperty("robot.proxyPassword"));
  ...
  public static String fixFlashLinkAndDownload(String docUrl, String content, String charset, String prefix, boolean isDownload, String relativePath, Vector flashsOuter)
  {
    ...
    if ((!isDownload) && (prefix.equals(""))) {
      ...
      int k = docUrl.lastIndexOf("/");
      String docUri = docUrl.substring(0, k + 1);
  ...
  public static String fixImageLinkAndDownload(String docUrl, String content, String charset, String prefix, boolean isDownload, String relativePath, Vector imgsOuter)
  {
      ...
      while (matcher.find()) {
        ...
        if (!imgUrl.startsWith("http:")) {
          if (!prefix.equals("")) {
            ...
            imgUrl = docUri + imgUrl;
          }
        }

        LogUtil.getLog(RobotUtil.class).info("imgUrl=" + imgUrl);
...
/* Location:           D:\yimihome\WebRoot\WEB-INF\lib\cloudwebsoft-cms-robot.jar
 * Qualified Name:     cn.js.fan.module.cms.robot.RobotUtil
