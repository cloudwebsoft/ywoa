...
public class FlashStoreDirDb extends ObjectDb implements Serializable {
    ...
    public FlashStoreDirDb() {
        init();
    }

    public FlashStoreDirDb(String code) {
        this.code = code;
        load();
        init();
    }
    ...
    public ObjectDb getObjectRaw(PrimaryKey pk) {
        return new FlashStoreDirDb(pk.getStrValue());
    }
    ...
    public void setQuerySave() {
        this.QUERY_SAVE = "update cws_cms_flash_store_dir set name=?,description=?,dir_type=?,orders=?,child_count=?,layer=? where code=?";
    }
    ...
    public void setQueryLoad() {
        this.QUERY_LOAD = "select code,name,description,parent_code,root_code,orders,layer,child_count,add_date,dir_type from cws_cms_flash_store_dir where code=?";
    }
    ...
    public Vector getChildren() {
        Vector v = new Vector();
        String sql =
                "select code from cws_cms_flash_store_dir where parent_code=? order by orders";
        Conn conn = new Conn(connname);
        ResultSet rs = null;
        try {
            ...
            if (rs != null) {
                while (rs.next()) {
                    ...
                    v.addElement(getFlashStoreDirDb(c));
                }
            }
        } catch (SQLException e) {
     ...
     */
    public Vector getAllChild(Vector vt, FlashStoreDirDb leaf) throws
            ErrMsgException {
    ...
    public synchronized boolean save() {
        ...
        try {
            ...
            try {
                if (r == 1) {
                    re = true;
                    FlashStoreDirCache dcm = new FlashStoreDirCache();
    ...
    public synchronized boolean save(String newParentCode) {
        ...
        FlashStoreDirDb lfparent = getFlashStoreDirDb(newParentCode);
        int oldorders = orders;
        int neworders = lfparent.getChildCount() + 1;
        int parentLayer = lfparent.getLayer();
        String sql = "update cws_cms_flash_store_dir set name=" +
                     StrUtil.sqlstr(name) +
        ...
        try {
            ...
            try {
                if (r == 1) {
                    ...
                    // DirListCacheMgr更新
                    FlashStoreDirChildrenCache.remove(oldParentCode);
                    FlashStoreDirChildrenCache.remove(newParentCode);

                    // 更新原来父结点中，位于本leaf之后的orders
                    sql =
                            "select code from cws_cms_flash_store_dir where parent_code=" +
                            StrUtil.sqlstr(oldParentCode) +
                            " and orders>" + oldorders;
                    ResultIterator ri = conn.executeQuery(sql);
                    while (ri.hasNext()) {
                        ResultRecord rr = (ResultRecord) ri.next();
                        FlashStoreDirDb dd = getFlashStoreDirDb(rr.getString(1));
                    ...
                    while (ir.hasNext()) {
                        FlashStoreDirDb childlf = (FlashStoreDirDb) ir.next();
                        int layer = parentLayer + 1 + 1;
                        String pcode = childlf.getParentCode();
                        while (!pcode.equals(code)) {
                            layer++;
                            FlashStoreDirDb lfp = getFlashStoreDirDb(pcode);
                    ...
                    // 将其原来的父结点的孩子数-1
                    FlashStoreDirDb oldParentLeaf = getFlashStoreDirDb(
                            oldParentCode);
                    ...
                    // 将其新父结点的孩子数 + 1
                    FlashStoreDirDb newParentLeaf = getFlashStoreDirDb(
                            newParentCode);
    ...
    public boolean AddChild(FlashStoreDirDb childleaf) throws
            ErrMsgException {
        //计算得出插入结点的orders
        int childorders = childCount + 1;

        String updatesql = "";
        String insertsql = "insert into cws_cms_flash_store_dir (code,name,parent_code,description,orders,root_code,child_count,layer,dir_type,add_date) values (?,?,?,?,?,?,?,?,?,?)";
        ...
        try {
            // 更改根结点的信息
            updatesql =
                    "update cws_cms_flash_store_dir set child_count=child_count+1" +
                    " where code=" + StrUtil.sqlstr(code);
            conn.beginTrans();
    ...
    public FlashStoreDirDb getFlashStoreDirDb(String code) {
        FlashStoreDirCache dcm = new FlashStoreDirCache();
        return dcm.getFlashStoreDirDb(code);
    }

    public boolean delsingle(FlashStoreDirDb leaf) {
        String sql = "delete from cws_cms_flash_store_dir where code=" +
        ...
        try {
            ...
            sql =
                    "update cws_cms_flash_store_dir set orders=orders-1 where parent_code=" +
                    StrUtil.sqlstr(leaf.getParentCode()) + " and orders>" +
                    leaf.getOrders();
            jt.executeUpdate(sql);
            sql =
                    "update cws_cms_flash_store_dir set child_count=child_count-1 where code=" +
                    StrUtil.sqlstr(leaf.getParentCode());
            jt.executeUpdate(sql);

            // 删除目录下的所有文件
            FlashStoreFileDb isfd = new FlashStoreFileDb();
            Iterator ir = isfd.listOfDir(leaf.getCode()).iterator();
            while (ir.hasNext()) {
                isfd = (FlashStoreFileDb)ir.next();
                isfd.del(jt);
            }
            jt.commit();
        } catch (SQLException e) {
    ...
    public synchronized void del(FlashStoreDirDb leaf) {
        delsingle(leaf);
        Iterator children = leaf.getChildren().iterator();
        while (children.hasNext()) {
            FlashStoreDirDb lf = (FlashStoreDirDb) children.next();
            del(lf);
        }
    }
    ...
    public FlashStoreDirDb getBrother(String direction) {
        String sql;
        RMConn rmconn = new RMConn(connname);
        FlashStoreDirDb bleaf = null;
        try {
            ...
            if (ri != null && ri.hasNext()) {
                ResultRecord rr = (ResultRecord) ri.next();
                bleaf = getFlashStoreDirDb(rr.getString(1));
            }
        } catch (SQLException e) {
            logger.error(e.getMessage());
        }
    ...
    public boolean move(String direction) {
        ...
        FlashStoreDirDb bleaf = getBrother(direction);
        if (bleaf != null) {
            isexist = true;
        }
        ...
        if (isexist) {
            ...
            try {
                conn.beginTrans();
                if (direction.equals("down")) {
                    sql = "update cws_cms_flash_store_dir set orders=orders+1" +
                          " where code=" + StrUtil.sqlstr(code);
    ...
    public void setQueryList() {
        this.QUERY_LIST = "select code from cws_cms_flash_store_dir";
    }
    ...
    public ObjectDb getObjectDb(Object objKey) {
        return getFlashStoreDirDb(objKey.toString());
    }
