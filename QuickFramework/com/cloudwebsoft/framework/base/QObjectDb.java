...
public abstract class QObjectDb implements java.io.Serializable {
	...
	public boolean initQObject(JdbcTemplate jt, String sql,
		...
		try {
			...
			jt.fillPreparedStatement(ps, objectParams);
			...
			if (rs == null) {
				...
				for (int i = 1; i <= colCount; i++) {
					mapIndex.put(rm.getColumnName(i).toUpperCase(),
							new Integer(i));
	...
	public synchronized boolean save(JdbcTemplate jt, ParamChecker paramChecker)
		...
		if (resultRecord == null) {
			throw new IllegalArgumentException(
					"resultRecord is null, may because of object is not loaded.");
		}
		for (int i = 0; i < len; i++) {
			try {
				resultRecord.set(fieldsForSave[i], paramChecker
						.getValue(fieldsForSave[i]));
			...
			if (rs == null) {
				...
				do {
					if (primaryKey.getType() == PrimaryKey.TYPE_INT) {
						...
						result.addElement(getQObjectDb(new java.util.Date(rs
								.getTimestamp(idIndex).getTime())));
					else if (primaryKey.getType() == PrimaryKey.TYPE_COMPOUND) {
						...
						while (ir.hasNext()) {
							...
							KeyUnit ku = (KeyUnit) keys.get(keyName);
							if (ku.getType() == primaryKey.TYPE_INT) {
								ku.setValue(new Integer(rs.getInt(ku
										.getOrders() + 1)));
							} else if (ku.getType() == primaryKey.TYPE_LONG) {
								ku.setValue(new Long(rs
										.getLong(ku.getOrders() + 1)));
							} else if (ku.getType() == PrimaryKey.TYPE_DATE) {
								ku.setValue(new java.util.Date(rs.getTimestamp(
										ku.getOrders() + 1).getTime()));
							} else {
		...
		try {
			...
			if (rs == null) {
				...
				do {
					if (primaryKey.getType() == PrimaryKey.TYPE_INT)
						result
								.addElement(getQObjectDb(new Integer(rs
										.getInt(1))));
					else if (primaryKey.getType() == PrimaryKey.TYPE_STRING)
						result.addElement(getQObjectDb(rs.getString(1)));
					else if (primaryKey.getType() == PrimaryKey.TYPE_LONG)
						result
								.addElement(getQObjectDb(new Long(rs.getLong(1))));
					else if (primaryKey.getType() == primaryKey.TYPE_DATE)
						result.addElement(getQObjectDb(new java.util.Date(rs
