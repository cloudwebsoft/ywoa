...
function findObjInForm(formName, objName) {
	return findObjInFormObj(o(formName), objName);
}

function findObjInFormObj(formObj, objName) {
	var obj = $(formObj).find('[name=' + objName + ']')[0];
	// console.log('findObjInFormObj formObj', formObj, 'objName', objName, 'obj', obj);
	if (obj == null) {
		return $(formObj).find('[id=' + objName + ']')[0];
	}
	else {
		return obj;
	}
...
function findObjByFormCode(formCode, objName) {
	var obj = $("form[formcode='" + formCode + "']").find('[name=' + objName + ']')[0];
	if (obj == null) {
		return $("form[formcode='" + formCode + "']").find('[id=' + objName + ']')[0];
	}
	else {
		return obj;
	}
...
function fo(objName, formId) {
	return findObj(objName, formId);
}

function findObj(objName, formId) {
	var formObj;
	if (formId) {
		formObj = o(formId);
	} else {
	...
	// console.log('formObj', formObj.id, 'objName', objName);
	if (formObj != null) {
		var obj = $(formObj).find("[name='" + objName + "']")[0];
		if (obj == null) {
			return $(formObj).find("[id='" + objName + "']")[0];
...
function findObjs(objName) {
	var formObj = getCurForm();
	if (formObj != null) {
		// console.log('formObj', formObj);
		var obj = $(formObj).find('[name=' + objName + ']')[0];
		if (obj == null) {
			var arr = [];
			$(formObj).find('[id=' + objName + ']').each(function() {
				arr.push(this);
			});
			return arr;
		}
			...
			var arr = [];
			$(formObj).find('[name=' + objName + ']').each(function() {
				arr.push(this);
			})
			return arr;
		}
...
// setCurrentFormObj后currentFormObj驻留内存，下次打开drawer再进后getCurForm找到的仍是之前的currentFormObj
function setCurrentFormObj(obj) {
	currentFormObj = obj;
}

function getCurrentFormObj() {
	return currentFormObj;
}

function destroyCurrentFormObj() {
	currentFormObj = null;
}
...
function getCurForm() {
	...
	// 在本方法中按visualFormRelate、visualForm、flowForm的优先级，可能会有例外的情况，故加入了getCurFormId方法以解决此问题
	if (typeof getCurFormId == 'function') {
		var curFormId = getCurFormId();
		// console.log('curFormId', curFormId);
		if (curFormId) {
			return o(curFormId);
		}
	}
	...
	var formObj = null;
	// 因为在filterJS中destroyCurrentFormObj，故currentFormObj为null，此时根据优先级来取formObj，visualFormRelate为嵌套表格的添加、修改页面，故优先级最高
	if (o('visualFormRelate')) {
		formObj = o('visualFormRelate');
	}
	else if (o('visualForm')) {
		formObj = o('visualForm');
	}
	else {
		formObj = o('flowForm');
	}
...
function setCtlValue(ctlName, ctlType, ctlValue, nestFormCode) {
	// 防止添加时，抽屉未完全打开，即关闭，致列表上方条件中的同名字段被置为undefined，经前端nextTick getVisable处理后，该问题已不存在
	// console.log('setCtlValue ctlName', ctlName, 'ctlValue', ctlValue);
	if (ctlValue == undefined) {
		console.log(ctlName + '  ctlValue is undefined');
		return;
	}
	if (ctlValue) {
	...
	try {
		// @task:当ctlValue为visualForm.cws_textarea_province.value且为空时，赋值后hideCtlect控件不能显示<option value=''>无</option>，而一定要将ctlValue置为""
		if (ctlValue == "")
			ctlValue = "";
		var obj = fo(ctlName);
		// 如果是嵌套表格nest_table
		if (nestFormCode != null) {
			...
			console.log('$nestTable[0]', $nestTable[0]);
			if ($nestTable[0]) {
...
function DisableCtl(name, ctlType, ctlValue, ctlValueRaw) {
	var curForm = getCurForm();
	if (!curForm) {
		console.warn('DisableCtl curForm 不存在');
		return;
	}
	...
	for(var i=0; i < len; i++) {
		...
		if (obj.name==name) {
			if (ctlType=="DATE" || ctlType=="DATE_TIME") {
				...
				obj.insertAdjacentHTML("AfterEnd", "<input type=hidden name='" + name + "' value='" + ctlValueRaw + "'  isDisabled='true' />");
				...
				var v = obj.checked;
				if (v) {
					obj.insertAdjacentHTML("AfterEnd", "<input type=hidden name='" + name + "' value='1' isDisabled='true' />");
					//obj.outerHTML = "(是)";
					obj.outerHTML = "<span id='" + name + "_show'>" + "<img src='" + getPublicPath() + "resource/images/checkbox_y.gif' align='absMiddle'></span>";
				}
				else {
					obj.insertAdjacentHTML("AfterEnd", "<input type=hidden name='" + name + "' value='0' isDisabled='true' />");
					// obj.outerHTML = "(否)";
					obj.outerHTML = "<span id='" + name + "_show'>" + "<img src='" + getPublicPath() + "resource/images/checkbox_n.gif' align='absMiddle'></span>";
				}
			}
			else if (ctlType == "select") {
				var text = $("[name='" + name + "'] option[value='" + ctlValueRaw + "']").text();
				obj.outerHTML = "<span id='" + name + "_show'>" + text + "</span><textarea style='display:none' name='" + name + "' isDisabled='true'>" + ctlValueRaw + "</textarea>";
			}			
			else if (ctlType=="radio") {
				// radio元素有多个，此处将所有的同名元素全部置为disabled
				...
				if (radioboxs!=null) {
					...
					for (j=0; j < radioboxs.length; j++) {
						if (radioboxs[j].disabled) {
							...
							// 防止将下面insertAdjacentHTML插入的hidden元素也置为disabled
							if (radioboxs[j].type != 'hidden') {
					...
					// 如果还没有radio元素被disabled，则插入hidden，这样只会插入一次
					if (!isSomeDisabled) {
						// 注意顺序，必须在后面一行，因为radioboxs会动态变化
						radioboxs[0].insertAdjacentHTML("AfterEnd", "<input type=hidden name='" + name + "' value='" + ctlValue + "' isDisabled='true' />");
					}
				}
			}
			else {
				...
				child.innerHTML = "<span id='" + name + "_show'>" + ctlValueShow + "</span><textarea style='display:none' name='" + name + "' isDisabled='true'>" + ctlValueRaw + "</textarea>";
				...
				if (findObj(name + "_btn")) {
					findObj(name + "_btn").style.display = "none";
				}
			}
...
function HideCtl(name, ctlType, macroType) {
	 var curForm = getCurForm();
	 if (!curForm) {
		console.warn('HideCtl curForm 不存在');
		return;
	}
   var len = curForm.elements.length;
   var isFound = false;
   for(var i=0;i < len;i++) {
		...
		if (obj.name==name) {
          ...
          if (btnImgObj)
				...
				else if (o(name + "_realname")) {
					// 当前用户宏控件
					o(name + "_realname").style.display = "none";
				}
				else if (o(name + "_realshow")) {
					// 表单域选择宏控件
				  o(name + "_realshow").style.display = "none";
					var btnObj = o(name + "_btn");
					if (btnObj) {
   ...
   for(var i=0;i < len;i++) {
		...
		if (obj.name==name) {
			if (ctlType=="checkbox") {
				...
				var $objs = $(curForm).find('input[name="' + name + '"][type="radio"]');
				$objs.each(function() {
						var el = this;
						if (el.value==ctlValue)
							el.outerHTML = "<span id='" + name + "_show'><img src='" + getPublicPath() + "resource/images/radio_y.gif' align='absMiddle'></span>";
						else
							el.outerHTML = "<span id='" + name + "_show'><img src='" + getPublicPath() + "resource/images/radio_n.gif' align='absMiddle'></span>";
				});
			}
			else {
			...
			var objRealShow = findObj(obj.name + '_realshow');
			if (objRealShow) {
				objRealShow.style.display = 'none';
			}
			var objBtn = findObj(obj.name + '_btn');
			if (objBtn) {
	...
	$("input[kind='DATE']").each(function() {
      ...
      var dateObj = this;
		...
		try {
			...
			$(dateObj).datetimepicker(dtOption);
		} catch (e) {}
	});
	$("input[kind='DATE_TIME']").each(function() {
      ...
      var dateObj = this;
		...
		try {
			...
			// $('#' + $(this).attr("id")).datetimepicker(dtOption);
			console.log('initDatePicker dateObjTime', dateObj);
			$(dateObj).datetimepicker(dtOption);
		} catch (e) {}
	});
	if (noName) {
