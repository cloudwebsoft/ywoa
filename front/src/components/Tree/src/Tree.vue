<script lang="tsx">
  ...
  export default defineComponent({
    ...
    setup(props, { attrs, slots, emit, expose }) {
      ...
      //开始拖拽
      function handleDragstart({ event, node }: AntTreeNodeDragEnterEvent) {
        // console.log('开始', event, node);
      }

      function handleDragenter({ event, node }: AntTreeNodeDragEnterEvent) {
        // console.log('handleDragenter', event, node);
      }

      const handleDrop = (info: AntTreeNodeDragEnterEvent) => {
        //   console.log(info);
        ...
        let dragObj: TreeDataItem; // Find dragObject
        let dragObjNewParent: TreeDataItem;
        ...
        if (!info.dropToGap) {
          loop(data, dropKey, (item: TreeDataItem) => {
            ...
            item.children.unshift(dragObj); /// where to insert 示例添加到头部，可以是随意位置
          });
        } else if (
        ...
        dragObjNewParent = getParent(treeDataRef.value, dragObj[props.treeKey]);
        emit('drop', info, dragObj, dragObjNewParent, treeDataRef.value); //当前移动级，移动到的父级，全部数据
      };
      // data:json， nodeId:节点
      function getParent(data2, nodeId2) {
        var arrRes = {};
        if (data2.length == 0) return arrRes;

        let rev = (data, nodeId) => {
          for (var i = 0, length = data.length; i < length; i++) {
            let node = data[i];
            if (!!node.children) {
              let isParent = node.children.some((iv) => iv[props.treeKey] === nodeId);
              if (isParent) {
                arrRes = node;
                break;
              } else {
                rev(node.children, nodeId2);
              }
            }
          }
          return arrRes;
      ...
      expose(instance);
      console.log('unref(getBindValues)', unref(getBindValues));
      return () => {
