          ...
          <template #default="{ text }">
            <template v-if="activeType === 'month'">
              ...
              <div v-else class="min-h-40 flex flex-col justify-items-start">
                ...
                <div class="max-h-30 overflow-auto">
                  <div v-for="(d, i) in text.con" :key="i" class="pb-2">
                    <span
                      class="text-left hover:text-red-400 hover:underline cursor-pointer"
                      @click="clickItmeInfo(d)"
                    >
                      <div>
                        {{ d.start ? dayjs(d.start).format('HH:mm') : '-' }} ~
                        {{ d.end ? dayjs(d.end).format('HH:mm') : '-' }}
                        {{ d.title }}
                      </div>
                    </span>
                  </div>
            ...
            <template v-else-if="activeType === 'date' && item.dataIndex == 'con1'">
              <span v-if="text && text.length > 0">
                <div v-for="(d, i) in text" :key="i" class="text-left pb-2">
                  <span
                    class="text-left hover:text-red-400 hover:underline cursor-pointer"
                    @click="clickItmeInfo(d)"
                  >
...
<script lang="ts">
  ...
  import { DatePicker, Radio, Table, Dropdown, Menu } from 'ant-design-vue';
  import { useTable } from '/@/components/Table';
  import type { Dayjs } from 'dayjs';
  import { dateUtil as dayjs } from '/@/utils/dateUtil';
  import { getvisualListCalendar } from '/@/api/module/module';
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const y = ref(500);
      const moduleListPage = ref<Recordable>({});
      const moduleSearchParams = ref<Recordable>({});

      // watch(
      //   [props.listPage, props.searchParams],
      //   ([newListPage, newParams], [oldListPage, oldParams]) => {
      //     console.log('watch 已触发: newListPage', newListPage);
      //     console.log('watch 已触发: newParams', newParams);
      //     moduleListPage.value = newListPage;
      //     if (moduleListPage.value.moduleCode) {
      //       setTimeout(() => {
      //         init();
      //       }, 10);
      //     }
      ...
      watch(
        ...
        (newParams: object) => {
          console.log('watch 已触发: newParams', newParams);
          moduleSearchParams.value = newParams;
          if (moduleListPage.value.moduleCode) {
            setTimeout(() => {
              // init();
              changeDate();
      ...
      function setWeekTableCol(data) {
        ...
        if (data.length > 0) {
          ...
          if (unref(activeType) == 'date') {
            getvisualListCalendar({
              code: moduleListPage.value.moduleCode,
              start: dayjs(unref(date)).format('YYYY-MM-DD'),
              end: dayjs(unref(date)).format('YYYY-MM-DD'),
              ...moduleSearchParams.value,
              op: 'search',
            }).then((res) => {
              if (res && res.length > 0) {
                const list = res;
            ...
            getvisualListCalendar({
              code: moduleListPage.value.moduleCode,
              start: dayjs(unref(date)).startOf('month').format('YYYY-MM-DD'),
              end: dayjs(unref(date)).endOf('month').format('YYYY-MM-DD'),
              ...moduleSearchParams.value,
              op: 'search',
            }).then((res) => {
              if (res && res.length > 0) {
      ...
      function clickItmeInfo(record: Recordable) {
        openDrawer(true, {
          record: {
            moduleCode: moduleListPage.value.moduleCode,
            id: record['id'],
            formCode: moduleListPage.value.formCode,
          },
          isUpdate: 3,
        });
      }
