...
@Slf4j
@Component
public class UserAuthorityCache extends ObjCache {

    @Autowired
    IUserAuthorityService userAuthorityService;

    private final String PREFIX_USER = "authority_";
    ...
    public void refreshUserAuthorities(String userName) {
        try {
            ...
            LogUtil.getLog(getClass()).error("refreshUserAuthorities:" + e.getMessage());
        }
    }
    ...
    public List<String> getUserAuthorities(String userName) {
        ...
        try {
            ...
            if (null != list) {
                // 防穿透
                if (ConstUtil.CACHE_NONE.equals(list.get(0))) {
