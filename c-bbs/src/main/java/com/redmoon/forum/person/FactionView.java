...
public class FactionView {
    Logger logger = Logger.getLogger(FactionView.class.getName());
    FactionDb rootLeaf;
    Vector UprightLineNodes = new Vector(); //用于显示竖线

    public FactionView(FactionDb rootLeaf) {
    ...
    // 显示根结点为leaf的树
    void ListTreeSimple(JspWriter out, FactionDb leaf,
     ...
     **/
    void ShowLeafSimple(JspWriter out, FactionDb leaf,
                  boolean isLastChild, String target, String link, String tableClass, String tableClassMouseOn) throws Exception {
        ...
        if (!isLastChild) {
            FactionDb brotherleaf = leaf.getBrother("down");
            ...
            if (brotherleaf != null) {
                ...
                int count = r.size();
                if (count>0) {
                    FactionUprightLineNode uln = new FactionUprightLineNode(layer, count);
        ...
        for (int k = rootLeaf.getLayer(); k <= layer - 1; k++) {
            ...
            while (ir.hasNext()) {
                FactionUprightLineNode node = (FactionUprightLineNode) ir.next();
                //如果在K层上存在一个竖线结点则画出
                if (node.getLayer() == k) {
    ...
    // 显示根结点为leaf的树
    void ListTree(JspWriter out, FactionDb leaf,
...
*/
     ...
     * isLastChild 是否为其父亲结点的最后一个孩子结点
     **/
    void ShowLeaf(JspWriter out, FactionDb leaf,
                  boolean isLastChild) throws Exception {
        String code = leaf.getCode();
        String name = leaf.getName();
        int layer = leaf.getLayer();
        ...
        if (!leaf.getCode().equals("root")) {
            //if (leaf.getIsHome())
                //out.print("首页&nbsp;");
            // out.print("<a href='faction_top.jsp?op=removecache&code=" + code +
            //          "'>清缓存</a>&nbsp;");
            if (leaf.getType() == leaf.TYPE_LIST)
                out.print("列表&nbsp;");
            else
            ...
            /*
            out.print(
                    "<a target=_parent href='faction_priv_m.jsp?dirCode=" +
                    StrUtil.UrlEncode(code, "utf-8") + "'>权限</a>&nbsp;");
            */
            out.print(
                    "<a target=dirbottomFrame href='faction_bottom.jsp?parent_code=" +
                    StrUtil.UrlEncode(code, "utf-8") + "&parent_name=" +
                    StrUtil.UrlEncode(name, "utf-8") +
                    "&op=AddChild'>添子目录</a>&nbsp;");
            out.print(
                    "<a target='dirbottomFrame' href='faction_bottom.jsp?op=modify&code=" +
            ...
            out.print(
                    ...
                    name +
                    "吗?')) window.location.href='faction_top.jsp?op=del&root_code=" + StrUtil.UrlEncode(rootLeaf.getCode()) + "&delcode=" +
                    StrUtil.UrlEncode(code, "utf-8") + "'\">删除</a>&nbsp;");
            out.print("<a href='faction_top.jsp?op=move&direction=up&root_code=" + StrUtil.UrlEncode(rootLeaf.getCode()) + "&code=" +
                      StrUtil.UrlEncode(code, "utf-8") + "'>上移</a>&nbsp;");
            out.print("<a href='faction_top.jsp?op=move&direction=down&root_code=" + StrUtil.UrlEncode(rootLeaf.getCode()) + "&code=" +
                      StrUtil.UrlEncode(code, "utf-8") + "'>下移</a>&nbsp;");
            out.print("<a href='faction_top.jsp?root_code=" +
                      StrUtil.UrlEncode(code, "utf-8") + "'>管理</a>");
        }
        out.println("  </td></tr></tbody></table>");
    }

    void ShowLeafAsOption(JspWriter out, FactionDb leaf, int rootlayer)
                  throws Exception {
        String code = leaf.getCode();
        String name = leaf.getName();
        int layer = leaf.getLayer();
        ...
        if (leaf.getChildCount()>0) {
            if (leaf.getType()==leaf.TYPE_LIST)
                ...
                out.print("<option value='" + FactionDb.TYPE_NONE_OPTION_VALUE +"'>" + blank + "╋ " + name + "</option>");
        }
        else {
            if (leaf.getType()==leaf.TYPE_LIST)
                ...
                out.print("<option value='" + FactionDb.TYPE_NONE_OPTION_VALUE + "'>" + blank + "├『" + name +
                      "』</option>");
        }
    }

    public void ShowLeafAsOptionToString(StringBuffer sb, FactionDb leaf, int rootlayer)
                  throws Exception {
        String code = leaf.getCode();
        String name = leaf.getName();
        int layer = leaf.getLayer();
        ...
        if (leaf.getChildCount()>0) {
            if (leaf.getType()==leaf.TYPE_LIST)
                ...
                sb.append("<option value='" + FactionDb.TYPE_NONE_OPTION_VALUE + "'>" + blank + "╋ " + name + "</option>");
        }
        else {
            if (leaf.getType()==leaf.TYPE_LIST)
                ...
                sb.append("<option value='" + FactionDb.TYPE_NONE_OPTION_VALUE + "'>" + blank + "├『" + name +
                      "』</option>");
        }
    }

    // 显示根结点为leaf的树
    public void ShowDirectoryAsOptions(JspWriter out, FactionDb leaf, int rootlayer) throws Exception {
    ...
    // 显示根结点为leaf的树
    public void ShowDirectoryAsOptionsToString(StringBuffer sb, FactionDb leaf, int rootlayer) throws Exception {
     ...
     * 把列表或无内容显示为蓝色
     * @param out JspWriter
     * @param leaf FactionDb
     * @param rootlayer int
     * @throws Exception
     */
    void ShowLeafAsOptionWithCode(JspWriter out, FactionDb leaf, int rootlayer)
                  throws Exception {
        String code = leaf.getCode();
        String name = leaf.getName();
        int layer = leaf.getLayer();
    ...
    public void ShowDirectoryAsOptionsWithCode(JspWriter out, FactionDb leaf, int rootlayer) throws Exception {
