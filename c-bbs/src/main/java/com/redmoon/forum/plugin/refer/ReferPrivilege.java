...
public class ReferPrivilege implements IPluginPrivilege {
    static Logger logger = Logger.getLogger(ReferPrivilege.class.getName());

    public ReferPrivilege() {
    }
    ...
    public boolean canAddNew(HttpServletRequest request, String boardCode, FileUpload fu) throws ErrMsgException {
        ...
        if (!au.isPluginBoard(boardCode))
            throw new ErrMsgException(ReferSkin.LoadString(request, "addNewErrorBoardInvalid"));
        return true;
    }
