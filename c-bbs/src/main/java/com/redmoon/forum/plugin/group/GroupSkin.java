...
public class GroupSkin implements ISkin {
    static Logger logger = Logger.getLogger(GroupSkin.class.getName());
    public static String resource = null;
    public static String code = GroupUnit.code;

    static final String GROUP = "plugin.group";
    ...
    private static Skin getSkinByCode(String skinCode) {
        Skin sk = null;
        try {
            sk = (Skin) RMCache.getInstance().getFromGroup(skinCode, GROUP);
        } catch (Exception e) {
        ...
        if (sk == null) {
            ...
            while (ir.hasNext()) {
                ...
                if (skin.getCode().equals(skinCode)) {
                    sk = skin;
                    try {
                        RMCache.getInstance().putInGroup(skinCode, GROUP,
                                sk);
                    } catch (Exception e) {
