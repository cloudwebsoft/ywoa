...
public class MusicDirDb extends ObjectDb implements Serializable {
    ...
    public ObjectDb getObjectRaw(PrimaryKey pk) {
        return new MusicDirDb(pk.getStrValue());
    }
    ...
    public void setQuerySave() {
        this.QUERY_SAVE = "update sq_forum_music_dir set name=?,description=?,dir_type=?,orders=?,child_count=?,layer=? where code=?";
    }
    ...
    public void setQueryLoad() {
        this.QUERY_LOAD = "select code,name,description,parent_code,root_code,orders,layer,child_count,add_date,dir_type from sq_forum_music_dir where code=?";
    }
    ...
    public Vector getChildren() {
        Vector v = new Vector();
        String sql =
                "select code from sq_forum_music_dir where parent_code=? order by orders";
        Conn conn = new Conn(connname);
        ResultSet rs = null;
        try {
     ...
     */
    public Vector getAllChild(Vector vt, MusicDirDb leaf) throws
            ErrMsgException {
    ...
    public synchronized boolean save() {
        ...
        try {
            ...
            try {
                if (r == 1) {
                    re = true;
                    MusicDirCache dcm = new MusicDirCache();
    ...
    public synchronized boolean save(String newParentCode) {
        ...
        MusicDirDb lfparent = getMusicDirDb(newParentCode);
        int oldorders = orders;
        int neworders = lfparent.getChildCount() + 1;
        int parentLayer = lfparent.getLayer();
        String sql = "update sq_forum_music_dir set name=" +
                     StrUtil.sqlstr(name) +
        ...
        try {
            ...
            try {
                if (r == 1) {
                    ...
                    // DirListCacheMgr更新
                    MusicDirChildrenCache.remove(oldParentCode);
                    MusicDirChildrenCache.remove(newParentCode);

                    // 更新原来父结点中，位于本leaf之后的orders
                    sql =
                            "select code from sq_forum_music_dir where parent_code=" +
                            StrUtil.sqlstr(oldParentCode) +
                            " and orders>" + oldorders;
                    ResultIterator ri = conn.executeQuery(sql);
                    while (ri.hasNext()) {
                        ResultRecord rr = (ResultRecord) ri.next();
                        MusicDirDb dd = getMusicDirDb(rr.getString(1));
                    ...
                    // 将其原来的父结点的孩子数-1
                    MusicDirDb oldParentLeaf = getMusicDirDb(
                            oldParentCode);
                    ...
                    // 将其新父结点的孩子数 + 1
                    MusicDirDb newParentLeaf = getMusicDirDb(
                            newParentCode);
    ...
    public boolean AddChild(MusicDirDb childleaf) throws
            ErrMsgException {
        //计算得出插入结点的orders
        int childorders = childCount + 1;

        String updatesql = "";
        String insertsql = "insert into sq_forum_music_dir (code,name,parent_code,description,orders,root_code,child_count,layer,dir_type,add_date) values (?,?,?,?,?,?,?,?,?,?)";
        ...
        try {
            // 更改根结点的信息
            updatesql =
                    "update sq_forum_music_dir set child_count=child_count+1" +
                    " where code=" + StrUtil.sqlstr(code);
            conn.beginTrans();
    ...
    public MusicDirDb getMusicDirDb(String code) {
        MusicDirCache dcm = new MusicDirCache();
        return dcm.getMusicDirDb(code);
    }

    public boolean delsingle(MusicDirDb leaf) {
        String sql = "delete from sq_forum_music_dir where code=" +
        ...
        try {
            ...
            sql =
                    "update sq_forum_music_dir set orders=orders-1 where parent_code=" +
                    StrUtil.sqlstr(leaf.getParentCode()) + " and orders>" +
                    leaf.getOrders();
            jt.executeUpdate(sql);
            sql =
                    "update sq_forum_music_dir set child_count=child_count-1 where code=" +
    ...
    public synchronized void del(MusicDirDb leaf) {
        delsingle(leaf);
        Iterator children = leaf.getChildren().iterator();
        while (children.hasNext()) {
            MusicDirDb lf = (MusicDirDb) children.next();
            del(lf);
        }
    }
    ...
    public MusicDirDb getBrother(String direction) {
        String sql;
        RMConn rmconn = new RMConn(connname);
        MusicDirDb bleaf = null;
        try {
            ...
            if (ri != null && ri.hasNext()) {
                ResultRecord rr = (ResultRecord) ri.next();
                bleaf = getMusicDirDb(rr.getString(1));
            }
        } catch (SQLException e) {
            logger.error(e.getMessage());
        }
    ...
    public boolean move(String direction) {
        ...
        MusicDirDb bleaf = getBrother(direction);
        if (bleaf != null) {
            isexist = true;
        }
        ...
        if (isexist) {
            ...
            try {
                conn.beginTrans();
                if (direction.equals("down")) {
                    sql = "update sq_forum_music_dir set orders=orders+1" +
                          " where code=" + StrUtil.sqlstr(code);
    ...
    public void setQueryList() {
        this.QUERY_LIST = "select code from sq_forum_music_dir";
    }
    ...
    public ObjectDb getObjectDb(Object objKey) {
        return getMusicDirDb(objKey.toString());
    }
