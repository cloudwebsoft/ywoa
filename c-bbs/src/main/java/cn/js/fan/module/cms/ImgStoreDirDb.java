...
public class ImgStoreDirDb extends ObjectDb implements Serializable {
    ...
    public ImgStoreDirDb() {
        init();
    }

    public ImgStoreDirDb(String code) {
        this.code = code;
        load();
        init();
    }
    ...
    public ObjectDb getObjectRaw(PrimaryKey pk) {
        return new ImgStoreDirDb(pk.getStrValue());
    }
    ...
    public void setQuerySave() {
        this.QUERY_SAVE = "update cws_cms_img_store_dir set name=?,description=?,dir_type=?,orders=?,child_count=?,layer=? where code=?";
    }
    ...
    public void setQueryLoad() {
        this.QUERY_LOAD = "select code,name,description,parent_code,root_code,orders,layer,child_count,add_date,dir_type from cws_cms_img_store_dir where code=?";
    }
    ...
    public Vector getChildren() {
        Vector v = new Vector();
        String sql =
                "select code from cws_cms_img_store_dir where parent_code=? order by orders";
        Conn conn = new Conn(connname);
        ResultSet rs = null;
        try {
            ...
            if (rs != null) {
                while (rs.next()) {
                    ...
                    v.addElement(getImgStoreDirDb(c));
                }
            }
        } catch (SQLException e) {
     ...
     */
    public Vector getAllChild(Vector vt, ImgStoreDirDb leaf) throws
            ErrMsgException {
    ...
    public synchronized boolean save() {
        ...
        try {
            ...
            try {
                if (r == 1) {
                    re = true;
                    ImgStoreDirCache dcm = new ImgStoreDirCache();
    ...
    public synchronized boolean save(String newParentCode) {
        ...
        ImgStoreDirDb lfparent = getImgStoreDirDb(newParentCode);
        int oldorders = orders;
        int neworders = lfparent.getChildCount() + 1;
        int parentLayer = lfparent.getLayer();
        String sql = "update cws_cms_img_store_dir set name=" +
                     StrUtil.sqlstr(name) +
        ...
        try {
            ...
            try {
                if (r == 1) {
                    ...
                    // DirListCacheMgr更新
                    ImgStoreDirChildrenCache.remove(oldParentCode);
                    ImgStoreDirChildrenCache.remove(newParentCode);

                    // 更新原来父结点中，位于本leaf之后的orders
                    sql =
                            "select code from cws_cms_img_store_dir where parent_code=" +
                            StrUtil.sqlstr(oldParentCode) +
                            " and orders>" + oldorders;
                    ResultIterator ri = conn.executeQuery(sql);
                    while (ri.hasNext()) {
                        ResultRecord rr = (ResultRecord) ri.next();
                        ImgStoreDirDb dd = getImgStoreDirDb(rr.getString(1));
                    ...
                    while (ir.hasNext()) {
                        ImgStoreDirDb childlf = (ImgStoreDirDb) ir.next();
                        int layer = parentLayer + 1 + 1;
                        String pcode = childlf.getParentCode();
                        while (!pcode.equals(code)) {
                            layer++;
                            ImgStoreDirDb lfp = getImgStoreDirDb(pcode);
                    ...
                    // 将其原来的父结点的孩子数-1
                    ImgStoreDirDb oldParentLeaf = getImgStoreDirDb(
                            oldParentCode);
                    ...
                    // 将其新父结点的孩子数 + 1
                    ImgStoreDirDb newParentLeaf = getImgStoreDirDb(
                            newParentCode);
    ...
    public boolean AddChild(ImgStoreDirDb childleaf) throws
            ErrMsgException {
        //计算得出插入结点的orders
        int childorders = childCount + 1;

        String updatesql = "";
        String insertsql = "insert into cws_cms_img_store_dir (code,name,parent_code,description,orders,root_code,child_count,layer,dir_type,add_date) values (?,?,?,?,?,?,?,?,?,?)";
        ...
        try {
            // 更改根结点的信息
            updatesql =
                    "update cws_cms_img_store_dir set child_count=child_count+1" +
                    " where code=" + StrUtil.sqlstr(code);
            conn.beginTrans();
    ...
    public ImgStoreDirDb getImgStoreDirDb(String code) {
        ImgStoreDirCache dcm = new ImgStoreDirCache();
        return dcm.getImgStoreDirDb(code);
    }

    public boolean delsingle(ImgStoreDirDb leaf) {
        String sql = "delete from cws_cms_img_store_dir where code=" +
        ...
        try {
            ...
            sql =
                    "update cws_cms_img_store_dir set orders=orders-1 where parent_code=" +
                    StrUtil.sqlstr(leaf.getParentCode()) + " and orders>" +
                    leaf.getOrders();
            jt.executeUpdate(sql);
            sql =
                    "update cws_cms_img_store_dir set child_count=child_count-1 where code=" +
    ...
    public synchronized void del(ImgStoreDirDb leaf) {
        delsingle(leaf);
        Iterator children = leaf.getChildren().iterator();
        while (children.hasNext()) {
            ImgStoreDirDb lf = (ImgStoreDirDb) children.next();
            del(lf);
        }
    }
    ...
    public ImgStoreDirDb getBrother(String direction) {
        String sql;
        RMConn rmconn = new RMConn(connname);
        ImgStoreDirDb bleaf = null;
        try {
            ...
            if (ri != null && ri.hasNext()) {
                ResultRecord rr = (ResultRecord) ri.next();
                bleaf = getImgStoreDirDb(rr.getString(1));
            }
        } catch (SQLException e) {
            logger.error(e.getMessage());
        }
    ...
    public boolean move(String direction) {
        ...
        ImgStoreDirDb bleaf = getBrother(direction);
        if (bleaf != null) {
            isexist = true;
        }
        ...
        if (isexist) {
            ...
            try {
                conn.beginTrans();
                if (direction.equals("down")) {
                    sql = "update cws_cms_img_store_dir set orders=orders+1" +
                          " where code=" + StrUtil.sqlstr(code);
    ...
    public void setQueryList() {
        this.QUERY_LIST = "select code from cws_cms_img_store_dir";
    }
    ...
    public ObjectDb getObjectDb(Object objKey) {
        return getImgStoreDirDb(objKey.toString());
    }
