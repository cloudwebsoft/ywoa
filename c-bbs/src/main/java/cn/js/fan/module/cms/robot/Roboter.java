...
import cn.js.fan.module.cms.DocImage;
...
public class Roboter {
    ...
    public Document gatherDocAndCreate(String docSrc, RobotDb rd)
        ...
        doc.create(rd.getString("dir_code"), title, title, 0, "admin", -1, author);
        doc = doc.getDocument(doc.getId());
        try {
            doc.UpdateExamine(rd.getInt("examine"));
            doc.UpdateSource(from);
    ...
    public boolean gatherDocument(HttpServletRequest request, String docUrl, RobotDb rd, String relativePath)
        ...
        boolean re = gatherDocContentOfPage(docUrl, docSrc, rd, doc, curPageNum, relativePath);
        if (!re) {
            return false;
        }
        ...
        while (!q.isEmpty()) {
            ...
            boolean ret = gatherDocContentOfPage(docUrl, docPageSrc, rd, doc, curPageNum + 1, relativePath);
            if (!ret)
                return false;
            curPageNum++;
    ...
    public boolean gatherDocContentOfPage(String docUrl, String docSrc, RobotDb rd, Document doc, int pageNum, String relativePath) {
        String content = "";
        ...
        content = RobotUtil.fixFlashLinkAndDownload(docUrl, content, rd.getString("charset"), rd.getString("doc_img_flash_prefix"), isDownloadFlash, relativePath, v3);
    ...
    public static String getRelativePath() {
        ...
        Config cfg = new Config();
        String path = cfg.getProperty("cms.file_webedit") + "/" + year + "/" + month;
    ...
    public static Vector getDocUrlsOfList(String listPageUrl, RobotDb rd)
        ...
        if (v.size() == 0) {
            throw new ErrMsgException(listPageUrl + " 未找到列表区域！");
        }
