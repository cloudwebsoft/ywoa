...
public class ImgStoreDirCache implements ICacheMgr {
    final String group = "CMS_IMAGE_STORE";

    static boolean isRegisted = false;

    Logger logger = Logger.getLogger(ImgStoreDirCache.class.getName());
    RMCache rmCache = RMCache.getInstance();

    String connname = "";

    public ImgStoreDirCache() {
 ...
 */
    ...
    public ImgStoreDirDb getImgStoreDirDb(String code) {
        ImgStoreDirDb leaf = null;
        try {
            leaf = (ImgStoreDirDb) rmCache.getFromGroup(code, group);
        } catch (Exception e) {
            logger.error("getImgStoreDirDb1:" + e.getMessage());
        }
        if (leaf == null) {
            leaf = new ImgStoreDirDb(code);
            if (leaf != null) {
                try {
                    rmCache.putInGroup(code, group, leaf);
                } catch (Exception e) {
                    logger.error("getImgStoreDirDb2:" + e.getMessage());
                }
            }
        } else
            leaf.renew();
