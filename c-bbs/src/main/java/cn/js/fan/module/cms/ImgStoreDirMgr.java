...
public class ImgStoreDirMgr {
    String connname = "";
    Logger logger = Logger.getLogger(ImgStoreDirMgr.class.getName());

    public ImgStoreDirMgr() {
    ...
    public boolean AddChild(HttpServletRequest request) throws
        ...
        ImgStoreDirDb lf = new ImgStoreDirDb();
        lf.setName(name);
        lf.setCode(code);
        ...
        ImgStoreDirDb dd = getImgStoreDirDb(parent_code);
        return dd.AddChild(lf);
    }

    public void del(String delcode) throws ErrMsgException {
        ImgStoreDirDb lf = getImgStoreDirDb(delcode);
        lf.del(lf);
    }
    ...
    public synchronized boolean move(HttpServletRequest request) throws
        ...
        ImgStoreDirDb dd = getImgStoreDirDb(code);
        return dd.move(direction);
    }

    public ImgStoreDirDb getImgStoreDirDb(String code) {
        ImgStoreDirDb dd = new ImgStoreDirDb();
        return dd.getImgStoreDirDb(code);
    }

    public ImgStoreDirDb getBrother(String code, String direction) throws
            ErrMsgException {
        ImgStoreDirDb dd = getImgStoreDirDb(code);
     ...
     */
    public void repairLeaf(ImgStoreDirDb lf) {
        ...
        int orders = 1;
        while (ir.hasNext()) {
            ImgStoreDirDb lfch = (ImgStoreDirDb)ir.next();
            // 重置孩子节点的排列顺序
        ...
        if (lf.getCode().equals(lf.ROOTCODE)) {
            ...
            if (parentCode.equals(lf.ROOTCODE))
                ...
                while (!parentCode.equals(lf.ROOTCODE)) {
                    ...
                    ImgStoreDirDb parentLeaf = getImgStoreDirDb(parentCode);
    ...
    // 修复根结点为leaf的树
    public void repairTree(ImgStoreDirDb leaf) throws Exception {
        ...
        // 刷新缓存
        ImgStoreDirCache dc = new ImgStoreDirCache();
