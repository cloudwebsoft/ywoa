...
public class SoftwareFileDb extends ObjectDb {
    long id;
    String name;
    String diskName;
    String visualPath;

    public SoftwareFileDb() {
        init();
    }

    public SoftwareFileDb(long id) {
        init();
        this.id = id;
    ...
    public void initDB() {
        this.tableName = "cws_cms_software_file";
        primaryKey = new PrimaryKey("id", PrimaryKey.TYPE_LONG);
        objectCache = new SoftwareFileCache(this);

        QUERY_CREATE = "insert into cws_cms_software_file (id, dir_code, name, disk_name, visual_path, file_size, ext, upload_date) values (?, ?,?,?,?,?,?,?)";
        QUERY_LIST = "select id from cws_cms_software_file order by upload_date desc";
        QUERY_DEL = "delete from cws_cms_software_file where id=?";

        QUERY_SAVE = "update cws_cms_software_file set name=? WHERE id=?";
        QUERY_LOAD = "SELECT dir_code, name, disk_name, visual_path, file_size, upload_date FROM cws_cms_software_file WHERE id=?";
        isInitFromConfigDB = false;
    }

    public IObjectDb getObjectRaw(PrimaryKey pk) {
        return new SoftwareFileDb(pk.getLongValue());
    }

    public SoftwareFileDb getSoftwareFileDb(long id) {
        return (SoftwareFileDb)getObjectDb(new Long(id));
    }

    public boolean create(JdbcTemplate jt) throws ErrMsgException,
            ResKeyException {
        id = SequenceMgr.nextID(SequenceMgr.CMS_SOFTWARE_FILE);
        try {
