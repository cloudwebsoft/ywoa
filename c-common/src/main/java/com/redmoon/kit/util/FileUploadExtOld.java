...
public class FileUploadExtOld extends FileUpload {
    ...
    public int doUpload(ServletContext application, HttpServletRequest request, String charset) throws
        ...
        if (tmpPath==null) {
            tmpPath = realPath + "FileUploadTmp/";
        ...
        while (i != -1) {
            ...
            if (newLine.startsWith("Content-Disposition:")) {
                if (newLine.indexOf("filename=\"") != -1) {
                    ...
                    FileUploadStatus status = (FileUploadStatus)session.getAttribute(UPLOADSTATUS);
                    if (status==null) {
                    ...
                    while (i != -1 && !newLine.startsWith(boundary)) {
                        ...
                        // 置上传状态
                        status.get(uploadSerialNo).setBytesRead(allFileSize);
                    ...
                    status.get(uploadSerialNo).setFinish(true);
