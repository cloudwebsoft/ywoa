...
public class ValidateCodeCreator {
    ...
    // 备选汉字的长度
    public static int cnlen = base.length();
    // 去除了0 o r i t f l等不易识别的字符
    public static String[] defaultChars = {"1", "2", "3", "4", "5", "6", "7", "8",
                                   "9", "$",
                                   "#", "a", "b", "c", "d", "e",
                                   "g",
                                   "h", "j", "k", "m", "n", "p",
    ...
    public static void create(HttpServletRequest request,
                                           HttpServletResponse response, int width, int height, int charNum, String[] chars) throws IOException {
        if (chars==null)
            chars = defaultChars;
        ...
        // int width = 60, height = 20, charNum = 4;
        ...
        // g.setFont(new Font(fontTypes[random.nextInt(fontTypesLength)],Font.BOLD, 15)); // 12 + random.nextInt(6)));

        String sRand = "";
        AffineTransform fontAT = new AffineTransform();
        fontAT.rotate(Math.toRadians(0));
        Font ft = new Font("TimesRoman", Font.PLAIN, 18);

        int len = chars.length;

        for (int i = 0; i < charNum; i++) {
            // int start = random.nextInt(cnlen);
            // String rand  = base.substring(start,start+1);    // 中文验证码
            String rand = chars[random.nextInt(len)]; // String.valueOf(random.nextInt(10));
            sRand += rand;
            ...
            g.drawString(rand, 13 * i + 6, 12 + random.nextInt(6));
        }
     ...
     * @return boolean
     */
    public static boolean validate(HttpServletRequest request) throws ErrMsgException {
        // 检测验证码
        ...
        if (!validateCode.equals(sessionCode))
            throw new ErrMsgException("验证码错误!");
        else
            return true;
    }
}
