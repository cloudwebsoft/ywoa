 ...
 * <p>Title: </p>
 *
 * <p>Description: 吸收自Jive的思想，在此加以扩展，用于从缓存获取对象列表</p>
 *
...
public class ObjectBlockIterator implements Iterator {
    ...
    private Object getNextElement() {
        ...
        if (endIndex != Integer.MAX_VALUE) {
            // 如果为max_value还用下句则有可能会导致CPU 100%
    ...
    private Object getElement(int index) {
        ...
        if (index < blockStart || index >= blockStart + objectDb.getBlockSize()) {
            ...
            this.infoBlock = objectDb.getObjectBlock(query, index);
    ...
    public void setIndex(ObjectDb obj) {
        ...
        for (int i = startIndex; i < endIndex; i++) {
            currentBlock = obj.getObjectBlock(query, i);
    ...
    public boolean comparePrimarKey(PrimaryKey primaryKey, Object objKey) {
        ...
        if (primaryKey.getKeyCount() == 1) {
            ...
            while (ir.hasNext()) {
                ...
                if (ku2 == null) {
                    throw new IllegalArgumentException("The key " + keyName + " is not exists in Primary");
                }
