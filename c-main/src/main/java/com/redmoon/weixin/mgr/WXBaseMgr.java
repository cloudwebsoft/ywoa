		...
		try {
			...
			if(result!=null && !result.equals("")){
				...
				if (json!=null && json.has("errmsg")) {
					if (errorCode!=0) {
						LogUtil.getLog(WXBaseMgr.class).error(result);
						LogUtil.getLog(WXBaseMgr.class).error("url=" + url + " body=" + body);
					}
				}
			}
...
/*	*//**
	...
	public  String getUserParam(UserDb userDb) {
		...
		try {
			...
			postUserMgr.setUserName(userDb.getName());
	...
	public  String getDeptParam(Department department) {
		...
		try {
			...
			deptObj.put("order", 1000 - department.getOrders()); // 微信文档称按从小到大排序，实测却是从大到小排序
		} catch (JSONException e) {
			e.printStackTrace();
		}
