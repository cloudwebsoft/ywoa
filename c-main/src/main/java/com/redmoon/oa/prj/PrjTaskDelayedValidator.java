 ...
 * @Description: 
 * @author: 
 * @Date: 2016-1-24下午04:04:00
 */
public class PrjTaskDelayedValidator implements IFormValidator {

	Logger logger = Logger.getLogger(PrjTaskRemovedValidator.class.getName());

	public PrjTaskDelayedValidator() {
	}
	...
	public void onWorkflowFinished(WorkflowDb wf, WorkflowActionDb lastAction)
		...
		String taskId = fdao.getFieldValue("task_id");
		Date dt = DateUtil.parse(fdao.getFieldValue("delayed_time"), "yyyy-MM-dd HH:mm:ss");
		
		String sql = "update form_table_prj_task set task_endtime=? where id="+taskId;
		JdbcTemplate jt = new JdbcTemplate();
		try {
			jt.executeUpdate(sql, new Object[]{dt});
			
			sql = "update form_table_prj_taskcontrol set pre_time=? where task_id=" + taskId;
			jt.executeUpdate(sql, new Object[]{dt});
