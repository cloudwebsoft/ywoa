...
public class WorkflowAttendedDesktopUnit implements IDesktopUnit {
	...
	public String display(HttpServletRequest request, UserDesktopSetupDb uds) {
		...
		String sql = "select distinct m.flow_id from flow_my_action m, flow f where m.flow_id=f.id and (m.user_name=" + StrUtil.sqlstr(privilege.getUser(request));
		if (isFlowProxy) {
			sql += " or m.proxy=" + StrUtil.sqlstr(privilege.getUser(request));
		}
		sql += ") and f.status<>" + WorkflowDb.STATUS_NONE
