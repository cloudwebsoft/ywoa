 ...
 * @version 1.0
 */
public class CountryCtl extends AbstractMacroCtl {
    public CountryCtl() {
    }
    ...
    public String convertToHTMLCtl(HttpServletRequest request, FormField ff) {
        ...
        if (ff.getValue() != null && !"".equals(ff.getValue())) {
            try {
                ...
                sql = "select region_id,region_name from oa_china_region where region_type=3 and parent_id=" + parentId
                        + " order by region_id";
    ...
    public String getValueByName(FormField ff, String name) {
        ...
        try {
            String sql = "select region_id from oa_china_region where region_type=3 and region_name='" + name + "'";
            JdbcTemplate jt = new JdbcTemplate();
            ResultIterator ri = jt.executeQuery(sql);
            if (ri.hasNext()) {
                ResultRecord rr = (ResultRecord) ri.next();
    ...
    public String getSqlForQuery(HttpServletRequest request, FormField ff, String value, boolean isBlur) {
        if (isBlur) {
            return "select f." + ff.getName() + " from ft_" + ff.getFormCode() + " f, oa_china_region r where f.city=r.region_id and r.region_type=3 and r.region_name like " +
                    StrUtil.sqlstr("%" + value + "%");
        }
        else {
            return "select f." + ff.getName() + " from ft_" + ff.getFormCode() + " f, oa_china_region r where f.city=r.region_id and r.region_type=3 and r.region_name=" +
                    StrUtil.sqlstr(value);
        }
    }
