    ...
    public String convertToHTMLCtl(HttpServletRequest request, FormField ff) {
        ...
        if (val==null) {
	        ...
	        if (docId==-1) {
	            ...
	            if (docId==-1) {
                    docId = StrUtil.toInt(ff.getDescription(), -1);
                    if (docId==-1) {
                        return "未设置ID";
                    }
                }
        ...
        if (doc.getType()==Document.TYPE_FILE) {
            // doc_getfile.jsp?pageNum=1&id=" + doc_id + "&attachId=" + file_id + "&op=download
            int attId = -1;
            ...
            str = "<a href='javascript:;' onclick=\"openWin('" + request.getContextPath() + "/fileark/download.do?pageNum=1&id=" + docId + "&attachId=" + attId + "&op=download')\">" + doc.getTitle() + "</a>";
        }
        else {
            str = "<a href='javascript:;' onclick=\"addTab('" + doc.getTitle() + "', '" + request.getContextPath() + "/doc_show.jsp?id=" + doc.getId() + "')\">" + doc.getTitle() + "</a>";
        }
