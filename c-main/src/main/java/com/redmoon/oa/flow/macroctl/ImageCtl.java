...
public class ImageCtl extends AbstractMacroCtl {
    ...
    public String convertToHTMLCtl(HttpServletRequest request, FormField ff) {
        ...
        str += "<input id='" + ff.getName() + "' name='" + ff.getName() + "' class='image-ctl' type='file' accept='image/gif,image/jpeg,image/jpg,image/png,image/bmp,image/svg' size=15>";
    ...
    public String converToHtml(HttpServletRequest request, FormField ff, String fieldValue) {
        ...
        if (!StrUtil.getNullStr(fieldValue).equals("")) {
        	if (fieldValue!=null && !fieldValue.equals("")) {
        	    ...
        	    if (pageType.indexOf(ConstUtil.PAGE_TYPE_WORD)!=-1) {
                    String defaultStr = ff.getDefaultValue().replaceAll("，", ",");
                    String[] ary = StrUtil.split(defaultStr, ",");
                    if (ary!=null) {
                        if (ary.length==2) {
                            str += "<img src='" + Global.getFullRootPath(request) +"/img_show.jsp?visitKey=" + visitKey + "&path="+ StrUtil.UrlEncode(fieldValue) +
                        ...
                        str += "<img src='" + Global.getFullRootPath(request) +"/img_show.jsp?visitKey=" + visitKey + "&path="+ StrUtil.UrlEncode(fieldValue) + "'><BR>";
                    }
                }
        	    else {
                    // 在流程中发邮件取摘要信息时，传入的request为null
                    str += "<a title='点击在新窗口中打开' href='" + Global.getRootPath() + "/img_show.jsp?visitKey=" + visitKey + "&path=" + StrUtil.UrlEncode(fieldValue) + "' target='_blank'>查看图片</a>";
                }
        	}
        }
        return str;
