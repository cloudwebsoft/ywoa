...
public class DocPriv extends ObjectDbA {
    ...
    public Vector<DeptDb> getDeptsOfDocPriv(int docId) {
        ResultSet rs = null;
        Conn conn = new Conn(connname);
        Vector result = new Vector();
        PreparedStatement ps = null;
    ...
    public boolean canUserDo(UserDb user, UserGroupDb[] groups, RoleDb[] roles, int docId, int privType) {
        ...
        // 如果是作者自己，则拥有对文档的全部权限
        if (doc.getAuthor().equals(user.getName())) {
            return true;
        }
    ...
    public boolean setDepts(int docId, String deptCodes) throws ErrMsgException {
     ...
     * @return Vector
     */
    public Vector<DocPriv> list(String sql) {
        ResultSet rs = null;
        Conn conn = new Conn(connname);
        Vector<DocPriv> result = new Vector<DocPriv>();
        try {
    ...
    public Vector<DocPriv> listByDocId(int docId) {
        String sql = "select id from doc_priv" + " where doc_id="+ docId+" order by name asc";
        return list(sql);
    }
}
