...
public class TreeSelectView {
	...
	void ShowLeaf(JspWriter out, TreeSelectDb leaf, boolean isLastChild)
		...
		if (leaf.getCode().equals(rootLeaf.getCode())) { // 如果是根结点
			out.println("<img onClick=\"ShowChild(this, '"
							+ tableid
		...
		if (!leaf.getParentCode().equals("-1")) {
			out.print("<a target=dirbottomFrame href='basic_tree_select_bottom.jsp?parent_code="
							+ StrUtil.UrlEncode(code, "utf-8")
							+ "&parent_name="
							+ StrUtil.UrlEncode(name, "utf-8")
							+ "&op=AddChild'>添子项</a>&nbsp;");
			out.print("<a target='dirbottomFrame' href='basic_tree_select_bottom.jsp?op=modify&code="
							+ StrUtil.UrlEncode(code, "utf-8")
							+ "&name="
							+ StrUtil.UrlEncode(name, "utf-8")
							+ "&description="
							...
							+ "'>修改</a>&nbsp;");
			out.print("<a target=_self href=# onClick=\"if (window.confirm('您确定要删除"
							+ name
	...
	void ListTreeFuncWithCheckbox(JspWriter out, TreeSelectDb leaf,
		...
		int i = 0;
		if (size > 0) {
			out.print("<table id='childoftable"
							+ leaf.getCode()
							+ "' cellspacing=0 cellpadding=0 width='100%' align=center><tr><td>");
		}
		Iterator ri = children.iterator();
