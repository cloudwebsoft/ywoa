	...
	public static String filterViewEditTagUrl(HttpServletRequest request, String moduleCode, String tagName) {
		...
		if (tagUrl.startsWith("{")) {
			try {
				...
				LogUtil.getLog(ModuleBackUtil.class).error(e);
			}
		}
		else {
						...
						if (name.equals(WorkflowPredefineDb.COMB_COND_TYPE_FIELD)) {
							...
							if (ff.getFieldType()==FormField.FIELD_TYPE_VARCHAR || ff.getFieldType()==FormField.FIELD_TYPE_TEXT) {
								...
								try {
									...
									LogUtil.getLog(ModuleBackUtil.class).error(ex);
								}
							}
		...
		try {
			...
			while (b!=-1) {
				...
				try {
					...
					LogUtil.getLog(ModuleBackUtil.class).error(e1);
					continue;
				}

				LogUtil.getLog(ModuleBackUtil.class).info("tableName=" + tableName);
				...
				LogUtil.getLog(ModuleBackUtil.class).info("cols=" + cols);
	...
	public static void parse(String formCode, String content) throws ErrMsgException {
		...
		try {
			...
			while (b!=-1) {
				...
				LogUtil.getLog(ModuleBackUtil.class).info("sql=" + insertSql);
		...
		try {
			...
			while (ir2.hasNext()) {
				...
				if (!"".equals(condition)) {
					...
					FormField ff = fd.getFormField(fieldName);
					if (ff == null) {
						DebugUtil.e(ModuleUtil.class, "doGetViewJS", "表单：" + fd.getName() + "(" + fd.getCode() + ") 中的字段：" + fieldName + " 不存在");
						continue;
					}
					// 如果是字符串型，则需加上双引号
					if (ff.getFieldType() == FormField.FIELD_TYPE_VARCHAR || ff.getFieldType() == FormField.FIELD_TYPE_TEXT) {
	...
	public static boolean doCheckSetup(HttpServletRequest request, String userName, IFormDAO fdao, FileUpload fu) throws ErrMsgException {
		...
		try {
			...
			while (ir2.hasNext()) {
				...
				boolean re = evalCheckSetupRule(request, userName, aryIf, fdao, filedList, fu);
				if (re) {
			...
			if (isCheckboxGroup) {
				...
				while (m.find()) {
					...
					if (fieldName.startsWith("request.")) {
						...
						// 替换掉换行，否则会报错：EvaluatorException: unterminated string literal
