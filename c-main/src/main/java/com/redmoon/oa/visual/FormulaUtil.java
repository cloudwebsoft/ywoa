...
import com.redmoon.oa.shell.Console;
...
public class FormulaUtil {
			...
			do {
		        ...
		        while (m.find()) {
		        	...
		        	if (args==null && params==null) {
						...
						// 因此，如果argsLen=0，并且paramsLen=1，则自动补第1个参数为空字符串，但这样仍会有问题，函数pos中是sql语句：select pos from form_table_personbasic where id={$personId}
	...
	public static String render(JdbcTemplate jt, String content, String[] params, String[] args, String kind, int fieldType) throws ErrMsgException {
		...
		// 检查是否含有函数，如：select pv from form_table_salary_set where position=’#rank({$userName})’
        ...
        while (m.find()) {            	            
        	...
        	if (args!=null && params!=null) {
        		...
        		if (args.length < params.length) {  
        			throw new ErrMsgException("函数：" + formulaCode + " 实参：" + strArgs + " 的数量小于形参：" + fma.params + " 的数量");
        		}        		
        	}
