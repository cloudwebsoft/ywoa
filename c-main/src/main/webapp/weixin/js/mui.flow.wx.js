(function($,window,document,undefined){
	...
	var _FLOW_INIT_AJAX_URL = "public/android/flow/init?";// 流程处理详情界面，用于 5+ App
	...
	$.Flow = $.Class.extend({
		...
		flowAttendDetail:function(){
			...
			if (mui.os.plus && mui.os.ios) {
				...
				url = this.getContextPath() + "/" + _FATTEND_DETAIL_AJAX_URL;
			}
		...
		flowDisposeInit:function(){  // 待办流程初始化接口 - 发起流程初始化接口
			...
			if (mui.os.plus && mui.os.ios) {
				// 将FLOW_INIT_AJAX_URL路径改为完整的路径，否则ios中5+app会因为spring security不允许url中包括../而致无法访问
				url = this.getContextPath() + "/" + _FLOW_DISPOSE_AJAX_URL;
			}
			var skey = self.options.skey;
			...
			var datas ;
			 if(myActionId == 0){
				 ...
				 if(mui.os.plus && mui.os.ios) {
					// 将FLOW_INIT_AJAX_URL路径改为完整的路径，否则ios中5+app会因为spring security不允许url中包括../而致无法访问
					url = this.getContextPath() + "/" + _FLOW_INIT_AJAX_URL;
				 }
			 } else {
			 ...
			 $.post(url,datas,function(data){
				...
				if(res == '0'){
					...
					if(isLight){
						...
						if(fields.length>0){
							Form.initForms(actionId, flowId, fields, formCode); // 初始化Form表单
						}
		...
		flowBackServer:function(){
			...
			if (mui.os.plus && mui.os.ios) {
				// 将FLOW_INIT_AJAX_URL路径改为完整的路径，否则ios中5+app会因为spring security不允许url中包括../而致无法访问
				url = this.getContextPath() + "/" + _FLOW_RETURN_AJAX_URL;
			}
			jQuery.ajax(url, {
		...
		flowSendServerPost:function() {
			...
			if (mui.os.plus && mui.os.ios) {
				// 将FLOW_INIT_AJAX_URL路径改为完整的路径，否则ios中5+app会因为spring security不允许url中包括../而致无法访问
				ajax_url = this.getContextPath() + "/" + _PRESET_FLOW_DISPOSE_AJAX_URL;
			}
			...
			if (isLight == 'true') {
				...
				if (mui.os.plus && mui.os.ios) {
					// 将FLOW_INIT_AJAX_URL路径改为完整的路径，否则ios中5+app会因为spring security不允许url中包括../而致无法访问
					ajax_url = this.getContextPath() + "/" + _FREE_FLOW_DISPOSE_AJAX_URL;
				}
		...
		getUsersByDepts:function(deptCode){
			...
			if (mui.os.plus && mui.os.ios) {
				// 将FLOW_INIT_AJAX_URL路径改为完整的路径，否则ios中5+app会因为spring security不允许url中包括../而致无法访问
				url = this.getContextPath() + "/" + _FLOW_MULTI_DEPTL;
			}
			jQuery.ajax(url, {
				dataType:'json',// 服务器返回json格式数据
				type:'post',// HTTP请求类型
