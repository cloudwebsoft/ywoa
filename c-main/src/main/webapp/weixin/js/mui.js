...
(function($) {
	$.addInit({
		...
		handle: function() {
			...
			// console.log('addInit pullrefresh', 'pullRefreshOptions:', pullRefreshOptions);
							...
							if(hasPulldown && pullRefreshOptions.down.style == "circle") { //原生转圈
								...
								$.plusReady(function() {
									...
									if ($.fn.pullRefresh_native != undefined) {
										...
										console.log('$.fn.pullRefresh_native为undefined，请注意页面script中起始处不能出现mui.init，否则此处可能因pullRefresh_native尚未被初始化，所以会为undefined');
									}
...
(function($, window, document) {
	...
	var createConfirm = function(message, title, btnArray, callback, type) {
		...
		// popup窗口选人的时候，该窗口好像不被认为是在5+app 环境中，调用mui.confirm的时候，报：plus is not defined
		// 而且$.plusReady(function() {} 运行不进去，故将改为如下方式，直接调用window.confirm
		if (!window.plus) {
			// H5版本，0为确认，1为取消
			if (window.confirm(message)) {
				callback({
