<%@ page contentType="text/html; charset=utf-8" language="java" import="java.sql.*" errorPage="" %><%@page import="cn.js.fan.util.*"%><%@page import="cn.js.fan.web.*"%><%@page import="com.redmoon.oa.*"%><%@page import="com.redmoon.oa.netdisk.*"%><%@page import="java.util.*"%><%@page import="java.io.*"%><%@page import="com.redmoon.oa.dept.*"%><%@page import="java.net.*"%>
<%@page import="javax.xml.crypto.Data"%><jsp:useBean id="fchar" scope="page" class="cn.js.fan.util.StrUtil"/><jsp:useBean id="fsecurity" scope="page" class="cn.js.fan.security.SecurityUtil"/><jsp:useBean id="privilege" scope="page" class="com.redmoon.oa.pvg.Privilege"/><%
  //你可以使用你服务器上的文件及其路径
...
String current_sort = ParamUtil.get(request, "cur");

//if(!current_sotr.equals("")){
	 java.util.Date current_time = att.getUploadDate();
	 String date_d = DateUtil.format(current_time , "yyyy-MM-dd HH:mm:ss");
