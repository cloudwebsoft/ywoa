...
<div class="mywork-list-wrap">
    ...
    <div class="mywork-list-time-week">
        <ul>
            ...
            <li style="list-style:none;" class="mywork-list-time-span2">第${dateCond}周（${dateArea}）</li>
            <li class="mywork-list-time-span3" id="after" onclick="after()"></li>
            <li class="mywork-list-time-span4" onclick="returnToday()">返回本周</li>
            <%} %>
        </ul>
        <form id="queryFrm" name="queryFrm" action="<%=request.getContextPath()%>/mywork/queryMyWeekWork" method="post">
            ...
            <input type="hidden" name="dateArea" id="dateArea" value="${dateArea}"/>
    ...
    <!--发布日报-->
    <c:forEach items="${list}" var="wl" varStatus="statu">
        <div class="mywork-list-master">
            <div id="mywork_${wl.id}">
                <div id="datearea_${wl.id}" class="mywork-list-master-p1">${wl.showTitle}</div>
...
<input type="hidden" value="${isLast}" id="isLast"/>
...
<script type="text/javascript">
                  ...
                  if (data.list[i].workLogAttachs != null && data.list[i].workLogAttachs.length > 0) {
                      ...
                      for (var j = 0; j < data.list[i].workLogAttachs.length; j++) {
                          ...
                          inhtml = inhtml + '<div id="attach_' + logId + '">'
                              ...
                              + '<a href="javascript:void(0);" id="del_' + waId + '" onclick="delAttach(' + waId + ' ,' + waId + ')" name="attDel_' + logId + '" style="display:none">  <img src="<%=request.getContextPath()%>/images/del.png" width="16" height="16"/></a>'
                              + '</div> ';
  ...
  %>
      ...
      var flag = true;
      var dateArea = $("#datearea_" + id).html();
          ...
          if (upfileCount == 1) {//无附件，ajax提交
              ...
              $.ajax({
                  ...
                  complete: function (XMLHttpRequest, status) {
                      $("#dayBody").hideLoading();
                      // if("<%=code%>"!=""){
                      // window.location.href = "<%=request.getContextPath()%>/mywork/myDayWork_do.jsp?code=<%=code%>&prjId="+prjId+"&workLog="+work_log;
                      // }
                  },
                  error: function (XMLHttpRequest, textStatus) {
                      // 请求出错处理
  ...
  function returnToday() {
      window.location.href = "queryMyWeekWork?logType=${logType}";
  }
