...
if (msdTop == null) {
	out.println(cn.js.fan.web.SkinUtil.makeErrMsg(request, "模块：" + codeTop + "不存在"));
	return;
}

String moduleNameTop = msdTop.getString("name");
...
if (parentIdTop==-1) {
	...
	if (msdTop.getInt("view_list")==ModuleSetupDb.VIEW_LIST_GANTT || msdTop.getInt("view_list")==ModuleSetupDb.VIEW_LIST_GANTT_LIST) {
		...
		%>
	  <li id="menu101">
		  <a href="<%=moduleUrlList%>"><span><%=msdTop.getString("name")%>日历看板</span></a>
	  </li>
	  <%
	}
	else {
		...
		%>
	    <li id="menu1">    
	    <a title="<%=moduleNameTop%>" href="<%=moduleUrlList%>"><span><%=moduleNameTop%></span></a>
	    </li>			
		<%
	}
		...
		if (mpdTop.canUserLog(pvgTop.getUser(request)) || mpdTop.canUserManage(pvgTop.getUser(request))) {
			...
			if (fdTop.isLog()) {
				...
				<%
					...
					if (isModuleLogRead) {
				%>
	  			<li id="menu5"><a href="javascript:;" onclick="addTab('浏览日志', '<%=request.getContextPath()%>/visual/module_list.jsp?op=search&code=module_log_read&read_type=<%=FormDAOLog.READ_TYPE_MODULE%>&module_code=<%=codeTop%>&form_code=<%=formCodeTop%>')"><span>浏览日志</span></a></li>
	  			<%
					}
			}
		}
	...
	if (msdTop.getInt("view_show")==ModuleSetupDb.VIEW_SHOW_CUSTOM) {	
		...
		%>
	  		<li id="menu1"><a title="<%=moduleNameTop%>" href="<%=request.getContextPath() + servletPathTop%>/module_edit.jsp?id=<%=parentIdTop%>&parentId=<%=parentIdTop%>&code=<%=codeTop%>&formCode=<%=formCodeTop%>&<%=requestParamsTop %>"><span><%=msdTop.getString("name")%></span></a></li>
	  	<%
		}
		else {
		%>
			<li id="menu1"><a title="<%=moduleNameTop%>" href="<%=request.getContextPath() + servletPathTop%>/module_show.jsp?id=<%=parentIdTop%>&parentId=<%=parentIdTop%>&code=<%=codeTop%>&formCode=<%=formCodeTop%>&<%=requestParamsTop %>"><span><%=msdTop.getString("name")%></span></a></li>
		<%
		}
	}
}
	...
	while (irTop.hasNext()) {
		...
		if (mpdTop.canUserSee(curUserTop) && mrdTop.getInt("is_on_tab")==1) {	
			...
			msdRelate = msdRelate.getModuleSetupDb(mrdTop.getString("relate_code"));
	  		String moduleNameRelated = msdRelate.getString("name");
	%>
			<li id="menu<%=menuItemTop%>"><a title="<%=moduleNameRelated%>" href="<%=request.getContextPath()%>/visual/module_list_relate.jsp?parentPageType=<%=pageTypeTop%>&parentId=<%=parentIdTop%>&menuItem=<%=menuItemTop%>&code=<%=codeTop%>&moduleCodeRelated=<%=mrdTop.getString("relate_code")%>&formCode=<%=formCodeTop%>&<%=requestParamsTop %>"><span><%=msdRelate.getString("name")%></span></a></li>
	<%
		}
		menuItemTop++;
	}
