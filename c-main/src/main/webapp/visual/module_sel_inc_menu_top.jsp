...
<div id="tabs1">
  <ul>
    <li id="menu1" title="<%=msdTop.getString("name")%>"><a href="module_list_sel.jsp?formCode=<%=formCodeTop%>&byFieldName=<%=byFieldNameTop%>&showFieldName=<%=showFieldNameTop%>&filter=<%=StrUtil.UrlEncode(filterTop)%>&openerFormCode=<%=openerFormCodeTop%>&openerFieldName=<%=openerFieldNameTop%>"><span><%=msdTop.getString("name")%>列表</span></a></li>
