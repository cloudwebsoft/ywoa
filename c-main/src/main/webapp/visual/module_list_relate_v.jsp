...
<%
    ...
    if (!mpd.canUserView(privilege.getUser(request))) {
        ...
        if (!"".equals(visitKey)) {
            String fId = String.valueOf(parentId);
    ...
    // 应通过visitKey
    /*if (!ModulePrivMgr.canAccessData(request, msd, parentId)) {
    ...
    /*if (cfg.getBooleanProperty("isModuleLogRead") && !formCode.equals(ConstUtil.MODULE_CODE_LOG) && !formCode.equals(ConstUtil.MODULE_CODE_LOG_READ) && !formCode.equals(ConstUtil.FORM_FORMULA)) {
        ...
        moduleLogService.logRead(formCode, code, parentId, userName, privilege.getUserUnitCode(request));
    }*/
    ...
    com.redmoon.oa.visual.FormDAO fdao = fdm.getFormDAO(parentId);
%>
<!DOCTYPE html>
<html>
<head>
    ...
    <script src="../flow/form_js/form_js_${formCode}.jsp?pageType=${pageType}&id=${id}&moduleCode=${code}&time=<%=Math.random()%>"></script>
...
<div class="page-main">
    ...
    <%
        // 关联模块分组按orders分组
        double vOrders = ParamUtil.getDouble(request, "vOrders", 1);
        ArrayList<String> dateFieldNamelist = new ArrayList<String>();
        List<ModuleSetupDb> list = new ArrayList<>();
        ...
        if (!isSubTagRelated) {
            for (ModuleRelateDb moduleRelateDb : mrd.getModulesRelated(formCode)) {
                mrd = moduleRelateDb;
                if (mrd.getDouble("relate_order") != vOrders) {
                    continue;
                }
            ...
            String[] subTagOrders = StrUtil.split(StrUtil.getNullStr(msd.getString("sub_nav_tag_order")), "\\|");
            int subLen = 0;
            if (subTags != null) {
    ...
    %>
    <style>
        <%=StrUtil.getNullStr(msdRelated.getCss(ConstUtil.PAGE_TYPE_LIST))%>
    </style>
    <div>
        <div class="module-name"><%=msdRelated.getName()%></div>
        ...
        <script type="text/html" id="toolbar_<%=moduleCodeRelated%>">
            <div class="layui-btn-container">
                    ...
                    if (btnNames != null && btnBclasses != null) {
                        ...
                        for (int i = 0; i < len; i++) {
                            ...
                            if (!btnScripts[i].startsWith("{")) {
                                ...
                                if ("batchBtn".equals(json.get("btnType"))) {
                                    isToolBtn = true;
                                }
                            }
                            if (isToolBtn) {
                ...
                <button class="layui-btn layui-btn-sm layui-btn-primary"
                        lay-event="event_<%=moduleCodeRelated%><%=i%>"><%=btnNames[i]%>
                </button>
                <%
                            }
                        }
    ...
    <script>
        layui.use('table', function () {
            ...
            table.render({
                ...
                ,method: 'post'
                ,url: 'moduleListRelate.do?mode=<%=mode%>&code=<%=code%>&formCodeRelated=<%=formCodeRelated%>&formCode=<%=formCode%>&parentId=<%=parentId%>&op=search&moduleCodeRelated=<%=moduleCodeRelated%>&mode=&tagName=<%=StrUtil.UrlEncode(tagName)%>'
                ,cols: [[
                    {type: 'checkbox'},
                    <%=colProps.toString()%>
                ]]
