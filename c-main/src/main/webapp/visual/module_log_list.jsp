...
DebugUtil.i(getClass(), "list_field", msd.getString("list_field"));
...
for (int i=0; i<len; i++) {
	...
	if (fieldName.startsWith("main:")) {
		...
		if (otherFields.length == 5) {
			FormDb otherFormDb = new FormDb();
			otherFormDb = otherFormDb.getFormDb(otherFields[2]);
			if (!otherFormDb.isLoaded()) {
				title = "表单：" + otherFields[2] + "不存在";
			}
			else {
...
function action(com, grid) {
	if (com=="导出") {
		...
		*/
		...
		%>
			window.open('<%=request.getContextPath()%>/visual/exportExcel.do?' + $("form").serialize() + '&cols=' + cols);
		<%}%>
	}
	else if (com=="全部导出") {
		window.open('<%=request.getContextPath()%>/visual/exportExcel.do?isAll=true&' + $("form").serialize());
	}	
	else if (com=="全部导入") {
