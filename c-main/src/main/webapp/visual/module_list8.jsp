...
<%}%>
  ...
  <tr align="center" id="<%=id%>">
    ...
    <% WorkLogForModuleMgr wlfm = new WorkLogForModuleMgr();
    	if(is_workLog==1){ %>
    		&nbsp;&nbsp;<a href="javascript:;" onClick="addTab('<%=msd.getString("name")%>汇报', '<%=request.getContextPath()%>/mywork/queryMyWork?code=<%=mainFormCode%>&id=<%=id%>')">汇报</a>
    <%
    	}
    %>
...
function action(com, grid) {
	if (com=="导出") {
		...
		window.open('exportExcel.do?<%=querystr%>&cols=' + cols);
	}
	else if (com=="全部导出") {
		window.open('exportExcel.do?isAll=true&<%=querystr%>');
	}	
	else if (com=="全部导入") {
