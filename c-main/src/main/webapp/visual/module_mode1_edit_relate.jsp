...
if (op.equals("saveformvalue")) {
	...
	if (re) {
		...
		<%
		out.print(StrUtil.jAlert_Redirect("操作成功！","提示", request.getRequestURL() + "?parentId=" + parentId + "&id=" + id + "&menuItem=" + menuItem + "&formCodeRelated=" + formCodeRelated + "&formCode=" + StrUtil.UrlEncode(formCode) + "&isShowNav=" + isShowNav));
	}
	else
...
<%}
	...
	out.print("<BR>");
%>
<form action="module_mode1_edit_relate.jsp?op=saveformvalue&parentId=<%=parentId%>&id=<%=id%>&formCodeRelated=<%=formCodeRelated%>&formCode=<%=StrUtil.UrlEncode(formCode)%>&isShowNav=<%=isShowNav%>&menuItem=<%=menuItem %>" method="post" enctype="multipart/form-data" name="visualForm" id="visualForm">
