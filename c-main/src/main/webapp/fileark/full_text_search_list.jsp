...
<table width="92%" border="0" align="center" class="p9">
    <tr>
        <td height="24" align="right">找到符合条件的记录 <b><%=paginator.getTotal() %>
        </b> 条　每页显示 <b><%=paginator.getPageSize() %>
        </b> 条　页次 <b><%=paginator.getCurrentPage() %>/<%=paginator.getTotalPages() %>
        </b></td>
    </tr>
</table>
<table class="tabStyle_1 percent98" width="96%" border="0" align="center" cellpadding="1" cellspacing="1">
    ...
    <tr>
        <td width="54%" height=23 align="left" bgcolor="<%=bgcolor%>">&nbsp;
            <a href="javascript:;" onclick="addTab('<%=title%>', '<%=request.getContextPath()%>/doc_show.jsp?id=<%=id%>')"><%=title%>
            </a>
        </td>
        <td width="17%" align="center" bgcolor="<%=bgcolor%>"><%=ud.getRealName()%>
        </td>
        <td width="22%" align="center" bgcolor="<%=bgcolor%>"><%=modifiedDate%>
        </td>
        <td width="7%" align="center" bgcolor="<%=bgcolor%>"><%=hit%>
        </td>
    </tr>
    <%
            j++;
