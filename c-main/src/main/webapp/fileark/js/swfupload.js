...
function initSwfUpload(swf){
	var swfUpload = new SWFUpload({
		...
		button_text: '<span class="redText">上传</span>',
		button_text_left_padding : 8,  
		button_text_top_padding : 0, 
		button_text_style : '.redText{color:#606060;font-size:14px;font-family:"微软雅黑";}',  
		button_width: 50,
		button_height: 25,
...
function fileDialogComplete(numSelected,numQueued,numTotalInQueued) {
	...
	if (queueErrorArray && queueErrorArray.length) {
		...
		if(numSelected >= 1){
			...
			showDialog("info",'<ol id="logList">'+listItem+'</ol>',"文件上传",400,75,dirCode);
			this.startUpload();
		}
...
function uploadSuccess(file,serverData,response) {
	...
	if(numSelect == uploadCount){
		numSelect = 0;
		uploadCount = 0;
		window.location.href = "document_list_m.jsp?dir_code="+dirCode+"&userName="+userName+"&mode="+mode+"&cooperateId="+cooperateId;
	}
}
