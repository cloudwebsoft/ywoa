...
if (leaf==null || !leaf.isLoaded()) {
	out.print(SkinUtil.makeErrMsg(request, "节点不能为空！"));
	return;
}
if (op.equals("del")) {
...
while (ri.hasNext()) {
    ...
    <tr class="row" style="BACKGROUND-COLOR: #ffffff">
      <td style="PADDING-LEFT: 10px">      &nbsp;<img src="images/arrow.gif" align="absmiddle"><%=rr.getInt("id")%></td>
      <td style="PADDING-LEFT: 10px">&nbsp;<a href="../fwebedit.jsp?op=edit&id=<%=rr.getInt("id")%>&dir_code=<%=StrUtil.UrlEncode((String)rr.get(1))%>&dir_name=<%=StrUtil.UrlEncode(dir_name)%>"><%=(String)rr.get(2)%></a></td>
...
<HR noShade SIZE=1>
<%if (!dir_code.equals("") && leaf!=null && leaf.getType()==2) {%>
