...
String sql = "SELECT doc_id,count(doc_id) as countName FROM doc_attachment_log l where 1=1";
...
%>
	<table id="searchTable" width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			...
			<form action="doc_att_log_rank.jsp" method="get">
...
%>
        <tr >
		  <td><a href="javascript:;" onclick="addTab('查看日志', '<%=request.getContextPath()%>/fileark/doc_log.jsp?type=xiazai&id=<%=doc.getId()%>&title=<%=StrUtil.UrlEncode(doc.getTitle())%>')"><%=doc.getTitle()%></a></td> 
          <td><%=count%></td>
        </tr>
<%}%>
	</tbody>
...
function changeSort(sortname, sortorder) {
	window.location.href = "doc_att_log_rank.jsp?<%=querystr%>&pageSize=" + flex.getOptions().rp + "&orderBy=" + sortname + "&sort=" + sortorder;
}

function changePage(newp) {
	if (newp)
		window.location.href = "doc_att_log_rank.jsp?<%=querystr%>&CPages=" + newp + "&pageSize=" + flex.getOptions().rp;
}

function rpChange(pageSize) {
	window.location.href = "doc_att_log_rank.jsp?<%=querystr%>&CPages=<%=curpage%>&pageSize=" + pageSize;
}
