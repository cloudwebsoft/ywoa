...
<%
String selPriv = ParamUtil.get(request,"priv");
String[] selPriArr = null;
if (!"".equals(selPriv)){
	selPriArr = selPriv.split(",");
}
...
while (ri.hasNext()) {
	...
	boolean b = false;
	if (selPriArr!=null&&selPriArr.length>0){    //数据回显判断
		for (int i=0;i<selPriArr.length;i++){
			String str = selPriArr[i];
			if (str.equals(priv))
				b = true;
		}
	}
    ...
    <tr class="highlight">
      ...
      <%
      if (b){
      out.print("<input type=\"checkbox\" id=\"code\" name=\"code\" value=\""+priv+"\" desc=\""+desc+"\" checked />");
      } else {
      out.print("<input type=\"checkbox\" id=\"code\" name=\"code\" value=\""+priv+"\" desc=\""+desc+"\"  />"); 
      }
      %>
      </td>
      <td><%=priv%></td>
...
function getCheckboxValue(checkboxname){
	...
	if (checkboxboxs!=null)
	{
		...
		if (checkboxboxs.length==null) {
			if (checkboxboxs.checked) {
				return checkboxboxs.getAttribute("value");
			}
		}
		for (i=0; i<checkboxboxs.length; i++)
		{
			...
			{
				if (CheckboxValue==''){
					CheckboxValue += checkboxboxs[i].getAttribute("value");
				}
				else{
					CheckboxValue += ","+ checkboxboxs[i].getAttribute("value");
				}
