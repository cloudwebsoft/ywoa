...
<head>
    ...
    <script>
        ...
        function refresh() {
            ...
            %>
            refreshOnlineUser();
            timeoutid = window.setTimeout("refresh()", <%=refreshOnlineInterval%> * 1000
        )
            ; // 每隔N秒钟刷新一次
        }
...
<div id="left" class="v2main">
    <div class="menutab" id="tab">
        <ul>
            <li class="selected-1" onclick="changeLeft(0);" id="menuTab">
				<a href="#"><img id="menuIcon" src="<%=skinPath%>/images/selico-1.png"
                    width="15" height="18"/>
                菜单</a></li>
            <li id="online" onclick="changeLeft(1);">
				<a href="#"><img id="onlineIcon" src="<%=skinPath%>/images/norico-2.png" width="18"
                    height="17"/> 在线</a></li>
        </ul>
    </div>
    <div id="leftMenuMiddle" class="leftsub">
        <div id="menuMain1" class="menuMain1">
            ...
            <div class="boxbg" id="leftMenu<%=k%>" isOpen=0
                 onclick="storeMenu('<%=lf.getCode()%>', true);openPage(this, '<%=lf.getName(request)%>','<%=lf.isCanRepeat()%>','true','<%=lf.getLink(request)%>','<%=lf.getTarget()%>')"
                 onmouseover="setDivBackground('leftMenu<%=k%>')" onmouseout="setDivBackground('leftMenu<%=k%>')">
                <%} else {%>
                <div class="boxbg" id="leftMenu<%=k%>" isOpen=0
                     onclick="<%=v2.size()>0?"displayLeftSubMenu(" + k + ")":""%>"
                     onmouseover="setDivBackground('leftMenu<%=k%>')" onmouseout="setDivBackground('leftMenu<%=k%>')">
                    <%} %>
                    <span class="span-1"><img src="<%=iconPath%>/icons/<%=lf.getIcon()%>" width="30"
                                              height="30"/></span>

                    <%if (!"".equals(lf.getLink(request))) {%>
                    <span class="span-2"><%=lf.getName(request)%></span><span><img
                        src="<%=skinPath%>/images/arrownormal.png" width="11" height="11" id="chainImg<%=k %>"/></span>
                    <%} else {%>
                    <span class="span-2"><%=lf.getName(request)%></span><span><img
                        src="<%=skinPath%>/images/arrownormal.png" width="11" height="11" id="chainImg<%=k %>"/></span>
                    <%}%>
                ...
                <ul class="submenuul" id="leftSubMenu<%=k%>" style="display:none">
                    <%
                        ...
                        while (ir2.hasNext()) {
                            ...
                            if (v3.size() == 0) {
                    %>
                    <li id="menuItem<%=x%>" isSelected=0
                        onclick="selectMenuItem(<%=x%>);setBackGround('menuItem<%=x%>');"
                        onmouseover="setDivBackground('menuItem<%=x%>')"
                        onmouseout="setDivBackground('menuItem<%=x++%>')">
                        <div class="listbox"
                             onclick="storeMenu('<%=lf2.getCode()%>', true);openPage(this, '<%=lf2.getName(request)%>','<%=lf2.isCanRepeat()%>','true','<%=lf2.getLink(request)%>','<%=lf2.getTarget()%>')">
                            <span class="new_f-1">■</span><%=lf2.getName(request)%>
                        </div>
                    </li>
                    <%} else {%>
                    <li>
                        <div class="listbox" id="leftMenu<%=k%>" isSelected=0 onclick="displayLeftSubMenu(<%=k%>)"
                             onmouseover="setDivBackground('leftMenu<%=k%>')"
                             onmouseout="setDivBackground('leftMenu<%=k%>')"><img src="<%=skinPath%>/images/openico.jpg"
                                                                                  width="15" height="15"
                                                                                  class="chainImg" id="chainImg<%=k%>"/>&nbsp;<span><%=lf2.getName(request)%></span>
                        </div>
                        <ul id="leftSubMenu<%=k%>" style="display:none">
                            ...
                            %>
                            <li class="3level" id="menuItem<%=x%>"
                                onclick="selectMenuItem(<%=x%>);setBackGround('menuItem<%=x%>');" isSelected=0
                                onmouseover="setDivBackground('menuItem<%=x%>')"
                                onmouseout="setDivBackground('menuItem<%=x++%>')">
                                <div class="listbox1"
                                     onclick="storeMenu('<%=lf3.getCode()%>', true);openPage(this, '<%=lf3.getName(request)%>','<%=lf3.isCanRepeat()%>','true','<%=lf3.getLink(request)%>','<%=lf3.getTarget()%>')">
                                    <%=lf3.getName(request)%>
                                </div>
                            </li>
            ...
            <div id="menuMain3" style="display:none;overflow:auto;" class="onlineCss">
                <div id="menuOnline">
                    ...
                    <div>
                        &nbsp;<%=deptName%>&nbsp;<a
                            title="登录时间：<%=DateUtil.format(oud.getLogTime(), "yyyy-MM-dd HH:mm:ss")%>"
                            onClick="addTab('<%=user.getRealName()%>', 'user_info.jsp?userName=<%=StrUtil.UrlEncode(user.getName())%>')"
                            target="mainFrame"><%=user.getRealName()%>
                    </a>&nbsp;&nbsp;<a
                            href="message_oa/message_ext/send.jsp?receiver=<%=StrUtil.UrlEncode(oud.getName())%>"
                            target="mainFrame" style="display:none">消息</a>
                    </div>
                    <%
                        }
                    %>
