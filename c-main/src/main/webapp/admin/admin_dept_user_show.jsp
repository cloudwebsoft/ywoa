...
%>
          ...
          <div class="percent98 centerBox"><a title="工作报告" href="javascript:;" onclick="addTab('<%=user.getRealName() %>的工作报告', '<%=request.getContextPath()%>/mywork/showWorkLogInfo.action?userName=<%=StrUtil.UrlEncode(user.getName())%>')"><strong>工作报告</strong></a></div>
		<%
...
%>
            ...
            <td align="center"><%=wld.getLogType() == WorkLogDb.TYPE_NORMAL ? "日报" : (wld.getLogType() == WorkLogDb.TYPE_WEEK ? "周报" : "月报")%> </td>
              <td>
			  <a href="javascript:;" onclick="addTab('查看工作报告', '<%=Global.getFullRootPath(request) %>/mywork/showWorkLogById.do?workLogId=<%=wld.getId()%>')" title="查看"><%=wld.getContent()%></a> </td>
            </tr>
            <%}%>
          </tbody>
        </table>
