...
<%
    ...
    if ("field".equals(op)) {
        ...
        boolean re = false;
        try {
            re = wpm.saveWriteProp(request);
        } catch (ErrMsgException e) {
            LogUtil.getLog(getClass()).error(e);
            out.print(responseUtil.getResJson(false, e.getMessage()));
            return;
        }
        out.print(responseUtil.getResJson(re).toString());
        return;
    }
...
<form id="form1" name="form1" action="flow_designer_write_back.jsp" method="post">
    <div style="margin-bottom:5px; text-align:center">
        <input type="button" value="清除" class="btn" onClick="clearup()"/>
        <input type="button" value="外部" class="btn" style="margin-left:20px"
               onClick="window.location.href='flow_designer_write_back_db.jsp?flowTypeCode=<%=flowTypeCode%>&internalName=<%=internalName%>'"/>
        <input type="button" value="保存" id="btnOk" class="btn" style="margin-left:20px"/>
    </div>
    <fieldset style="padding-left:10px;">
        ...
        <br/>
        回写时机：
        <input style="margin-top:5px;" type="radio" id="timestamp1" name="timestamp"
               value="flowFinish" <%="".equals(internalName) ? "checked=\"true\"" : ""%> onClick="onFlowFinishClick()"/>流程结束
        <input style="margin-top:5px;" type="radio" id="timestamp2" name="timestamp"
               value="nodeFinish" <%=!"".equals(internalName) ? "checked=\"true\"" : ""%>
               onClick="onActionNodeClick()"/>核定结点通过<br/>
        ...
        <div style="background-color:#dbe1f3;margin-top:10px;height:30px;width:100%;">
            <img src="images/combination.png" style="margin-bottom:5px;margin-top:5px;"/>
            &nbsp;<a href="javascript:;" onClick="openCondition()">配置条件</a>&nbsp;
            <%if (hasCondition) { %>
            <img src="images/gou.png" style="margin-bottom:-5px;width:20px;height:20px;" id="imgId"/>
            <%} %>
        </div>
        <img style="margin-top:5px;cursor:pointer;" align="absMiddle" onClick="addField()"
             src="<%=SkinMgr.getSkinPath(request)%>/images/write-back-add.png"/>&nbsp;&nbsp;&nbsp;&nbsp;
        <img style="margin-top:5px;cursor:pointer;" align="absMiddle" onClick="delField()"
             src="<%=SkinMgr.getSkinPath(request)%>/images/write-back-close.png"/>&nbsp;&nbsp;&nbsp;&nbsp;
        <br/>
        <div id="repeat">
            <%
                int count = 1;
            ...
            %>
            <div>
                <span style="margin-top:5px;">设置字段值：</span>
                <select id="formfield<%=i%>" name="formfield<%=i%>" style="margin-top:5px;width:160px">
                    <%=opts %>
                </select>
                <textarea style="margin-top:5px;" rows="2" cols="30" readOnly="true" name="math<%=i%>" id="math<%=i%>"
                          onClick="openRelateWin(this, '<%=i%>')"></textarea>
            </div>
            <script>
            ...
            <div>
                ...
                <textarea style="margin-top:5px;" rows="2" cols="30" readonly="true" name="math0" id="math0"
                          onClick="openRelateWin(this, '0')"></textarea>
            </div>
            <%
                }
            %>
        ...
        $.ajax({
            ...
            success: function (data, status) {
                ...
                for (i = 0; i < fields.length; i++) {
                    ...
                    str += "<span id='formfield" + k + "' name='" + fields[i].name + "' type='" + fields[i].type + "' style='float:left; width:120px'>" + fields[i].title + "</span>";
                    var val = "";
                    // val = eval("insertFields." + fields[i].name);
                    // console.log(fields[i].name + " val=" + val);
                    if (insertFields.hasOwnProperty(fields[i].name)) {
                        val = insertFields[fields[i].name];
                    }
