...
<%
    ...
    String ieVersion = formViewDb.getString("ie_version");
    ...
    if ("save".equals(op)) {
        ...
        formViewDb.set("show_rule", xml);
        boolean re = formViewDb.save();
    ...
    Vector v = formParser.parseCtlFromView(content, ieVersion, fd);
    Iterator irV = v.iterator();
    while (irV.hasNext()) {
        FormField ff = (FormField) irV.next();
        opts.append("<option value='" + ff.getName() + "'>" + ff.getTitle() + "</option>");
    }*/
    JSONArray ary = FormParser.parseElementFromView(request, formViewDb, fd);
...
<head>
    ...
    <script language="JavaScript">
        ...
        function onload() {
            var viewSetup = '<%=StrUtil.getNullStr(formViewDb.getString("show_rule"))%>';
            if (viewSetup == "") {
                return;
            }
...
<table width="100%" align="center" cellpadding="2" cellspacing="0" class="tabStyle_1 percent80" id="tabCond" style="padding:0px; margin:0px; margin-top:3px; width:100%">
    <tr>
        <td height="22" colspan="5" align="center"><input name="okbtn" type="button" onclick="ModifyView()" value="保存"/>
            &nbsp;&nbsp;
            <input type="button" value="增加" onclick="addCond()"/>
        </td>
    </tr>
</table>
