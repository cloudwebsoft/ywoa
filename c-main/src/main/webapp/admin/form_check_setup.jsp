        ...
        function onload() {
            ...
            $xml.find("rules").children().each(function (i) {
                ...
                for (var index in jsonAry) {
                    arrFields.push('field' + valCount);

                    var json = jsonAry[index];
                    ...
                    if (index > 0) {
                        ...
                        str += '<select id="logical' + valCount + '"><option value="and" ' + (json.logical=='and'?'selected':'') + '>并且</option><option value="or" ' + (json.logical=='or'?'selected':'') + '>或者</option></select>';
                    }
...
<table width="100%" align="center" cellpadding="2" cellspacing="0" class="tabStyle_1 percent80" id="tabCond" style="padding:0px; margin:0px; margin-top:3px; width:100%">
    <tr>
        <td height="22" colspan="5" align="center"><input name="okbtn" type="button" class="btn btn-default" onclick="ModifyRules()" value="保存"/>
            &nbsp;&nbsp;
            <input type="button" value="增加" class="btn btn-default" onclick="addCond()"/></td>
    </tr>
</table>
<br/>
...
<script>
    ...
    function addThen(tdId) {
        ...
        // 删除除已生成的select2
        $(newSpanVal).find('.select2-container').remove();
        $(newSpanVal).find('select[name=field]').removeClass();
        ...
        $('#div' + tdId + valCount).prepend('<select id="logical' + valCount + '"><option value="and">并且</option><option value="or">或者</option></select>&nbsp;');
        
        // 重新生成select2
        $('#field' + valCount).select2();
    ...
    function makeRules() {
        ...
        $("tr[id^='rules']").each(function () {
            ...
            $("#" + trId + " select[name='field']").each(function (k) {
                ...
                var logical = $('#logical' + num).val();

                if (thens == "")
                    thens += "{\"field\":\"" + $(this).val() + "\", \"operator\":\"" + operator + "\", \"val\":\"" + val + "\", \"logical\":\"" + logical + "\"}";
                else
                    thens += ", {\"field\":\"" + $(this).val() + "\", \"operator\":\"" + operator + "\", \"val\":\"" + val + "\", \"logical\":\"" + logical + "\"}";
            });
            thens = "[" + thens + "]";
