...
function ShowChild(imgobj, name) {
	...
	if (tableobj==null) {
		o("ifrmGetChildren").contentWindow.location.href = "dept_ajax_getchildren.jsp?op=simple&parentCode=" + name + "&actionUrl=<%=request.getContextPath()%>/admin/admin_dept_user.jsp";
...
//tv.ListSimpleAjax(request, out, "midFrame", "admin_dept_user.jsp", "", "", true);
...
$(document).ready(function(){
	...
	myjsTree = $('#deptTree').jstree({
			            ...
			            if (eventNodeName == 'INS') {                   
		                   ...
		                   window.open("admin_dept_user.jsp?deptCode="+code,"midFrame");
			            }
						...
						myjsTree.jstree("select_node", initCode);
   						window.open("admin_dept_user.jsp?deptCode="+initCode,"midFrame");
   					});
   					for(var i=0;i<isHide.length;i++){
						$("#"+isHide[i]).hide();
					}
