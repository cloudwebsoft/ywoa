...
-->
<%if (!isAll.equals("y")) {%>
    &nbsp;&nbsp;<input name="button"  class="btn" type="button" onclick="javascript:location.href='flow_dir_priv_add.jsp?dirCode=<%=StrUtil.UrlEncode(leafPriv.getDirCode())%>&tabIdOpener=<%=tabIdOpener%>';" value="添加" />
<%}%>
</form>
    </td>
  </tr>
...
&nbsp;<input class="btn" type=button onclick="jConfirm('您确定要删除吗?','提示',function(r){ if(!r){return;}else{window.location.href='flow_dir_priv_m.jsp?op=del&isAll=<%=isAll%>&dirCode=<%=StrUtil.UrlEncode(curCode)%>&id=<%=lp.getId()%>&tabIdOpener=<%=tabIdOpener%>'}}) " style="cursor:pointer" value="删除"/></td>
    </tr></form>
<%}%>
  </tbody>
</table>
