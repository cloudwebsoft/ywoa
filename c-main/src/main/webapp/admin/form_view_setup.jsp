...
<%
    ...
    DebugUtil.i(getClass(), "view setup", fd.getViewSetup());
%>
<!DOCTYPE html>
<html>
<head>
    ...
    <script language="JavaScript">
        ...
        function onload() {
            // 会导致{"field":"gb", "operator":">=", "val":"\"a\""}中的\"丢失
            // var viewSetup = <%=fd.getViewSetup()%>;

            // textarea中取o('viewSetup').value会导致出现 > < 符号原来的转义字符被转换过来
            var viewSetup = o('viewSetup').value;
            console.log('viewSetup', viewSetup);
            if (viewSetup == "") {
                return;
            }
            viewSetup = viewSetup.replaceAll('<>', '&lt;&gt;');
            viewSetup = viewSetup.replaceAll('<=', '&lt;=');
            viewSetup = viewSetup.replaceAll('>>=', '>&gt;=');
...
<textarea id="viewSetup" style="display: none"><%=fd.getViewSetup()%></textarea>
<table width="100%" align="center" cellpadding="2" cellspacing="0" class="tabStyle_1 percent80" id="tabCond" style="margin:0 auto; padding:0px; margin-top:3px; width:100%">
    <tr>
        <td height="22" colspan="5" align="center"><input class="btn btn-default" id="okbtn" type="button"
                                                          onclick="ModifyView()" value="保存"/>
            &nbsp;&nbsp;
            <input class="btn btn-default" type="button" value="增加" onclick="addCond()"/></td>
    </tr>
</table>
<div id="mainFormOptDiv" style="display:none"><%=optionsVar %>
</div>
