...
UE.plugins['text'] = function () {
	...
	me.addListener( 'mouseover', function( t, evt ) {
		...
		var el = evt.target || evt.srcElement;
        var cwsPlugins = el.getAttribute('cwsplugins');
		if ( /input/ig.test( el.tagName ) && cwsPlugins==thePlugins) {
			var html = popup.formatHtml(
...
UE.plugins['macros'] = function () {
    ...
    me.commands[thePlugins] = {
        execCommand:function (f) {
            var dialog = new UE.ui.Dialog({
                iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/macros.jsp?formCode=<%=formCode%>',
                name:thePlugins,
                editor:this,
                title: '宏控件',
...
UE.plugins['radios'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        var el = evt.target || evt.srcElement;
        var cwsPlugins = el.getAttribute('cwsplugins');
        if ( /span/ig.test( el.tagName ) && cwsPlugins==thePlugins) {
            var html = popup.formatHtml(
...
UE.plugins['select'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        var el = evt.target || evt.srcElement;
        var cwsPlugins = el.getAttribute('cwsplugins');
        if ( /select|span/ig.test( el.tagName ) && cwsPlugins==thePlugins) {
            var html = popup.formatHtml(
...
UE.plugins['progressbar'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        var el = evt.target || evt.srcElement;
        var cwsPlugins = el.getAttribute('cwsplugins');
        if ( /img/ig.test( el.tagName ) && cwsPlugins==thePlugins) {
            var html = popup.formatHtml(
 ...
 * @example
 * ```javascript
 * editor.execCommand( 'calculation');
 * ```
 */
UE.plugins['calculation'] = function () {
	...
	me.commands[thePlugins] = {
		execCommand:function () {
			var dialog = new UE.ui.Dialog({
				iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/calculation.jsp?formCode=<%=formCode%>',
				name:thePlugins,
				editor:this,
				title: '计算控件',
				cssRules:"width:600px;height:450px;",
...
UE.registerUI('button_template',function(editor,uiName){
    ...
    editor.registerCommand(uiName,{
        execCommand:function(){
            try {
                cwsFormDesigner.exec('leipi_template');
                //cwsFormDesigner.fnCheckForm('save');
            } catch ( e ) {
                alert('打开模板异常');
            }
...
UE.registerUI('button_preview',function(editor,uiName){
    ...
    editor.registerCommand(uiName,{
        execCommand:function(){
            try {
                cwsFormDesigner.fnReview();
            } catch ( e ) {
                alert('cwsFormDesigner.fnReview 预览异常');
            }
        }
    });
    //创建一个button
    ...
    editor.registerCommand(uiName,{
        execCommand:function(){
            try {
                ...
                alert('cwsFormDesigner.fnCheckForm("save") 保存异常');
            }
