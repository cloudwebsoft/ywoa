...
<table class="percent98" width="98%" align="center">
  <tr><td align="center">
<select name="showyear" onChange="var y=this.options[this.selectedIndex].value; window.location.href='mywork_dept_month.jsp?deptCode=<%=StrUtil.UrlEncode(deptCode)%>&showyear=' + y;">
...
%>
    <tr>
      <td height="22" align="center"><%=user.getRealName()%></td>
      <%
	  ...
	  for (int i=1; i<=12; i++) {
		  ...
		  if (wld==null)
			wld = new WorkLogDb();
		  wld = wld.getWorkLogDb(user.getName(), WorkLogDb.TYPE_MONTH, showyear, i);
		  String cls = "";
		  if (wld==null) {
			cls = "class='none'";
			noCount ++;
	  ...
	  $("#td<%=row%>_<%=i%>").qtip({
		...
		content: {
			text: "加载中...",
			ajax: {
			  url: "<%=request.getContextPath()%>/mywork/mywork_tip.jsp?userName=<%=StrUtil.UrlEncode(user.getName())%>&logType=<%=WorkLogDb.TYPE_MONTH%>&year=<%=showyear%>&item=<%=i%>"
			},
			title: {
				text: '<%=i%>月',
				button: "关闭"
