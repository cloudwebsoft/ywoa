...
if (logType==WorkLogDb.TYPE_NORMAL) {
	...
	<%
		...
		%>
		<%=wld.getContent()%><a href="javascript:;" onclick="addTab('<%=user.getRealName()%> 日报', '<%=request.getContextPath()%>/mywork/showWorkLogById?workLogId=<%=wld.getId()%>')">查看详细</a>
		<%
	}
}
else {
	...
	<%
		...
		%>
			<%=wld.getContent()%><a href="javascript:;" onclick="addTab('<%=user.getRealName()%> 周报', '<%=request.getContextPath()%>/mywork/showWorkLogById?workLogId=<%=wld.getId()%>')">查看详细</a>
		<%
		}
		else {
		%>
			<%=wld.getContent()%><a href="javascript:;" onclick="addTab('<%=user.getRealName()%> 月报', '<%=request.getContextPath()%>/mywork/showWorkLogById?workLogId=<%=wld.getId()%>')">查看详细</a>
		<%
		}
	}
}
