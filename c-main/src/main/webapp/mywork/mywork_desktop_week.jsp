...
<%
    ...
    */
    ...
    String sql = "select id from work_log where userName=" + StrUtil.sqlstr(userName) + " and log_type=" + WorkLogDb.TYPE_WEEK + " order by myDate desc";
...
<div id="drag_<%=id%>" dragTitle="<%=udsd.getTitle()%>" count="<%=udsd.getCount()%>"
     wordCount="<%=udsd.getWordCount()%>" class="portlet drag_div bor">
    <div id="drag_<%=id%>_h" class="box ibox-title">
        ...
        %>
        <h5>
            <i class="fa <%=udsd.getIcon()%>"></i>&nbsp;&nbsp;<a href="<%=request.getContextPath()%>/mywork/queryMyWeekWork.action?logType=1"><%=udsd.getTitle()%></a>
        </h5>
        <%
        }
        else {
        ...
        <div class="titleimg">
            ...
            &nbsp;&nbsp;</div>
        <div class="titletxt">&nbsp;&nbsp;<a href="<%=request.getContextPath()%>/mywork/queryMyWeekWork.action?logType=1"><%=udsd.getTitle()%></a></div>
        <%
            }
        %>
    </div>
        ...
        <table class='article_table'>
            ...
            <tr>
                <td class='article_content'><a
                        href="<%=request.getContextPath()%>/mywork/showWorkLogById?workLogId=<%=wld.getId()%>"
                        title="<%=StrUtil.HtmlEncode(wld.getContent())%>"><%=StrUtil.getAbstract(request, wld.getContent(), udsd.getWordCount(), "，")%>
                </a></td>
                <td class='article_time' style="width:135px">[<%=wld.getLogItem()%>
                    周&nbsp;<%=DateUtil.format(wld.getMyDate(), "yyyy-MM-dd")%>]
                </td>
            </tr>
            <%}%>
        </table>
        <%} else {%>
        <div class='no_content'><img title='暂无工作周报' src='images/desktop/no_content.jpg'></div>
        <%
            }
        %>
