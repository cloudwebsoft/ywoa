	...
	$.fn.fixheadertable = function(options) {
		...
		return this.each(function() {
			...
			function restrictRows(table, nbrows) {
				...
				_nbpagesDiv.html('共' + length + '条  &nbsp;' + options.rowsPerPage + '/页&nbsp;  页码 ' + _currentpage + ' / ' + _nbpages);
			...
			function buildPager(table) {
				...
				_button_set = $('<div id="' + table.id + '_radio" style="float:left; display:none">' +
			...
			if(options.wrapper){
				...
				var tampon = _wrapper.wrap('<div class="ui-widget" style="padding : 0px; font-size : 1em;"></div>').parent();
