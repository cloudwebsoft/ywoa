        ...
        for (var z = 0; z < numOfRow; z++) {
            ...
            for (var y = 0; y < settings.columns.length; y++) {
                ...
                if (settings.columns[y].type == 'custom') {
                    ...
                    } else if (settings.columns[y].type == 'datepicker') {
                    ...
                    } else if (settings.columns[y].type == 'datetimepicker') {
                    	$('#' + ctrlId).datetimepicker({
                    		lang:'ch',
                			datepicker:true,
                			timepicker:true,
					...
					else if (settings.columns[y].type == 'userSelect') {
						/*
						$('#' + ctrlId).click(function() {
							openWinUserSelect(this);
						})
						*/
						// 将其改为两个输入框控件
						var name = $('#' + ctrlId).attr("name");
						var value = $('#' + ctrlId).val();
						$('#' + ctrlId).attr("name", name + "_realshow");
						$('#' + ctrlId).attr("id", name + "_realshow");
						$('#' + ctrlId + "_realshow").attr("readonly", "readonly");
						$('#' + ctrlId + "_realshow").parent().append("<input id='" + ctrlId + "' name='" + name + "' type='hidden' value='" + value + "' />");
						$('#' + ctrlId + "_realshow").click(function() {
							openWinUserSelect(o(name));
						})
                    }					
                }
            ...
            if (records != null && records.length) {
                ...
                for (var r = 0; r < insertResult.addedRows.length; r++) {
                    for (var c = 0; c < settings.columns.length; c++) {
                        ...
                        // 如果存在realshow辅助显示字段，则显示其值
                        var uniqueIndex = settings._rowOrder[r];
                        var columnName = settings.columns[c].name;
                        var objRealShow = document.getElementById(settings.idPrefix + '_' + columnName + "_" + uniqueIndex + '_realshow');
                    	// console.log(settings.idPrefix + '_' + columnName + "_" + uniqueIndex + '_realshow');
                        if (objRealShow!=null) {
                        	objRealShow.value = records[r][settings.columns[c].name + "_realshow"];
                        }
                    }
