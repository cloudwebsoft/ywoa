...
<%@ page import="com.redmoon.oa.pvg.OnlineUserDb"%>
<%@ page import="com.redmoon.oa.ui.Skin"%>
<%@ page import="com.redmoon.oa.ui.SkinMgr"%>
<%@ page import="com.redmoon.oa.ui.menu.Leaf"%>
<%@ page import="com.redmoon.oa.ui.menu.LeafChildrenCacheMgr"%>
...
<head>
    ...
    <script type="text/javascript">
        ...
        function exitOa() {
            ...
            window.top.location.href = "logout?skincode=<%=skincode %>";
        }
    </script>
...
<div class="onlineList" style="border:1px #aeaeae solid;width:125px;height:625px;overflow-y:auto;position:absolute;display:none;z-index:9999;background:#fff;" onblur="closeOnline()">
    ...
    <div style="margin:5px 0 5px 5px;">
        ...
        ：&nbsp;<a title="登录时间：<%=DateUtil.format(oud.getLogTime(), "yyyy-MM-dd HH:mm:ss")%>" onclick="addTab('<%=user.getRealName()%>', 'user_info.jsp?userName=<%=StrUtil.UrlEncode(user.getName())%>')" target="mainFrame" style="cursor:pointer"><%=user.getRealName()%>
    </a>&nbsp;&nbsp;<a href="message_oa/message_frame.jsp?op=send&receiver=<%=StrUtil.UrlEncode(oud.getName())%>" target="mainFrame" style="display:none">消息</a>
    </div>
    <%
        }
    %>
...
<div>
    ...
    <ul id="flexmenu1" class="flexdropdownmenu">
        ...
        %>
        <li>
            <a class="startMenuItem" href="javascript:void(0);" link="<%=lf.getLink(request)%>" hidefocus="true" canRepeat="<%=lf.isCanRepeat()%>" target="<%=lf.getTarget()%>"><%=lf.getName()%>
            </a>
            <%
                if (v2.size() > 0) {%>
            <ul>
                ...
                %>
                <li>
                    <a class="startMenuItem" href="javascript:void(0);" link="<%=lf2.getLink(request)%>" hidefocus="true" canRepeat="<%=lf2.isCanRepeat()%>" target="<%=lf2.getTarget()%>"><%=lf2.getName()%>
                    </a>
                    <%if (v3.size() > 0) {%>
                    <ul>
                        <%
                        ...
                        %>
                        <li>
                            <a class="startMenuItem" href="javascript:void(0);" link="<%=lf3.getLink(request)%>" hidefocus="true" canRepeat="<%=lf3.isCanRepeat()%>" target="<%=lf3.getTarget()%>"><%=lf3.getName()%>
                            </a>
                        </li>
                        <%}%>
                    </ul>
...
<div id="dialog" title="消息" style="display:none">
    ...
    <div align="center" style="margin-top:10px">
        <a href="javascript:IKnow()">我知道了</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick="$('#dialog').dialog('close')">&nbsp;关闭窗口
        <span style="display: none;">(<span id="sec"></span>)</span>
    </a>
    </div>
</div>
<!-- 快捷菜单 -->
...
<div id="submenu-wrap" class="submenu-wrap" style="display:none;" onmouseleave="removeAllMenusClass();">
    ...
    <div id="<%=lf.getCode()%>" style="display:none;">
        ...
        <div class='submenu-wrap-box' id="<%=lf1_1.getCode() %><%=flagNum %>">
            ...
            <div class='submenu-wrap-thirdmenu'>
                ...
                <div class='submenu-wrap-box-div'>
                    <a href='javascript:;' onclick="removeAllMenusClass();openPage(this, '<%=lf1_2.getName(request)%>','<%=lf1_2.isCanRepeat() %>','true','<%=lf1_2.getLink(request)%>','<%=lf1_2.getTarget() %>')"><%=lf1_2.getName(request)%>
                    </a>
                </div>
                <%
                        menuNum++;
        ...
        <div class='submenu-wrap-box-title'>
            <div class='submenu-wrap-box-div'>
                <a href='javascript:;' onclick="removeAllMenusClass();openPage(this, '<%=lf1_1.getName(request)%>','<%=lf1_1.isCanRepeat() %>','true','<%=lf1_1.getLink(request)%>','<%=lf1_1.getTarget() %>')"><%=lf1_1.getName(request)%>
                </a>
            </div>
        </div>
    </div>
...
<div id="allOthers" style="display:none;">
    ...
    <div class='submenu-wrap-box'>
        <div class='submenu-wrap-box-title'>
            <div class='submenu-wrap-box-div'>
                <a href='javascript:;' onclick="removeAllMenusClass();openPage(this, '<%=lf.getName(request)%>','<%=lf.isCanRepeat()%>','true','<%=lf.getLink(request)%>','<%=lf.getTarget()%>')"><%=lf.getName(request)%>
                </a>
            </div>
        </div>
    </div>
    ...
    <div class='submenu-wrap-box' id="<%=lf.getCode() %><%=flagNum2 %>">
        <div class='submenu-wrap-box-title'>
            <div><%=lf.getName(request)%> / <%=lf4.getName(request)%>
            </div>
        </div>
        <div class='submenu-wrap-thirdmenu'>
            <%
            ...
            %>
            <div class='submenu-wrap-box-div'>
                <a href='javascript:;' onclick="removeAllMenusClass();openPage(this, '<%=lf5.getName(request)%>','<%=lf5.isCanRepeat()%>','true','<%=lf5.getLink(request)%>','<%=lf5.getTarget()%>')"><%=lf5.getName(request)%>
                </a>
            </div>
            <%
                    menuNum2++;
    ...
    <div class='submenu-wrap-box'>
        <div class='submenu-wrap-box-title'>
            <div><%=lf.getName(request) %> /</div>
            <div class='submenu-wrap-box-div'><a href='javascript:;' onclick="removeAllMenusClass();openPage(this, '<%=lf4.getName(request)%>','<%=lf4.isCanRepeat()%>','true','<%=lf4.getLink(request)%>','<%=lf4.getTarget()%>')"><%=lf4.getName(request)%>
            </a></div>
        </div>
    </div>
    <hr/>
...
<script type="text/JavaScript">
    ...
    function notifyMsg(msgs) {
        ...
        // 20210118 因chrome下当失去焦点时，会暂停tickout，而当重获焦点时，会执行中间暂停的N次tickout，致窗口会反复升降弹出，
        // 而通过window.onblur、window.onfocus来控制setTimeout是否执行并不理想，因为事件有时能检测到，而大多数时候检测不到
        // tickout(15);
