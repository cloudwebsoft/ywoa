...
if (hasCond && !"matchNextBranch".equals(op)) {
	if (!"matchAfterSelDept".equals(op) && !"reMatchUser".equals(op)) { // 选择兼职部门
...
if (hasCond && !isCondSatisfied) {
  ...
  <%
	  ...
	  %>
		  ...
		  <input class="btn btn-default" type="button" value="选择用户" onClick="OpenModifyWin('<%=towa.getInternalName()%>', '<%=towa.getId()%>', 'true')">
	  <%}
	  else {
		  ...
		  if (towa.getNodeMode()== WorkflowActionDb.NODE_MODE_USER_SELECTED || towa.getNodeMode()== WorkflowActionDb.NODE_MODE_USER) {
			  nodeMode = WorkflowActionDb.NODE_MODE_USER_SELECTED;
		  }
		  %>
		  <span id=spanUsers_<%=towa.getId()%>>          
		  <span id="checker">
	  ...
	  %>
          ...
          <%		  
		  if (canSelUser) {%>
	          <input class="btn btn-default" type="button" value="选择用户" onClick="OpenModifyWin('<%=towa.getInternalName()%>', '<%=towa.getId()%>', '<%=flagXorRadiate?"true":"false"%>')">
		  <%}
	  }
  ...
  if (q==0) {
	...
	String deptOfUserWithMultiDept = ParamUtil.get(request, "deptOfUserWithMultiDept");
%>
	<input type="hidden" name="deptOfUserWithMultiDept0" value="<%=deptOfUserWithMultiDept%>"/>
  ...
  <%	  }
	  ...
	  %>
		  ...
		  %>
		  <input class="btn btn-default" type="button" value="选择用户" onclick="OpenModifyWin('<%=towa.getInternalName()%>', '<%=towa.getId()%>', '<%=flagXorRadiate?"true":"false"%>', <%=wa.getId()%>)">
	  <%
			  }
	  }
...
<%
  if (isMatchUserException) {
      ...
      <span id="spanTokenException" style="display:none">isMatchUserException</span><!--用于在flow_dispose.jsp页面上判断是否需选择兼职部门-->
		<span style="margin: 5px 0px 10px 10px">
	  请选择您所在的部门&nbsp;
	  <%
	  DeptMgr dm = new DeptMgr();
	  while (irdu.hasNext()) {
	  ...
	  <script>
		  spanNextUser.style.display = "";
		  // 清除deptOfUserWithMultiDept0，否则会出现class cast exception
		  $('#deptOfUserWithMultiDept0').remove();
	  </script>
	  <%
  }
}
