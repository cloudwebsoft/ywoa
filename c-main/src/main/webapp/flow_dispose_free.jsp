...
if ( wa==null || !wa.isLoaded() ) {
	out.print(SkinUtil.makeErrMsg(request, "流程中的相应动作：" + actionId + "不存在！"));
	return;
}
...
if (!wf.isLoaded()) {
	out.print(SkinUtil.makeErrMsg(request, "流程不存在"));
	return;
}
...
if (lf == null) {
	out.print(SkinUtil.makeErrMsg(request, "流程类型已被删除"));
	return;
}
...
while (ir.hasNext()) {
	...
	%>
      ...
      <div id="formContent" class="form-content" style="clear:both">
		...
		%>
		<br>
        <div id="divAnnex" style="width:98%;background-color:#efefef;padding-top:10px;padding-bottom:10px;margin-bottom:50px;<%=replyDis %>">
...
<%}%>
	...
	,'-',
		...
		<%} else {%>
		{
		  ...
		  title: '<%=conf.getBtnTitle("FLOW_BUTTON_CANCEL_ATTENTION").startsWith("#")?LocalUtil.LoadString(request,"res.flow.Flow","cancelAttentionTitle"):conf.getBtnTitle("FLOW_BUTTON_CANCEL_ATTENTION")%>',
