...
<div id="photo">
附近位置：<%=ld.getString("address")%><br>
时间：<%=DateUtil.format(ld.getDate("create_date"), "yyyy-MM-dd HH:mm:ss")%>
<%if (ld.getString("file_path")!=null && !"".equals(ld.getString("file_path"))) {%>
<div>
<a href="javascript:void(0)" ><img style="border:0px; width:100%;" src="<%=request.getContextPath()%>/img_show.jsp?path=upfile/<%=ld.getString("file_path")%>"  onclick="window.open('<%=request.getContextPath() %>/img_show.jsp?path=upfile/<%=ld.getString("file_path")%>')"/></a>
</div>
<%}%>
</div>
