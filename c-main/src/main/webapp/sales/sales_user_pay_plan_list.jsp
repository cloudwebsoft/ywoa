...
function doSort(orderBy) {
	...
	window.location.href = "sales_user_pay_plan_list.jsp?op=<%=op%>&customer=<%=StrUtil.UrlEncode(customer)%>&beginDate=<%=strBeginDate%>&endDate=<%=strEndDate%>&orderBy=" + orderBy + "&sort=" + sort;
}
</script>
</head>
<body>
...
String sql = "select p.id from " + fd.getTableNameByForm() + " p, form_table_sales_order o, form_table_sales_customer c where c.sales_person=" + StrUtil.sqlstr(userName) + " and p.cws_id=o.id and o.cws_id=c.id";
if (op.equals("search")) {
	if (!customer.equals("")) {
		sql = "select p.id from " + fd.getTableNameByForm() + " p, form_table_sales_order o, form_table_sales_customer c where c.sales_person=" + StrUtil.sqlstr(userName) + " and p.cws_id=o.id and o.cws_id=c.id and c.customer like " + StrUtil.sqlstr("%" + customer + "%");
	}
	if (beginDate!=null) {
...
<form id="formSearch" name="formSearch" method="get" action="sales_user_pay_plan_list.jsp">
      ...
      <%
        ...
        <tr>
          ...
          <td align="center">
          <a href="../visual/module_mode1_show_relate.jsp?parentId=<%=fdao.getCwsId()%>&id=<%=fdao.getId()%>&menuItem=6&formCodeRelated=sales_ord_pay_plan&formCode=sales_order&isShowNav=1">查看</a>
          </td>
        </tr>
      <%
		}
...
function changeSort(sortname, sortorder) {
	window.location.href = "sales_user_pay_plan_list.jsp?<%=querystr%>&pagesize=" + flex.getOptions().rp + "&orderBy=" + sortname + "&sort=" + sortorder;
}

function changePage(newp) {
	if (newp){
		window.location.href = "sales_user_pay_plan_list.jsp?<%=querystr%>&CPages=" + newp + "&pagesize=" + flex.getOptions().rp;
		}
}

function rpChange(pagesize) {
	window.location.href = "sales_user_pay_plan_list.jsp?<%=querystr%>&CPages=<%=curpage%>&pagesize=" + pagesize;
}
