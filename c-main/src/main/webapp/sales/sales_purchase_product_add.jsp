...
function doAddRow(response) {
	...
	for (var i=0;i<productTable.rows[0].cells.length;i++) {
		...
		if (i==0)
			...
			the_cell.innerHTML = "<input name='price' size=3>" + data.measure_unit;
		else if (i==2)
			the_cell.innerHTML = "<input name='num' size=3>" + data.measure_unit;
		else if (i==3)
			the_cell.innerHTML = "<input name='total' size=10>";
		else if (i==4)
...
function onload() {	
	...
	for (var i=1; i<rows; i++) {
		...
		the_cell.innerHTML = "<input name='price' size=3 value='" + cel[j+1].innerText + "'>";
		the_cell = newRow.insertCell(2);
		the_cell.innerHTML = "<input name='num' size=3 value='" + cel[j+2].innerText + "'>";
		the_cell = newRow.insertCell(3);
		the_cell.innerHTML = "<input name='total' size=10 value='" + cel[j+3].innerText + "'>";
		the_cell = newRow.insertCell(4);
		the_cell.innerHTML = "<input name='remark' size=10 value='" + cel[j+4].innerText + "'>";
  ...
  <tr>
    ...
    <ul class="simpleTree">
        <li class="root"><span>产品分类</span>
            <ul class="ajax">
              <li id='sales_provider'>{url:sales_purchase_product_add_ajax.jsp?op=list}</li>
            </ul>
        </li>
    </ul>
	</td>
