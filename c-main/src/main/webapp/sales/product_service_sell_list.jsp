...
String formCode = "sales_product_service_sell";
...
<body>
<%@ include file="product_service_sell_inc_menu_top.jsp"%>
<script>
...
<table class="tabStyle_1 percent98" width="98%" border="0" align="center" cellpadding="2" cellspacing="1">
  ...
  <%	
		...
		%>
  <tr align="center">
    <td width="26%"><a href="product_service_sell_show.jsp?id=<%=id%>&formCode=<%=formCode%>"><%=fdao.getFieldValue("product_name")%></a></td>
    ...
    <td width="17%"><a href="product_service_sell_edit.jsp?id=<%=id%>&formCode=<%=StrUtil.UrlEncode(formCode)%>">编辑</a>&nbsp;&nbsp;<a onClick="if (!confirm('您确定要删除么？')) return false" href="../visual_del.jsp?id=<%=id%>&formCode=<%=formCode%>&privurl=<%=StrUtil.getUrl(request)%>">删除</a>&nbsp;&nbsp; </td>
  </tr>
  <%
		}
%>
