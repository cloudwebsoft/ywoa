...
querystr = "op=" + op + "&code=" + moduleCode + "&menuItem=" + menuItem + "&formCode=" + formCode + "&formCodeRelated=" + formCodeRelated + "&customerId=" + parentId + "&parentId=" + parentId + "&orderBy=" + orderBy + "&sort=" + sort + "&isShowNav=" + isShowNav;
if (!sqlUrlStr.equals(""))
	querystr += "&" + sqlUrlStr;
...
function doSort(orderBy) {
	...
	window.location.href = "module_list_relate.jsp?menuItem=<%=menuItem%>&parentId=<%=parentId%>&formCode=<%=formCode%>&menuItem=<%=menuItem%>&formCodeRelated=<%=formCodeRelated%>&orderBy=" + orderBy + "&sort=" + sort + "&isShowNav=<%=isShowNav%>";
}
...
function del(){
	...
	jConfirm("您确定要删除吗？","提示",function(r){
		if(!r){return;}
			...
			window.location.href = "module_list_relate.jsp?action=del&<%=querystr%>&CPages=<%=curpage%>&id=" + id + "&isShowNav=<%=isShowNav%>";
		}
	})
}
</script>
...
<table class="percent98" width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="48%" height="28" align="left">
	<%if (mpd.canUserAppend(privilege.getUser(request))) {%>    
    <input name="button" class="btn" type="button" onclick="window.location.href='module_add_relate.jsp?code=<%=StrUtil.UrlEncode(moduleCode)%>&customerId=<%=parentId %>&parentId=<%=parentId%>&menuItem=<%=menuItem%>&formCode=<%=formCode%>&amp;formCodeRelated=<%=formCodeRelated%>&isShowNav=<%=isShowNav%>'" value="添加" />
    <%} %>
    &nbsp;&nbsp;
    <input name="button3" class="btn" type="button" onclick="window.location.href='module_search_relate.jsp?code=<%=moduleCode%>&customerId=<%=parentId %>&parentId=<%=parentId%>&amp;menuItem=<%=menuItem%>&amp;formCode=<%=formCode%>&amp;formCodeRelated=<%=formCodeRelated%>&isShowNav=<%=isShowNav%>'" value="查询" /></td>
...
<%
	...
	%>	
		...
		<%}%>		
		<!--<a href="module_show_relate.jsp?parentId=<%=parentId%>&id=<%=id%>&formCodeRelated=<%=formCodeRelated%>&formCode=<%=formCode%>&isShowNav=<%=isShowNav%>">
		-->
		<span>
		<%if (!fieldName.equals("cws_creator")) {
			if (fieldName.startsWith("main")) {
	...
	<td>
		<%if (mpd.canUserSee(privilege.getUser(request))) {%>
		<a href="module_show_relate.jsp?code=<%=moduleCode%>&customerId=<%=parentId %>&parentId=<%=parentId%>&id=<%=id%>&menuItem=<%=menuItem%>&code=<%=formCodeRelated%>&parentModuleCode=<%=formCode%>&isShowNav=<%=isShowNav%>">查看</a>
		<%}%>
		<%if (mpd.canUserModify(privilege.getUser(request))) {%>
		&nbsp;&nbsp;&nbsp;&nbsp;
		<a href="module_edit_relate.jsp?code=<%=moduleCode%>&customerId=<%=parentId %>&parentId=<%=parentId%>&id=<%=id%>&menuItem=<%=menuItem%>&formCodeRelated=<%=formCodeRelated%>&formCode=<%=formCode%>&isShowNav=<%=isShowNav%>">编辑</a>
		<%}%>
		<%if (mpd.canUserManage(privilege.getUser(request))) {%>
		&nbsp;&nbsp;&nbsp;&nbsp;
		<a onclick=" jConfirm('您确定要删除么？','提示',function(r){ if(!r){event.returnValue=false;}else{ window.location.href='module_list_relate.jsp?action=del&code=<%=moduleCode%>&op=<%=op%>&customerId=<%=parentId %>&id=<%=id%>&formCode=<%=formCode%>&formCodeRelated=<%=formCodeRelated%>&menuItem=<%=menuItem%>&parentId=<%=parentId%>&CPages=<%=curpage%>&orderBy=<%=orderBy%>&sort=<%=sort%>&<%=sqlUrlStr%>&isShowNav=<%=isShowNav%>'}}) " style="cursor:pointer">删除</a>
		<%}%>
	    <% WorkLogForModuleMgr wlfm = new WorkLogForModuleMgr();
	    	if(is_workLog==1){ %>
	    		&nbsp;&nbsp;<a href="javascript:;" onclick="addTab('<%=msd.getString("name")%>汇报', '<%=request.getContextPath()%>/mywork/queryMyWork.action?code=prj_task&id=<%=id%>')">汇报</a>
	    <%
	    	}
	    %>
		</td>
...
<table width="98%" border="0" cellspacing="0" cellpadding="0" align="center" class="percent98">
  <tr>
    <td width="50%" height="23" align="left"><input class="btn" name="button2" type="button" onclick="selAllCheckBox('ids')" value="全选" />
	  ...
	  <%if (mpd.canUserSee(privilege.getUser(request))) {%>      
      &nbsp;&nbsp;
      <input name="button4" type="button" class="btn" onclick="javascript:window.open('<%=request.getContextPath() %>/visual/exportExcelRelate.do?<%=querystr%>')" value="导出" />
      <%} %>
<%
...
%>	
    ...
    <td width="50%" align="right"><%
		out.print(paginator.getCurPageBlock("module_list_relate.jsp?"+querystr));
	%></td>
  </tr>
</table>
</body>
