...
if (op.equals("setForum")) {
	Element e = pm.getRootElement();
	Iterator ir = e.getChildren("plugin").iterator();
	while (ir.hasNext()) {
		Element pl = (Element)ir.next();
		if (pl.getAttributeValue("code").equals(RemarkUnit.code)) {
			Element etype = (Element)pl.getChild("type");
			if (etype.getText().equals(PluginUnit.TYPE_BOARD)) {
				etype.setText(PluginUnit.TYPE_FORUM);
			}
			else {
				etype.setText(PluginUnit.TYPE_BOARD);				
			}
			pm.writemodify();
			pm.reload();
			out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request, "info_op_success"), "manager.jsp"));			
			return;
		}
	}
}
...
<table width='100%' cellpadding='0' cellspacing='0' >
  <tr>
    <td class="head">管理插件-<%=RemarkSkin.LoadString(request, "name")%></td>
  </tr>
</table>
<br>
<table width="98%" border='0' align="center" cellpadding='0' cellspacing='0' class="frame_gray">
  <tr> 
    <td height=20 align="left" class="thead">管理 - <%=RemarkSkin.LoadString(request, "name")%></td>
  </tr>
  <tr>
    <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td align="center"><a href="config_remark.jsp">配置</a></td>
      </tr>
    </table>
      <br>
...
&nbsp;&nbsp;&nbsp;&nbsp;          <input type=submit value="设为支持<%=RemarkSkin.LoadString(request, "name")%>功能">
&nbsp;
<a href="manager.jsp?op=setForum">
