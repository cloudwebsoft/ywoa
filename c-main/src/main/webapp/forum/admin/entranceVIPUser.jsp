...
if (op.equals("add")) {
	...
	if (!boardCode.equals("")) {
		String entranceCode = VIPUserEntrance.CODE;
...
if (op.equals("deluser")) {
	...
	VIPUserDb be = new VIPUserDb();
	be = be.getVIPUserDb(userName);
	if (be.del()) {
		out.print(StrUtil.Alert(SkinUtil.LoadString(request, "info_op_success")));
	}
	else
...
- <%=SkinUtil.LoadString(request, "res.config.entrance", VIPUserEntrance.CODE)%></td>
  </tr>
  <tr> 
    <td valign="top"><br>
...
Vector v = br.list(VIPUserEntrance.CODE);
...
&nbsp;&nbsp;&nbsp;&nbsp;	  		    </td>
				          <td width="65%" align="left">			    <input type=submit value="<lt:Label res="res.label.forum.admin.entrance" key="add_board"/>">
&nbsp;&nbsp;<input type="button" onClick="window.location.href='VIPUser_add.jsp'" value="<lt:Label res="res.label.forum.admin.entrance" key="add_vip_user"/>"></td>
		      </tr></form>
      </table>
      <%
int pagesize = 10;
...
String sql = "select userName from " + vtu.getTableName() + " order by beginDate desc";	
...
int i = 0;
while (irt.hasNext()) {
	vtu = (VIPUserDb)irt.next();
	i++;
%>
        <form id="form<%=i%>" name="form<%=i%>" action="?op=modify" method="post">
          <tr align="center">
            <td height="22" bgcolor="#FFFFFF"><%=um.getUser(vtu.getUserName()).getNick()%> </td>
            ...
            <td height="22" bgcolor="#FFFFFF"><a href="VIPUser_edit.jsp?userName=<%=StrUtil.UrlEncode(vtu.getUserName())%>"><lt:Label key="op_edit"/></a>&nbsp;&nbsp;<a href="#" onClick="if (confirm('<lt:Label key="confirm_del"/>')) window.location.href='?op=deluser&userName=<%=StrUtil.UrlEncode(vtu.getUserName())%>'"><lt:Label key="op_del"/></a></td>
          </tr>
        </form>
        <%}%>
      </table>
