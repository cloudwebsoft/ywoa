...
<title>Board visit day Statistic</title>
...
<table width="98%" border='0' align="center" cellpadding='0' cellspacing='0' class="frame_gray">
  <tr> 
    <td height=20 align="left" class="thead">版块 - <%=lf.getName()%> 访问统计</td>
  </tr>
  <tr> 
    <td valign="top"><br>
      <TABLE width="92%" border=0 align=center cellPadding=0 cellSpacing=1>
      <TBODY>
      <form name="fmFilter" action="board_visit_statistic_day_detail.jsp?op=search&boardcode=<%=boardcode%>" method="post">
        <TR>
...
<script type="text/javascript">
	...
	String sql = "select count(*) from " + bvld.getTable().getName() + " where boardcode=? and add_date>=? and add_date<=?";
	int dayCount = 0;
	JdbcTemplate jt = new JdbcTemplate();
	d = DateUtil.addDate(d, 1);	
	while (DateUtil.datediffMinute(ed, d)>=-1) {	
		ri = jt.executeQuery(sql, new Object[] {boardcode, oldd, d});
		if (ri.hasNext()) {
  ...
  <tr>
    <td align="center"><%
JFreeChart chart = ChartFactory.createBarChart3D("版块日访问统计图", 
                  "日期",
                  "访问量",
                  dataset,
