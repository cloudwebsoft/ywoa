...
<%		
	...
	if (op.equals("del")) {
		...
		String sql = "delete from " + bvld.getTable().getName() + " where boardcode<>" + StrUtil.sqlstr(Leaf.CODE_BLOG) + " and add_date<=?";
		if (beginDate.equals("")) {
...
<input name="button" type="button" onClick="del()" value="删除"></TD>
	...
	%>
      <tr align="center">
        ...
        <td align="left" bgcolor="#FFF7FF"><a target="_blank" href="../../userinfo.jsp?username=<%=StrUtil.UrlEncode(md.getName())%>"><%=um.getUser(md.getName()).getNick()==null?"游客":um.getUser(md.getName()).getNick()%></a></td>
