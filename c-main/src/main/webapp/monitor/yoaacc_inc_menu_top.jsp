...
<div id="tabs1">
  <ul>
    ...
    if (moduleUrlList.equals("")) {
      moduleUrlList = request.getContextPath() + "/" + "monitor/yoaacc_list.jsp?formCode=" + StrUtil.UrlEncode(formCodeTop);
    }
    else {
...
<%
// 当页面为edit、show、list即yoaacc_edit.jsp yoaacc_show.jsp yoaacc_list.jsp时不需要处理关联模块时
...
if (parentIdTop==-1) {
	...
	%>	
	<li id="menu2"><a href="<%=request.getContextPath()%>/monitor/yoaacc_add.jsp?formCode=<%=formCodeTop%>"><span>添加</span></a></li>
	<%}%>	
...
if (parentIdTop!=-1) {
	...
	%>
		<li id="menu<%=menuItemTop%>"><a href="<%=request.getContextPath()%>/monitor/yoaacc_list_relate.jsp?parentId=<%=parentIdTop%>&menuItem=<%=menuItemTop%>&formCodeRelated=<%=mrdTop.getString("relate_code")%>&formCode=<%=formCodeTop%>"><span><%=fdTop.getFormDb(mrdTop.getString("relate_code")).getName()%></span></a></li>
	<%
		menuItemTop++;
	}
	%>
