...
String isShowMessage = ParamUtil.get(request,"isShowMessage");
...
String sql = "select distinct a.id,a.sender,a.subject,a.mydate,a.is_readed,a.email_addr from email a left join email_attach b on a.id = b.emailId where  a.email_addr=" + StrUtil.sqlstr(email) + " and a.msg_type=" + MailMsgDb.TYPE_DRAFT;
...
if(op.equals("updateTotal")){
	...
	String update_sql = "select distinct a.id,a.sender,a.subject,a.mydate,a.is_readed,a.email_addr from email a left join email_attach b on a.id = b.emailId where  a.email_addr=" + StrUtil.sqlstr(email) + " and a.msg_type=" + MailMsgDb.TYPE_DRAFT;
...
<form id="formSearch" action="list_draft.jsp" method="get">
...
function changeSort(sortname, sortorder) {
	window.location.href = "list_draft.jsp?id=<%=id%>&pageSize=" + flex.getOptions().rp + "&orderBy=" + sortname + "&sort=" + sortorder;
}

function changePage(newp) {
	if (newp)
		window.location.href = "list_draft.jsp?id=<%=id%>&CPages=" + newp + "&pageSize=" + flex.getOptions().rp + "&orderBy=<%=orderBy%>" + "&sort=<%=sort%>"+ "&subMenu=<%=subMenu%>" + "&subMenuButton=<%=subMenuButton%>";
}

function rpChange(pageSize) {
	window.location.href = "list_draft.jsp?id=<%=id%>&CPages=<%=curpage%>&pageSize=" + pageSize + "&orderBy=<%=orderBy%>" + "&sort=<%=sort%>"+ "&subMenu=<%=subMenu%>" + "&subMenuButton=<%=subMenuButton%>";
}
