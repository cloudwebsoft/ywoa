...
<%
	...
	String sql = "select distinct a.id,a.sender,a.subject,a.mydate,a.is_readed,a.email_addr,a.msg_level from email a left join email_attach b on a.id = b.emailId where  a.email_addr=" + StrUtil.sqlstr(email) + " and a.msg_type=" + MailMsgDb.TYPE_DRAFT;
	  		...
	  		while(ri.hasNext()){
	  			...
	  			String str ="<tr id=\""+rr.getInt("id")+"\" lang=\""+rr.getInt("is_readed")+"\">"
	  						...
	  						if(rr.getInt("is_readed") == 1){
								...
								if(msgLevel == 1){
										str+= "<div title=\"紧急\" style=\"display:inline\"><font color =\"red\" style=\"display:inline\">!</font></div>";
									}
								str += "</div></td>";
							}else{
	  						...
	  						str +="<td><div style=\"width:190px;text-align:left;\"><a href=\"mail_edit.jsp?isClickMenu=1&draftId="+rr.getInt("id")+"&emailAddr="+rr.getString("email_addr")+"&subMenu="+subMneu+"&subMenuButton="+subMenuButton+"&box="+MailMsgDb.TYPE_DRAFT+"\">"+sender+"</a></div></td>";
	  						str +="<td><div style=\"width:500px;text-align:left;\"><a href=\"mail_edit.jsp?isClickMenu=1&draftId="+rr.getInt("id")+"&emailAddr="+rr.getString("email_addr")+"&subMenu="+subMneu+"&subMenuButton="+subMenuButton+"&box="+MailMsgDb.TYPE_DRAFT+"\">"+subject+"</a></div></td>";
