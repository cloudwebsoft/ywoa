...
String sql = "select l.lxr,w.mydate,w.id,l.id,l.contact_type from form_table_day_lxr l, form_table_day_work_plan w where w.id=l.cws_id and l.is_visited='否' and w.cur_user=? order by w.id desc";
// out.print(sql);
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr> 
      ...
      <table width="98%" align="center" cellspacing="0" cellpadding="0" class="tabStyle_1 percent98">
        <tr>
          <td width="9%" class="tabStyle_1_title">编号</td> 
          <td width="17%" class="tabStyle_1_title">联系人</td> 
          <td width="11%" class="tabStyle_1_title">制订日期</td>
          <td width="33%" class="tabStyle_1_title">客户</td>
          <td width="14%" class="tabStyle_1_title">联系方式</td>
      ...
      <%	
        ...
        <tr>
          ...
          <td><a href="../sales/customer_show.jsp?id=<%=fdaoCustomer.getId()%>&formCode=sales_customer" target="_blank"><%=fdaoCustomer.getFieldValue("customer")%></a></td>
          <td><%=rr.getString("contact_type")%></td>
          <td align="center">
			<%if (curUser.equals(privilege.getUser(request)) || privilege.canAdminUser(request, curUser)) {%>
			<a target="_blank" href="../visual/module_edit_relate.jsp?parentId=<%=rr.getInt(3)%>&id=<%=rr.getInt(4)%>&menuItem=&moduleCodeRelated=day_lxr&code=day_work_plan&isShowNav=0">编辑</a>&nbsp;&nbsp;&nbsp;&nbsp;
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr> 
		<td width="50%"><input name="button3"  class="btn" type="button" onclick="selAllCheckBox('mobiles')" value="全选" />
&nbsp;
<%
if (com.redmoon.oa.sms.SMSFactory.isUseSMS()) {
%>
<input name="button" class="btn" type="button" onclick="sendSms()" value="短信" />
<%}%></td>
