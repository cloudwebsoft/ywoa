...
function changeSort(sortname, sortorder) {
	...
	// console.log(params);
	var urlStr = "<%=request.getContextPath()%>/public/flow/listAttLog.do?" + params;	
...
%>
var requestParams = [];
requestParams.push({name:'flowId', value:'<%=flowId%>'});
requestParams.push({name:'attId', value:'<%=attId%>'});

var colModel = [<%=colProps%>];
$("#grid").flexigrid({
	url: '../public/flow/listAttLog.do',
...
function del(ids) {
	...
	jConfirm('您确定要删除么？', '提示', function(r) {	
		...
		$.ajax({
			type: "post",
			url: "<%=request.getContextPath()%>/public/flow/delLog.do",
			data: {
				ids: ids
			},
			dataType: "html",
