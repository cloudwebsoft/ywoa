...
<%
    ...
    if (!"".equals(typeCode)) {
        ...
        if (colLeaf == null) {
            out.println(cn.js.fan.web.SkinUtil.makeErrMsg(request, "流程typeCode=" + typeCode + "不存在"));
            return;
        }
    ...
    if (!"".equals(typeCode)) {
        ...
        canQuery = leafPriv.canUserQuery(myname);
    }
    else {
        //  canUserQuery中根目录给予了所有人查询权限，故当typeCode为空时需判断是否为管理员，而不能用leafPriv.canUserQuery(Leaf.CODE_ROOT)来判断
...
<table id="searchTable" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td>
            <form class="search-form" action="flow_list.jsp" method="get" onsubmit="return false">
                ...
                <span class="condSpan">
                从
                <input id="fromDate" name="f.fromDate" title="发起时间"/>
                至
                <input id="toDate" name="f.toDate" title="发起时间"/>
                </span>
                <%
                ...
                <input type="submit" value='' class="tSearch" onclick="doQuery()" />
            </form>
        </td>
    </tr>
</table>
    ...
    $(document).ready(function () {
        ...
        flex = $("#grid").flexigrid({
            ...
            %>
                ...
                showToggleBtn: <%=canQuery?true:false%>,
