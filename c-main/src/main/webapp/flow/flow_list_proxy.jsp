...
try {
	com.redmoon.oa.security.SecurityUtil.antiSQLInject(request, privilege, "userName", myname, getClass().getName());
}
...
function onTypeCodeChange(obj) {
	if(obj.options[obj.selectedIndex].value=='not'){jAlert(obj.options[obj.selectedIndex].text+' 不能被选择！','提示'); return false;}
	window.location.href = "flow_list_proxy.jsp?op=search&by=" + $("by").value + "&typeCode=" + obj.options[obj.selectedIndex].value;
}
...
<table id="searchTable" width="80%" border="0" align="center">
    <tr>
      <td align="center">
  <form name=form1 action="flow_list_proxy.jsp?op=search" method=post onsubmit="return submitValidate()">
...
<table width="981" height="48" id="grid">
  <thead>
    <tr>
      ...
      <th width="75"><lt:Label res="res.flow.Flow" key="processor"/></th>
...
while (ir.hasNext()) {
	...
	UserDb checkerUser = um.getUserDb(mad.getChecker());
	%>
    <tr>
	  ...
	  if (isExpired) {%>
        <img src="../images/flow/expired.png" align="absmiddle" alt="超时" />
        <%}%>      
      ...
      <td align="left"><%=checkerUser.getRealName()%></td>
...
function changeSort(sortname, sortorder) {
	window.location.href = "flow_list_proxy.jsp?<%=querystr%>&pageSize=" + flex.getOptions().rp + "&orderBy=" + sortname + "&sort=" + sortorder;
}

function changePage(newp) {
	if (newp)
		window.location.href = "flow_list_proxy.jsp?<%=querystr%>&CPages=" + newp + "&pageSize=" + flex.getOptions().rp;
}

function rpChange(pageSize) {
	window.location.href = "flow_list_proxy.jsp?<%=querystr%>&CPages=<%=curpage%>&pageSize=" + pageSize;
}
