...
<table id="searchTable" border="0" align="center">
  <tr>
    <td align="center"> 
      <form name="formSearch" class="search-form" action="flow_performance_user_list_average.jsp" method="get">
        &nbsp;&nbsp;年度
        <select id="showyear" name="showyear" onchange="var y=this.options[this.selectedIndex].value; window.location.href='flow_performance_user_list_average.jsp?deptCode=<%=deptCode%>&showyear=' + y;">
...
<table width="583" border="0" cellpadding="0" cellspacing="0" id="grid">
  ...
  <tr class="highlight">
	...
	<%
	   sb.append(NumberUtil.round(average, 2));
	   sb.append(",");
	%>
  <%
}
...
<td><%=NumberUtil.round(yAverage, 2)%></td>
	<%
	   sb.append(NumberUtil.round(yAverage, 2));
       sb.append("|");
	%>
</tr>
<%}%>
...
function changeSort(sortname, sortorder) {
	window.location.href = "flow_performance_user_average.jsp?deptCode=<%=deptCode%>&pageSize=" + flex.attr('p').rp + "&orderBy=" + sortname + "&sort=" + sortorder;
}

function changePage(newp) {
	if (newp)
		window.location.href = "flow_performance_user_average.jsp?deptCode=<%=deptCode%>&CPages=" + newp + "&pageSize=" + flex.attr('p').rp;
}

function rpChange(pageSize) {
	window.location.href = "flow_performance_user_average.jsp?deptCode=<%=deptCode%>&CPages=<%=curpage%>&pageSize=" + pageSize;
}
function onReload() {
