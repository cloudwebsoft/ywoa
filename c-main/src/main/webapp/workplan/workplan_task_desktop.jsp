...
<div id="drag_<%=id%>" dragTitle="<%=udsd.getTitle()%>" count="<%=udsd.getCount()%>"
    ...
    <div id="drag_<%=id%>_h" class="box ibox-title">
        ...
        %>
        <h5>
            <i class="fa <%=udsd.getIcon()%>"></i>&nbsp;&nbsp;<a href="<%=request.getContextPath()%>/workplan/workplan_task_list.jsp"><%=udsd.getTitle()%></a>
        </h5>
        <%
        }
        else {
        ...
        <div class="titleimg">
            ...
            &nbsp;&nbsp;</div>
        <div class="titletxt">&nbsp;&nbsp;<a href="<%=request.getContextPath()%>/workplan/workplan_task_list.jsp"><%=udsd.getTitle()%></a></div>
        <%
            }
        %>
    </div>
        ...
        <table class='article_table'>
            ...
            <tr>
                <td class='article_content'>
                    <a href="workplan/workplan_task.jsp?id=<%=wptd.getLong("work_plan_id")%>&userName=<%=StrUtil.UrlEncode(userName)%>"
                       title="<%=StrUtil.HtmlEncode(wptd.getString("name"))%>"><%=StrUtil.getAbstract(request, wptd.getString("name"), udsd.getWordCount(), "，")%>
                    </a>
                </td>
                <td class='article_time'><%=DateUtil.format(wptd.getDate("start_date"), "yyyy-MM-dd")%>
                </td>
            </tr>
            <%}%>
        </table>
