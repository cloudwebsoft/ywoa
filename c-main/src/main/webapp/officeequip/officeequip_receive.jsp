...
<form action="?op=chageStorageCount&type=0" name="form1" id="form1" method="post">
<tr>
  <td colspan="2" class="tabStyle_1_title">办公用品领用登记 </td>
</tr>
...
<tr>
    ...
    <tr>
      <td align="center"><script>
       ...
       领用数量：</td>
      <td>
		<input name="storageCount" type="text" id="storageCount" size="20" >
	<font color=red>  *  </font>
	</td>
      </tr>
    <tr>
      <td align="center">领&nbsp;&nbsp;用&nbsp;&nbsp;人：</td>
      <td>
      <%if (privilege.isUserPrivValid(request, "officeequip")) {%>
    ...
    <tr>
      ...
      <td><input id="opDate" name="opDate" type="text" value="<%=dt%>" size="20">
        </td>
      </tr>
    <tr>
      <td align="center">备&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;注：</td>
      <td><textarea  name="abstracts"  id="abstracts" style="width:90%" rows="5"></textarea></td>
    </tr>
 <tr> 
  <td colspan="2" align="center">
     <input name="submit" type="button" class="btn"  value="确定" onclick="mysubmit()" > 
     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
     <input type="button" class="btn"  value="返回" onclick="window.location.href='officeequip_return_list.jsp?opType=<%=OfficeOpDb.TYPE_RECEIVE%>'" ></td>
</tr>
</form>
</table>
<script>
...
function mysubmit(){
	$.ajax({
		url: "officeequip_do.jsp?op=op_add&type=<%=OfficeOpDb.TYPE_RECEIVE%>&" + $('#form1').serialize(),
		type: "post",
		dataType: "json",
		success: function(data, status){
			if(data.ret == 1){
				...
				$('#abstracts').val("");
				$('#opDate').val("<%=dt %>");
				jAlert_Redirect(data.msg, "提示", "officeequip_return_list.jsp?opType=<%=OfficeOpDb.TYPE_RECEIVE%>");
			}
			else {
				jAlert(data.msg, "提示");
			}
