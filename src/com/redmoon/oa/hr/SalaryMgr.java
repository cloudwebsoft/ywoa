...
public class SalaryMgr {
	...
	public static double makeSingleSubjectForUser(String personId, Vector vtSubjectOfBook, int bookId, int year, int month, FormDAO fdaoSubject, FormDAO fdaoFormula, FormDb fdFormula, Map<String, Double> mapSubjectVal) throws ErrMsgException {
		...
		if (mapSubjectVal.containsKey(colName)) {
			return mapSubjectVal.get(colName).doubleValue();
		}
		...
		// 公式型
			...
			if (code==null || "".equals(code)) {
				DebugUtil.log(SalaryMgr.class, "makeSingleSubjectForUser", "科目：" + colName + " 中的公式未选择！");
				return 0;
			}

			String[] paramAry = null;
			int paramAryLen = 0;
			// 科目中的公式，参数中需包含personId,bookId,year,month的子集，且参数名需一致
			// 取公式的形参
			...
			// 如果科目中的形参不为空，则以科目中的形参为准，如果为空，则表示以公式中的参数为准
			if (subjectParamsAry!=null) {
				if (paramAryLen != subjectParamsAry.length) {
					throw new ErrMsgException("科目" + subjectName + "中的参数" + subjectParams + "与公式中的参数" + params + "的数量不一致！");
				}
			...
			if (paramAryLen > 0) {
				for (int i = 0; i < paramAryLen; i++) {
					if (paramAry[i].equalsIgnoreCase("personId")) {
						paramAry[i] = personId;
					}
					else if (paramAry[i].equalsIgnoreCase("bookId")) {
						paramAry[i] = String.valueOf(bookId);
					}
					else if (paramAry[i].equalsIgnoreCase("year")) {
						paramAry[i] = String.valueOf(year);
					}
					else if (paramAry[i].equalsIgnoreCase("month")) {
						...
						if (!subjCode.equals(colName)) {
							...
							if (fdaoSubj != null) {
								...
								if (mapSubjectVal.containsKey(subjCode)) {
									...
									mapSubjectVal.put(subjCode, Double.valueOf(subjVal));
								}
								paramAry[i] = String.valueOf(subjVal);
							}
							else {
								// 如果不是科目，则说明参数paramAry[i]可能未赋值，当然也可能为常量
								// 例：科目 餐补/天 是用的公式#getSalarySet，其公式参数为personId,item，而如果科目参数如果为空，则此处的paramAry[i]未能被替换，仍为item
								// 如果paramAry[i]既不是数字型也不是字符串型，则说明未被赋值
								if (!StrUtil.isDouble(paramAry[i]) && !paramAry[i].startsWith("\"")) {
									throw new ErrMsgException("科目 " + subjectName + " 调用公式" + code + "时参数" + paramAry[i] + "非法，不是personId、bookId、year、month，也不是数字、字符串常量，或其它科目");
								}
							}
						}
					}
		...
		mapSubjectVal.put(colName, Double.valueOf(val));
		return val;
	}
