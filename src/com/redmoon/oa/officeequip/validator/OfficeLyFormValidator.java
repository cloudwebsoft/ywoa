...
public class OfficeLyFormValidator implements IFormValidator {
	...
	public void onWorkflowFinished(WorkflowDb wf, WorkflowActionDb lastAction)
		...
		String count = fdao.getFieldValue("receiveCount");
		...
		if (wf.getStatus() == WorkflowDb.STATUS_FINISHED) {
			...
			office.setType(OfficeOpDb.TYPE_RECEIVE);
			Date d = null;
			if (opDate.equals("")) {
				d = new Date();
			} else {
	...
	public boolean validate(HttpServletRequest request, FileUpload fu,
		...
		if (wa.getId() == actionId) {
			// 检查人员编号是否已存在
			String count = fu.getFieldValue("receiveCount");
