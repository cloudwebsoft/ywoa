...
public class ModulePrivDb extends ObjectDb {
    ...
    public void initDB() {
        ...
        QUERY_LOAD =
                "select form_code,name,priv_type,see,append,manage,modify,view,search,re_active,field_write,field_hide,import,export,del,log from visual_module_priv where id=?";
        QUERY_DEL = "delete from visual_module_priv where id=?";
        QUERY_SAVE =
                "update visual_module_priv set see=?,append=?,manage=?,modify=?,view=?,search=?,re_active=?,field_write=?,field_hide=?,import=?,export=?,del=?,log=? where id=?";
        QUERY_CREATE =
                "insert into visual_module_priv (name,priv_type,see,append,manage,form_code,id,modify,view,search,re_active,field_write,field_hide,import,export,del,log) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    }
    ...
    public boolean canUserDo(UserDb user, UserGroupDb[] groups, RoleDb[] roles, int privType) {
        ...
        while (ir.hasNext()) {
            ...
            //　权限项对应的是组用户
            if (lp.getType() == ModulePrivDb.TYPE_USERGROUP) {
                // 组为everyone
                if (lp.getName().equals(UserGroupDb.EVERYONE)) {
                    ...
                    if (privType==PRIV_APPEND) {
                    	...
                    	return lp.getSearch()==1;
                    }
                    else if (privType==PRIV_REACTIVE) {
                    	return lp.getReActive()==1;
                    }
                    else if (privType==PRIV_IMPORT) {
                    	return lp.getImportXls()==1;
                    }
                    else if (privType==PRIV_EXPORT) {
                    	return lp.getExportXls()==1;
                    }
    ...
    public boolean canUserDo(String username, int privType) {
        ...
        if (privType!=PRIV_SEARCH && privType!=PRIV_REACTIVE) {
        ...
        if (canUserDo(user, groups, roles, privType))
            return true;
    ...
    public boolean delPrivsOfUserOrGroup(String username) {
        ...
        boolean r = false;
        try {
            String sql = "delete from visual_module_priv where name=?";
