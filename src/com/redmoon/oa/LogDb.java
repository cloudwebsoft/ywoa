...
public class LogDb extends ObjectDb {
    ...
    public void initDB() {
        ...
        QUERY_CREATE = "insert into " + tableName + " (USER_NAME, IP, LOG_TYPE, ACTION, LOG_DATE, ID, unit_code, remark,device) values (?,?,?,?,?,?,?,?,?)";
    ...
    public boolean create() {
        ...
        boolean re = false;
        // QUERY_CREATE = "insert into " + tableName + " (USER_NAME, IP, TYPE, ACTION, LOG_DATE) values (?,?,?,?,?)";
        id = (int)SequenceManager.nextID(SequenceManager.OA_LOG);
        try {
            ...
            pstmt.setString(5, "" + new java.util.Date().getTime());
            pstmt.setInt(6, id);
            pstmt.setString(7, unitCode);
            pstmt.setString(8, remark);
            pstmt.setInt(9, device);
    ...
    public boolean save() {
        ...
        try {
            ...
            pstmt.setInt(3, type);
            pstmt.setString(4, action);
            pstmt.setString(5, "" + date.getTime());
            pstmt.setInt(6, id);
            re = conn.executePreUpdate()==1?true:false;
            if (re) {
                LogCache rc = new LogCache(this);
    ...
    public void load() {
        ...
        try {
            ...
            if (rs != null) {
                if (rs.next()) {
                    // "select USER_NAME, IP, TYPE, ACTION, LOG_DATE from " + tableName + " where id=?";
                    userName = StrUtil.getNullStr(rs.getString(1));
                    ip = StrUtil.getNullStr(rs.getString(2));
                    type = rs.getInt(3);
                    action = rs.getString(4);
                    date = new java.util.Date(Long.parseLong(rs.getString(5).
                            trim()));
    ...
    public int getLoginCount(String unitCode, java.util.Date beginDate, java.util.Date endDate) {
        ...
        ResultIterator ri;
		try {
			ri = jt.executeQuery(sql, new Object[]{unitCode, new Long(beginDate.getTime()), new Long(endDate.getTime())});
	        if (ri.hasNext()) {
    ...
    public int getLoginCountOfUser(String userName, int year, int month) {
		...
		ResultIterator ri;
		try {
			ri = jt.executeQuery(sql, new Object[]{userName, new Integer(TYPE_LOGIN), new Long(d.getTime()), new Long(d2.getTime())});
			if (ri.hasNext()) {
				ResultRecord rr = (ResultRecord)ri.next();
				return rr.getInt(1);
			}			
