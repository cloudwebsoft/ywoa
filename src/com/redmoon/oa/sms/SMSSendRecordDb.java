...
public class SMSSendRecordDb extends ObjectDb {
    ...
    public ListResult listResult(String listsql, int curPage, int pageSize) throws
        ...
        try {
            ...
            int p = -1;
            // 2011-7-8黄利康改
            p = listsql.indexOf(" group by batch");
            if (p != -1) { // 在list页面
                countsql = listsql.replaceFirst("id", "count(id)");
                JdbcTemplate jt = new JdbcTemplate();
                ResultIterator ri = jt.executeQuery(countsql);
                if (ri != null && ri.hasNext()) {
                    total = ri.getRows();
                }
            } else { // 在detail_list页面
                countsql = SQLFilter.getCountSql(listsql);
