    ...
    public boolean createWithoutFile(ServletContext application,
        ...
        try {
            ...
            if (sisDdxc.equals("true")) {
                ...
                int orders = 1;
                for (int i = 0; i < len; i++) {
                	String path = mfu.getSavePath().replaceAll("\\\\","/");
                    String filepath = path+ attachFileNames[i];
                ...
                if (ret == FileUpload.RET_SUCCESS) {
                    ...
                    while (ir.hasNext()) {
                        ...
                        String path = mfu.getSavePath().replaceAll("\\\\","/");
                        filepath = path + fi.getDiskName();
                        long attachId = SequenceManager.nextID(SequenceManager.OA_DOCUMENT_ATTACH_CMS);
                        sql =
                                "insert into document_attach (id,fullpath,doc_id,name,diskname,visualpath,file_size,page_num,orders,upload_date,ext) values (" +
                                attachId + "," +
                                StrUtil.sqlstr(filepath) + "," + docId + "," +
        ...
        try {
            ...
            if (sisDdxc.equals("true")) {
                ...
                while (ir.hasNext()) {
                    ...
                    long attachId = SequenceManager.nextID(SequenceManager.OA_DOCUMENT_ATTACH_CMS);
                    sql =
                            "insert into document_attach (id,fullpath,doc_id,name,diskname,visualpath,page_num,orders,file_size,upload_date,ext) values (" +
                            attachId + "," +
                            StrUtil.sqlstr(filepath) + "," + docId + "," +
                            StrUtil.sqlstr(fi.getName()) +
                            "," + StrUtil.sqlstr(fi.getDiskName()) + "," +
