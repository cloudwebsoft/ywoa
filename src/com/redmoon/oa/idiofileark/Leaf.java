...
public class Leaf implements Serializable,ITagSupport {
    ...
    public static final int TYPE_SENTBOX = 7;
    public static final int TYPE_DELBOX = 8;
    ...
    private String code = "", name = "", description = "", parent_code = "-1",
            root_code = "", add_date = "", user_name = "";
    private int orders = 1, layer = 1, child_count = 0, islocked = 0;
    final String LOAD = "select code,name,description,parent_code,root_code,orders,layer,child_count,add_date,islocked,type,isHome,doc_id,template_id,pluginCode,IS_SYSTEM,target from oa_idiofileark_dir where code=?";
    boolean isHome = false;
    final String dirCache = "IDIOFILEARKDIR";
    ...
    /**
     * 根据用户ID及系统目录类型获取系统目录的编码
     * @param user UserDb
     * @param type int
     * @return String
     */
    public static String getSystemDirCode(UserDb user, int type) {
        if (type == Leaf.TYPE_INBOX) {
            return user.getId() + "_" + Leaf.TYPE_INBOX;
        } else if (type == Leaf.TYPE_OUTBOX) {
            return user.getId() + "_" + Leaf.TYPE_OUTBOX;
        } else if (type == Leaf.TYPE_SENTBOX) {
            return user.getId() + "_" + Leaf.TYPE_SENTBOX;
        } else if (type == Leaf.TYPE_DELBOX) {
            return user.getId() + "_" + Leaf.TYPE_DELBOX;
        }
        return null;
    }
    ...
    public Vector getChildren() {
        Vector v = new Vector();
        String sql = "select code from oa_idiofileark_dir where parent_code=? order by orders asc";
        Conn conn = new Conn(connname);
        ResultSet rs = null;
        try {
    ...
    public synchronized boolean update() {
        ...
        String sql = "update oa_idiofileark_dir set name=" + StrUtil.sqlstr(name) +
        ...
        try {
            ...
            try {
                if (r == 1) {
                    ...
                    sql = "select code from oa_idiofileark_dir where parent_code=" + StrUtil.sqlstr(oldParentCode) +
    ...
    public boolean AddChild(Leaf childleaf) throws
        ...
        String insertsql = "insert into oa_idiofileark_dir (code,name,parent_code,description,orders,root_code,child_count,layer,type,add_date,IS_SYSTEM,isHome,target) values (";
        insertsql += StrUtil.sqlstr(childleaf.getCode()) + "," +
                ...
                "," + SQLFilter.getDateStr(DateUtil.format(new java.util.Date(), "yyyy-MM-dd HH:mm:ss"), "yyyy-MM-dd HH:mm:ss") + "," + intIsSystem + "," + intIsHome + "," + StrUtil.sqlstr(childleaf.getTarget()) + ")";
        ...
        try {
            //更改根结点的信息
            updatesql = "Update oa_idiofileark_dir set child_count=child_count+1" +
                        " where code=" + StrUtil.sqlstr(code);
            conn.beginTrans();
    ...
    public boolean delsingle(Leaf leaf) {
        ...
        try {
            ifd.delByDirCode(leaf.getCode());
        } catch (ErrMsgException e) {
            logger.error("delsingle:" + StrUtil.trace(e));
        }

        RMConn rmconn = new RMConn(connname);
        try {
            String sql = "delete from oa_idiofileark_dir where code=" + StrUtil.sqlstr(leaf.getCode());
            boolean r = rmconn.executeUpdate(sql) == 1 ? true : false;
            sql = "update oa_idiofileark_dir set orders=orders-1 where parent_code=" + StrUtil.sqlstr(leaf.getParentCode()) + " and orders>" + leaf.getOrders();
            rmconn.executeUpdate(sql);
            sql = "update oa_idiofileark_dir set child_count=child_count-1 where code=" + StrUtil.sqlstr(leaf.getParentCode());
            rmconn.executeUpdate(sql);
    ...
    public Leaf getBrother(String direction) {
        ...
        try {
            if (direction.equals("down")) {
                sql = "select code from oa_idiofileark_dir where parent_code=" +
    ...
    public boolean move(String direction) {
        ...
        if (isexist) {
            ...
            try {
                conn.beginTrans();
                if (direction.equals("down")) {
                    sql = "update oa_idiofileark_dir set orders=orders+1" +
                          " where code=" + StrUtil.sqlstr(code);
    ...
    public boolean create() throws ErrMsgException {
        int intIsSystem = isSystem() ? 1 : 0;
        int intIsHome = isHome ? 1 : 0;
        String insertsql = "insert into oa_idiofileark_dir (code,name,parent_code,description,orders,root_code,child_count,layer,type,add_date,IS_SYSTEM,isHome,target) values (";
        insertsql += StrUtil.sqlstr(code) + "," +
                StrUtil.sqlstr(name) +
                "," + StrUtil.sqlstr(parent_code) +
                "," + StrUtil.sqlstr(description) + "," +
                orders + "," + StrUtil.sqlstr(root_code) +
                "," + child_count + "," + layer + "," + type +
                "," + SQLFilter.getDateStr(DateUtil.format(new java.util.Date(), "yyyy-MM-dd HH:mm:ss"), "yyyy-MM-dd HH:mm:ss") +"," + intIsSystem + "," + intIsHome + "," +
                StrUtil.sqlstr(target) + ")";
        ...
        boolean re = false;
        try {
            re = conn.executeUpdate(insertsql) == 1 ? true : false;
    ...
    public String getRootCode(String childCode){
        String sql = "select root_code from oa_idiofileark_dir where code=" + StrUtil.sqlstr(childCode);
        RMConn rmconn = new RMConn(connname);
        ...
        try {
            ...
            if (ri != null && ri.hasNext()) {
                ResultRecord rr = (ResultRecord) ri.next();
                rootCode = rr.getString(1);
            }
        } catch (SQLException e) {
            logger.error("getRootCode: " + e.getMessage());
        }
        return rootCode;
