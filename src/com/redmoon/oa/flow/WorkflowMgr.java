...
import org.apache.poi2.hpsf.WritingNotSupportedException;
...
public class WorkflowMgr {
    ...
    public long initWorkflow(String userName, String typeCode, String flowTitle, long projectId, int level, long parentActionId) throws
        ...
        try {
            ...
            startActionId = wfd.createFromString(flowString);
    ...
    public boolean ReturnAction(HttpServletRequest request, WorkflowDb wf,
                                WorkflowActionDb myAction, long myActionId) throws
            ErrMsgException {
        /*
        ...
        */
        ...
        if (fdm.update(request, true)) {
            ...
            if (re) {
				...
				String script = wpm.getActionReturnScript(wpd.getScripts(),
						wad.getInternalName());
				if (script != null) {
					...
					try {
						...
						// 赋值当前用户
						sb.append("String userName=\"" + privilege.getUser(request)
								+ "\";");
						...
						if (obj != null) {
							...
							if (!ret) {
								...
								LogUtil.getLog(getClass()).error(
										"ReturnAction bsh errMsg=" + errMsg);
							}
						}
					} catch (EvalError e) {
				...
				if (!lf.isDebug()) {
	                ...
	                Config cfg = new Config();
	                boolean flowNotifyByEmail = cfg.getBooleanProperty(
	                        "flowNotifyByEmail");
    ...
    public void setXorRadiateNextBranch(WorkflowActionDb myAction, String XorNextActionInternalNames) throws ErrMsgException {
        if (myAction.isXorRadiate()) {
            ...
            if (XorNextActionInternalNames.equals("")) { // NODE_SELECTED在flow_dispose.jsp表示存在被忽略的节点，可以认为已有后继节点被选中
                throw new ErrMsgException("请选择后续节点");
            } else {
    ...
    public boolean checkLock(HttpServletRequest request, WorkflowDb wf) throws ErrMsgException {
   	...
   	public BSHShell runDeliverScript(HttpServletRequest request, String curUserName, WorkflowDb wf, FormDAO fdao, MyActionDb mad, String script, boolean isTest) {
    ...
    public boolean FinishAction(HttpServletRequest request, WorkflowDb wf, WorkflowActionDb myAction, long myActionId) throws ErrMsgException {
        ...
        /*
        // 流程中的节点是否在flow_dispose.jsp中被修改，如异或修改、选择节点上的用户
        boolean isFlowModified = StrUtil.getNullString(fu.getFieldValue("isFlowModified")).equals("1");
        ...
        */
        ...
        if (isModifyFlow) {
            String flowstring = StrUtil.getNullStr(fu.getFieldValue(
                    "flowstring"));
            // 在处理会签时发现问题，如果使用modifyFlowString，当两个用户同时处理并行的会签节点时，会因为在提交时一前一后，致流程图在解析时，后一用户会把前一用户的状态恢复为正处理的状态
            // if (!wf.modifyFlowString(request, flowstring))
            //    throw new ErrMsgException("修改流程图失败！");
        ...
        */
        ...
        if (mad.getActionStatus() != WorkflowActionDb.STATE_PLUS) {
	        	...
	        	if (ary!=null) {
	        		for (int k=0; k<ary.length; k++) {
	        			if (XorNextActionInternalNames.equals("")) {
	        				XorNextActionInternalNames = ary[k];
	        			}
	        			else {
	        				// 防止重复
	        				String tmpString = "," + XorNextActionInternalNames + ",";
	        				if (tmpString.indexOf("," + ary[k] + ",")==-1) {
	        					XorNextActionInternalNames += "," + ary[k];
	        				}
	        			}
	        		}
			...
			if (XorNextActionInternalNames.indexOf(",")!=-1) {
				WorkflowPredefineDb wpd = new WorkflowPredefineDb();
				wpd = wpd.getDefaultPredefineFlow(wf.getTypeCode());	        
	        ...
	        while (irto.hasNext() && !isRecycle) {
            	...
            	if (wfa.getUserName() != null && !wfa.getUserName().equals("")) {
	            	...
	            	while (it.hasNext()) {
	            		WorkflowActionDb waDb = (WorkflowActionDb) it.next();
	        ...
	        while (irto.hasNext()) {
	            ...
	            if (isUserSelectable) {
    	            ...
    	            if (!wfa.getUserName().equals(uNames)) {
    	                ...
    	                if (// status == WorkflowActionDb.STATE_FINISHED || // 2009-9-29原来流程能够即时编辑时，不允许在下一节点已经完成的情况下，编辑节点上的用户，而现在流程不允许即时编辑，所以该限制可以去除
...
/*    	                if (wfa.getUserName().equals("")) {
    		                wfa.setUserName(uNames);
    		                wfa.setUserRealName(uRealNames);
    	                } else {
    	                	// 判断新选的用户中是否有不存在于wfa中的，如果有，则加入其中
    	                	String uns = "";
    	                	String urs = "";
    	                	for (String u : userNames) {
    	                		if (!wfa.getUserName().contains(u.trim())) {
    	                			uns += (uns.equals("") ? "" : ",") + u.trim();
    	                			user = um.getUserDb(u.trim());
    	                			urs += (urs.equals("") ? "" : ",") + user.getRealName();
    	                		}
    	                	}
    	                	if (!uns.equals("")) {
    	                		wfa.setUserName(wfa.getUserName() + "," + uns);
    			                wfa.setUserRealName(wfa.getUserRealName() + "," + urs);
    	                	}
    	                }	*/
            ...
            */
            ...
            if (!lf.isDebug() && myAction.isMsg()) {
	            ...
	            while (ir.hasNext()) {
	                ...
	                if (flowNotifyByEmail) {
	                    user = um.getUserDb(mad2.getUserName());
	            ...
	            if (wf.getStatus()==WorkflowDb.STATUS_FINISHED) {
	               ...
	               fc = StrUtil.format(fc, new String[] {wf.getTitle(),
	                                   myAction.getUserRealName(),
	                                   DateUtil.format(myAction.getCheckDate(),
	                                                   "yyyy-MM-dd")});
	               ...
	               if (isUseMsg) {
	                ...
	                if (flowNotifyByEmail) {
	                    user = um.getUserDb(wf.getUserName());
	                    if (user.getEmail()!=null && !user.getEmail().equals("")) {
	                        String action = "op=show|userName=" + user.getName() + "|" +
	                                        "flowId=" + wf.getId();
	                        action = cn.js.fan.security.ThreeDesUtil.encrypt2hex(
		...
		if (aNames!=null) {
			for (int i=0; i<aNames.length; i++) {
				...
				if (wa.getStatus()!=WorkflowActionDb.STATE_NOTDO) {
					...
					WorkflowActionDb wad = new WorkflowActionDb();
					try {
						v = wad.matchActionUser(wa, myAction, false);
					} catch (ErrMsgException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
    ...
    public boolean saveFormValue(HttpServletRequest request, WorkflowDb wf, WorkflowActionDb myAction) throws ErrMsgException {
       ...
       if (op.equals("XorCondNodeCommit")) {
        ...
        if (op.equals("saveformvalueBeforeXorCondSelect"))
     ...
     * @throws ErrMsgException
     */
    public boolean discard(HttpServletRequest request, int flowId) throws ErrMsgException {
        WorkflowDb wfd = new WorkflowDb();
        wfd = wfd.getWorkflowDb(flowId);
        ...
        //    throw new ErrMsgException(SkinUtil.LoadString(request, "pvg_invalid"));
        Privilege privilege = new Privilege();
        boolean re = wfd.discard(privilege.getUser(request));
        if (re) {
			FormDAO fdao = new FormDAO();
			FormDb fd = new FormDb();
			Leaf lf = new Leaf();
    ...
    public boolean finishActionSingle(HttpServletRequest request, MyActionDb mad, String userName, StringBuffer sb) throws ErrMsgException {
        ...
        if (lf.getType()==Leaf.TYPE_FREE) {
            ...
            try {
            	wa.changeStatus(request, wf, userName, WorkflowActionDb.STATE_FINISHED, reason, result, resultValue, mad.getId());
            }
            catch(ErrMsgException e) {
            	sb.append( wf.getTitle() + "，" + e.getMessage() + "\r\n" );
