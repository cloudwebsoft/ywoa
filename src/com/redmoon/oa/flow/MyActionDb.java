...
public class MyActionDb extends ObjectDb implements IDesktopUnit {
    ...
    public void returnMyAction() {
        ...
        // 置其它当前正在办理的节点的状态为因返回而忽略，除去被返回的节点
    ...
    public Vector getFlowDoingWithoutAction(long flowId) {
        Vector v = new Vector();
    ...
    public Vector getActionDoing(long actionId) {
        Vector v = new Vector();
     ...
     * @return
     */
    public Vector getMyActionDbOfToActionDoing() {
    	Vector v = new Vector();
		...
		String sql = "select id from " + tableName + " where action_id=? and is_checked=0";
    	try {
        	Vector links = wld.getToWorkflowLinks(wa);
     ...
     * @return MyActionDb
     */
    public Vector getOthersOfActionDoing() {
        Vector v = new Vector();
	...
	public Vector getActionUnchecked() {
    ...
    public String getPageList(HttpServletRequest request, UserDesktopSetupDb uds) {
     ...
     public String display(HttpServletRequest request, UserDesktopSetupDb uds) {
         ...
         /*
         String sql = "select id from flow_my_action where (user_name=" +
                      StrUtil.sqlstr(privilege.getUser(request)) + " or proxy=" +
                      StrUtil.sqlstr(privilege.getUser(request)) +
                      ") and (is_checked=0 or is_checked=" + MyActionDb.CHECK_STATUS_SUSPEND + ") and sub_my_action_id=" + MyActionDb.SUB_MYACTION_ID_NONE + " order by receive_date desc";
         */
    ...
    public MyActionDb getMyActionDb(long id) {
        MyActionDb log = (MyActionDb)getObjectDb(new Long(id));
        return log;
    }
    ...
    public long resume(String checker) {
        ...
        if (re) {
            ...
            Conn conn = new Conn(connname);
            // QUERY_CREATE = "insert into " + tableName + " (action_id, receive_date) values (?,?)";
            try {
                ...
                // 置新的到期时间
                if (expireDate != null)
                    pstmt.setTimestamp(8, new java.sql.Timestamp(expireDate.getTime()));
                else
        ...
        try {
            ...
            if (rs != null) {
                if (rs.next()) {
                    ...
                    checked = checkStatus==1 || checkStatus==CHECK_STATUS_TRANSFER || checkStatus==CHECK_STATUS_RETURN || checkStatus==CHECK_STATUS_HANDOVER || checkStatus==CHECK_STATUS_SUSPEND_OVER || checkStatus==CHECK_STATUS_PASS_BY_RETURN;
    ...
    public synchronized static boolean isNotifyExist(long flowId, String userName, long privMyActionId) {
        ...
        try {
            // LogUtil.getLog(MyActionDb.class).info(sql + " " + userName + " flowId=" + flowId + " " + privMyActionId + " );
            JdbcTemplate jt = new JdbcTemplate();
            ResultIterator ri = jt.executeQuery(sql, new Object[]{new Long(flowId), userName, new Long(privMyActionId), new Integer(CHECK_STATUS_NOT)});
            if (ri.hasNext()) {
                return true;
            }
        } catch (SQLException ex) {
     ...
     */
    public synchronized static int isActionExist(long actionId, String userName) {
    	String sql = "select id from flow_my_action where action_id=? and user_name=? and is_checked=?";
        try {
            // LogUtil.getLog(MyActionDb.class).info(sql + " " + userName + " flowId=" + flowId + " " + privMyActionId + " );
            JdbcTemplate jt = new JdbcTemplate();
            ResultIterator ri = jt.executeQuery(sql, new Object[]{new Long(actionId), userName, new Integer(CHECK_STATUS_WAITING_TO_DO)});
            if (ri.hasNext()) {
            	ResultRecord rr = (ResultRecord) ri.next();
                return rr.getInt(1);
            }
