...
public class SQLCtl extends AbstractMacroCtl {
			...
			if (flowId==-1) {
				...
				if (request.getRequestURL().indexOf("_relate")!=-1) {
					...
					if (sqlResult.equals(FormUtil.ERROR_PARSE)) {
						...
						sql = parseAndSetFieldValue(request, fd, sql);
					}
					else {
						sql = sqlResult;
					}
	...
	public static String parseAndSetFieldValue(HttpServletRequest request,
	...
	public static ResultIterator getResultByDAO(IFormDAO fdao, FormField ff) {
		String[] ary;
		String desc = StrUtil.getNullStr(ff.getDescription());
		if ("".equals(desc)) {
			ary = getSql(ff.getDefaultValue());
	...
	public static String getCtlHtml(HttpServletRequest request, int flowId, FormField ff) {
		...
		if (ri.size() == 0) {
			return "<span id='" + ff.getName() + "_box'><input id='" + ff.getName() + "' name='" + ff.getName() + "' type='hidden' /></span>";
		}
		...
		if (ri.size() == 1) {
			...
			if (rr.getRow().size() == 1) { // 只有一列
				...
				return "<span id='" + ff.getName() + "_box' style='display:" + (isHidden?"none":"") + "'><input id='" + ff.getName() + "' name='" + ff.getName()
					+ "' value='" + StrUtil.getNullStr(rr.getString(1)) + "' " + readonly + " /></span>";
			} else {
				ri.beforeFirst();
			}
		}
	...
	public String getDisableCtlScript(FormField ff, String formElementId) {
		...
		// 如：奔讯的日常报销中的月度剩余预算SQL控件，在发起节点后需显示于嵌套表的列表中，而在嵌套表格2不能添加和编辑时，开始节点隐藏，需要记录其值
