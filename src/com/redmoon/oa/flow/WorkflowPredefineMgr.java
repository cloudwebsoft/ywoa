...
public class WorkflowPredefineMgr {
	...
	public boolean modify(HttpServletRequest request) throws ErrMsgException, Exception{
		...
		else if (flowString.indexOf("version:")==-1) {
			// version:1, 3, 0, 0;
	...
	public boolean modifyFree(HttpServletRequest request)
		...
		for (int i = 0; i < len; i++) {
			...
			String delAttach = ParamUtil.get(request, roleCodes[i]
					+ "_delAttach");
			flowString += start + "," + stop + "," + archive + "," + discard
					+ "," + del + "," + delAttach;
			// 将可写表单域的分隔符用|替换,
			String fieldWrite = ParamUtil.get(request, roleCodes[i]
					+ "_fieldWrite");
		...
		String starter_fieldWrite = ParamUtil
				.get(request, "starter_fieldWrite");

		flowString += "[starter]" + starterRoleCodes + ":" + starter_fieldWrite
				+ "[/starter]";
