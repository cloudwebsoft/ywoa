...
public class FormSQLBuilder {
    ...
    public FormSQLBuilder(com.redmoon.oa.visual.FormDAO moduleFdao, JSONObject jsonTabSetup) {
     ...
     public static String getArchiveTable(){
        String sql = "select table_code from FORM_TABLE order by table_code desc";
        return sql;
     }

     public static String getArchiveTableField(String table_short_code){
        String sql = "select id from FORM_FIELD where table_short_code = " + StrUtil.sqlstr(table_short_code) + " order by id";
        return sql;
     }

     public static String getArchiveTableField(String table_short_code,String field_code){
        String sql = "select id from FORM_FIELD where table_short_code = " + StrUtil.sqlstr(table_short_code) + " and field_code = " + StrUtil.sqlstr(field_code);
        return sql;
     }

     public static String getFormQuery() {
         String sql = "select id from form_query order by id desc";
         return sql;
     }
     ...
     public static String getFormQueryCondition(int query_id) {
         String sql = "select id from FORM_QUERY_CONDITION where query_id = " + query_id + " order by condition_field_code";
         return sql;
     }

     public static String getUserAssess(String userName) {
         String sql = "select id from FORM_ASSESS where username = " +
                      StrUtil.sqlstr(userName);
         return sql;
     }

     public static String getUserProfession(String userName) {
         String sql = "select id from FORM_PROFESSION where username = " +
                      StrUtil.sqlstr(userName);
         return sql;
     }

     public static String getUserRewards(String userName) {
         String sql = "select id from FORM_REWARDS where username = " +
                      StrUtil.sqlstr(userName);
         return sql;
     }

     public static String getUserDuty(String userName) {
         String sql = "select id from FORM_DUTY where username = " +
                      StrUtil.sqlstr(userName) + " order by mydate desc";
         return sql;
     }

     public static String getQueryPrivilege(int queryId, int type) {
         String sql = "select id from FORM_QUERY_PRIVILEGE where query_id = " +
     ...
     public static String getUserPrivilege(String userName,String deptCode) {
         String sql = "select id from FORM_PRIVILEGE where userName = " +
      ...
      * 根据条件Vector，组合出条件
      * @param condV Vector
      * @param chartFieldCode String 饼图或柱状图中需用到的字段编码
     ...
     public String getConditions(FormQueryDb fqd, Vector condV, FormField chartFormField, String chartFieldValue) {
         ...
         while (ir.hasNext()) {
             ...
             */
             ...
             // 如果like的值为空
             if (aqcd.getConditionSign().equals("like")) {
                 if (!val.equals("'%%'") && !val.equals("''")) {
                     conditions += aqcd.getConditionFieldCode() + " " +
                             aqcd.getConditionSign() + " " + val;
                 } else {
                     /*
                     System.out.println(FormSQLBuilder.class +
                                        " getConditionSign=" +
                     ...
                     */
                     ...
                     if (p == conditions.length() - " and ".length()) {
                         conditions = conditions.substring(0, p);
                     }
                 }
             } else {
                 if (aqcd.getConditionType().equals("SELEDATE")) {
                     if (chartFormField!=null && chartFormField.getName().equalsIgnoreCase(aqcd.getConditionFieldCode())) {
                         conditions += aqcd.getConditionFieldCode() + " " +
                                 aqcd.getConditionSign() + " " +
                                 SQLFilter.getDateStr(chartFieldValue, "yyyy-MM-dd");
                     }
                     else {
                         if (!aqcd.getInputValue().equals("")) {
                             conditions += aqcd.getConditionFieldCode() + " " +
                                     aqcd.getConditionSign() + " " +
                                     val;
                         } else {
                     ...
                     if (chartFormField!=null && chartFormField.getName().equalsIgnoreCase(aqcd.getConditionFieldCode())) {
                         conditions += aqcd.getConditionFieldCode() + " " +
                                 aqcd.getConditionSign() + " " +
                                 StrUtil.sqlstr(chartFieldValue); // @task:对于不同类型的字段，应分别处理，不应都加sqlstr
                     }
                     else {
                         if (!val.equals("''")) {
                             conditions += aqcd.getConditionFieldCode() + " " +
	...
	public static String getConditionsWithModuleFormValueXXX(Vector condV, com.redmoon.oa.visual.FormDAO fdao,
		...
		while (ir.hasNext()) {
			...
			if (aqcd.getConditionSign().equals("like")) {
				conditions += aqcd.getConditionFieldCode() + " "
						+ aqcd.getConditionSign() + " " + StrUtil.sqlstr(val);
			} else {
				if (aqcd.getConditionType().equals("SELEDATE")) {
					...
					conditions += aqcd.getConditionFieldCode() + " "
							+ aqcd.getConditionSign() + " "
							+ SQLFilter.getDateStr(val, formatStr);
				} else {
					// 如果为选择型，且比较模式为不等于，则置sign为<>
					if (aqcd.getConditionType().equals("SELECTED")) {
						if (aqcd.getCompareType() == FormQueryConditionDb.COMPARE_TYPE_NOT_EQUALS) {
	...
	public String getCondValue(String condField, FormDb fd, Map mapConds) {
		...
		String queryField = ""; // 查询条件
		String field = "";
		Iterator ir = mapConds.keySet().iterator();
		while (ir.hasNext()) {
			field = (String) ir.next();
      ...
      * @param fieldCodeTypeSelectUsedByChart String 饼图或柱状图中用到的selectd类型的字段，该字段如果不为空，则只能在条件Map中保留一个，以便于赋值，并组合成SQL语句数组
      * @return Map
      */
     ...
     public String getQueryRelated(int queryId) {
         ...
         String sql = getSmartQuerySQL(aqd, condV, null, "", "", "");
         return sql;
     }
     ...
     public String getSmaryQueryWithoutOrderBy(HttpServletRequest request, int queryId) throws ErrMsgException {
         ...
         while (ir.hasNext()) {
             ...
             if (aqcd.getCompareType() == FormQueryConditionDb.COMPARE_TYPE_UNDER) {
                 String deptStr = StrUtil.sqlstr(inputValue);
                 ...
                 Iterator irdd = vt.iterator();
                 while (irdd.hasNext()) {
                     dd = (DeptDb) irdd.next();
                     deptStr += "," + StrUtil.sqlstr(dd.getCode());
                 }
