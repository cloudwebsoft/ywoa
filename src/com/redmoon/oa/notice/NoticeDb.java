...
public class NoticeDb extends ObjectDb {
	...
	public boolean isUserReaded(String userName) {
		String sql = "select id from oa_notice_reply where notice_id="
				+ id + " and user_name=" + StrUtil.sqlstr(userName)
				+ " and is_readed=1";
		JdbcTemplate jt = new JdbcTemplate();
	 ...
	 * @return
	 */
	public Vector listImportant(String userName) {
		String curDay = DateUtil.format(new java.util.Date(), "yyyy-MM-dd");		
		String sql = "select id from oa_notice where notice_level=1 and begin_date<=" + SQLFilter.getDateStr(curDay, "yyyy-MM-dd") + " and (end_date is null or end_date>=" + SQLFilter.getDateStr(curDay, "yyyy-MM-dd") + ") ";
		sql += " and (id in (select notice_id from oa_notice_reply where user_name = " + StrUtil.sqlstr(userName) + "))";
