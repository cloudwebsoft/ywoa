...
public class MessageDb extends ObjectDb implements IMessage, IDesktopUnit {
    ...
    public int getNewMsgCount(String receiver) {
        ...
        String sql =
                "select count(*) from oa_message where isreaded=0 and box=" + MessageDb.INBOX + " and is_sent=1 and receiver=? and is_dustbin=0";
        return mc.getNewMsgCount(receiver, sql);
    }
...
/*                if (id == 0) { // 之前如果未create
                ...
                sendMsgTigase(UserDb.SYSTEM, ary[i], title);
            }
        }
        return true;
    }
    ...
    public boolean sendMsgBysender(String receiver, File[] files, String[] uploadFileNames)
        ...
        if (isRTXUsed) {
            ...
            int delayTime = rc.getIntProperty("notifyDelay") * 1000;
            re = RTXUtil.sendNotify(this.receiver, title, content, "" + delayTime);
            boolean sysMsgOnlyUseRTX = rc
                    .getBooleanProperty("sysMsgOnlyUseRTX");
            if (sysMsgOnlyUseRTX)
                return re;
        }
...
/*                SendNotice se = new SendNotice();
    ...
    public boolean sendSysMsg(String receiver, String title, String content) throws ErrMsgException {
        ...
        if (isRTXUsed) {
            rtx.RTXConfig rc = rtx.RTXConfig.getInstance();
            int delayTime = rc.getIntProperty("notifyDelay") * 1000;
            re = RTXUtil.sendNotify(receiver, title, content, "" + delayTime);
            boolean sysMsgOnlyUseRTX = rc.getBooleanProperty("sysMsgOnlyUseRTX");
            if (sysMsgOnlyUseRTX)
                return re;
        }
        ...
        try {
            ...
            if (re) {
                ...
                sendMsgTigase(UserDb.SYSTEM, receiver, title);
            }
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("sendSysMsg: " + e.getMessage());
            // 注释掉，因为也可有是微信启用了，但是在本机测试
            // throw new ErrMsgException("数据库操作错误！");
        } finally {
            if (conn != null) {
                conn.close();
    ...
    private void sendMsgTigase(String sender, String receiver, String title) {
        ...
        if (tigaseCfg.getBooleanProperty("isUse")) {
            ...
            String skey = receiver + "|" + "OA" + "|"
                    + date.getTime();
            skey = ThreeDesUtil.encrypt2hex(cloudConfig.getProperty("key"),
                    "op=sysmsg&skey=" + skey + "&id=" + id);
            JSONObject json = new JSONObject();
            json.put("content", title + Global.getFullRootPath()
                    + "/public/tigase_login.jsp?skey=" + skey);
            json.put("fromUserName", sender);
            json.put("timeSend", date.getTime());
            json.put("type", 1);
            TigaseConnection tc = new TigaseConnection();
            tc.sendMsg(sender, receiver, json.toString());
            try {
                Thread.sleep(1);
    ...
    public int sendSysMsgNotice(long noticeId, String[] users, String title, String content) throws ErrMsgException {
        ...
        if (isRTXUsed) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                RTXUtil.sendNotify(receiver, title, content, "" + delayTime);
            }
            boolean sysMsgOnlyUseRTX = rc.getBooleanProperty("sysMsgOnlyUseRTX");
            if (sysMsgOnlyUseRTX)
                return 0;
        ...
        try {
            for (int i = 0; i < len; i++) {
                ...
                sql += SQLFilter.getDateStr(DateUtil.format(curDate, "yyyy-MM-dd HH:mm:ss"), "yyyy-MM-dd HH:mm:ss") + ",";
                sql += 0 + "," + 0 + "," + StrUtil.sqlstr("noticeId=" + noticeId) + ",";
    ...
    public String renderAction(HttpServletRequest request) {
        ...
        if (myaction.equals(ACTION_FLOW_DISPOSE)) {
            ...
            if (wf == null || wf.getDocId() == -1) {
                ...
                wpd = wpd.getPredefineFlowOfFree(wf.getTypeCode());
                if (wpd.isLight()) {
                    return "<a href='javascript:;' onclick=\"addTab('" + StrUtil.toHtml(wf.getTitle()) + "', '" + request.getContextPath() + "/flow_dispose_light_show.jsp?flowId=" + strFlowId + "')\">点击查看流程处理过程</a>";
                } else {
                    return "<a href='javascript:;' onclick=\"addTab('" + StrUtil.toHtml(wf.getTitle()) + "', '" + request.getContextPath() + "/flow_modify.jsp?flowId=" + strFlowId + "')\">点击查看流程处理过程</a>";
                }
            }
            ...
            if (!strId.equals("")) {
                NoticeDb nd = new NoticeDb(StrUtil.toInt(strId));
                if (nd == null || !nd.isLoaded()) {
                    return "<span style='color:red'>当前通知公告已删除</span>";
                } else {
                    return "<a href='javascript:;' onclick=\"addTab('" + nd.getTitle() + "', '" + request.getContextPath() + "/notice/notice_detail.jsp?id=" + strId + "&isShow=1')\">点击查看</a>";
                }
            }
        }
        return "";
    ...
    public String getSenderPortrait() {
        ...
        user = user.getUserDb(sender);
        return UserSetupMgr.getPortrait(user);
    }
