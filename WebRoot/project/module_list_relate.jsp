...
while (ir.hasNext()) {
	...
	if (code.equals(formCodeRelated)) {
		if (mrd.getInt("relate_type") == ModuleRelateDb.TYPE_SINGLE) {
			...
			if (fdao!=null) {
				long id = fdao.getId();			
				//response.sendRedirect("../visual/module_show_relate.jsp?id=" + id + "&parentId=" + parentId + "&formCodeRelated=" + formCodeRelated + "&formCode=" + formCode);
				return;
			}
		}
	}
...
querystr = "op=" + op + "&projectId=" + projectId + "&menuItem=" + menuItem + "&formCode=" + formCode + "&formCodeRelated=" + formCodeRelated + "&parentId=" + parentId + "&orderBy=" + orderBy + "&sort=" + sort + "&isShowNav=" + isShowNav;
if (!sqlUrlStr.equals(""))
	querystr += "&" + sqlUrlStr;
	...
	try {
		if (fdm.del(request)) {
			...
			if (!privurl.equals(""))
				...
				out.print(StrUtil.Alert_Redirect("删除成功！", "module_list_relate.jsp?" + querystr + "&CPages=" + curpage + "&isShowNav=" + isShowNav));
			return;
		}
	}
	catch (ErrMsgException e) {
...
<table class="percent98" width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="48%" height="28" align="left"><input name="button" class="btn" type="button" onclick="window.location.href='module_add_relate.jsp?projectId=<%=projectId%>&parentId=<%=parentId%>&menuItem=<%=menuItem%>&formCode=<%=formCode%>&formCodeRelated=<%=formCodeRelated%>&isShowNav=<%=isShowNav%>'" value="添加" />
    &nbsp;&nbsp;
    <input name="button3" class="btn" type="button" onclick="window.location.href='module_search_relate.jsp?parentId=<%=parentId%>&amp;menuItem=<%=menuItem%>&amp;formCode=<%=formCode%>&amp;formCodeRelated=<%=formCodeRelated%>&isShowNav=<%=isShowNav%>&menuItem=<%=menuItem %>'" value="查询" />
    &nbsp;&nbsp;
    <input name="button4" type="button" class="btn" onclick="javascript:window.open('<%=request.getContextPath()%>/visual/module_excel_relate.jsp?<%=querystr%>')" value="导出至Excel" />
    </td>
...
<%
	...
	%>	
		...
		<%}%>		
		<!-- ../visual/module_show_relate.jsp?parentId=<%=parentId%>&id=<%=id%>&formCodeRelated=<%=formCodeRelated%>&formCode=<%=formCode%>&isShowNav=<%=isShowNav%> -->
		<span>
		<%if (!fieldName.equals("cws_creator")) {
			if (fieldName.startsWith("main")) {
				String[] ary = StrUtil.split(fieldName, ":");
	...
	<td>
		...
		<a href="module_edit_relate.jsp?parentId=<%=parentId%>&id=<%=id%>&menuItem=<%=menuItem%>&formCodeRelated=<%=formCodeRelated%>&formCode=<%=formCode%>&isShowNav=<%=isShowNav%>">编辑</a>&nbsp;&nbsp;&nbsp;&nbsp;<a onclick="if (confirm('您确定要删除么？')) window.location.href='module_list_relate.jsp?action=del&projectId=<%=projectId%>&op=<%=op%>&id=<%=id%>&formCode=<%=formCode%>&formCodeRelated=<%=formCodeRelated%>&menuItem=<%=menuItem%>&parentId=<%=parentId%>&CPages=<%=curpage%>&orderBy=<%=orderBy%>&sort=<%=sort%>&<%=sqlUrlStr%>&isShowNav=<%=isShowNav%>'" href='javascript:;'>删除</a>
		<%}%></td>
  </tr>
  <%
  }
