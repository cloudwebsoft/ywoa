...
function cws_validateMode()
{
	...
	alert('<lt:Label res="res.label.editor_full.editor" key="info_mode_submit"/>');
	IframeID.focus();
	return false;
}
...
{	
	...
	content = content.replace(RegExp, '<lt:Label res="res.label.editor_full.editor" key="info_script"/>');
	RegExp = /<P>&nbsp;<\/P>/gi;
...
function experienceCanSee() {
	...
	var fontbegin = "[HIDE_EXP="+enterpoints+"]";
	var fontend = "[/HIDE_EXP]";
	if ((IframeID.document.selection)&&(IframeID.document.selection.type == "Text")) {
		var range = IframeID.document.selection.createRange();
		var ch_text = range.text;
		range.text = fontbegin + ch_text + fontend;
