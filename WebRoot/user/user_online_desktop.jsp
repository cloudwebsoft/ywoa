...
<div id="drag_<%=id%>" dragTitle="<%=udsd.getTitle()%>" count="<%=udsd.getCount()%>" wordCount="<%=udsd.getWordCount()%>" class="portlet drag_div bor" >
 <div id="drag_<%=id%>_h" class="box">
    	...
    	<div class="titleimg">
        <!--<img src="images/desktop/online_rank.png" width="40" height="40" />-->
        <i class="fa <%=udsd.getIcon()%>"></i>
        &nbsp;&nbsp;</div>
        <div class="titletxt">&nbsp;&nbsp;<a href="user/user_online_list.jsp"><%=udsd.getTitle()%></a></div>
    </div>
	...
	%>
        <li style="float:left;width:100%;height:32px;line-height:32px;">
        	<span style="display:block;float:left;width:70%">
        		...
        		<a title="<%=NumberUtil.round(user.getOnlineTime(), 1)%>小时" target="_blank" href="user_info.jsp?userName=<%=StrUtil.UrlEncode(user.getName())%>"><%=user.getRealName()%></a>&nbsp;&nbsp;
        	</span>
