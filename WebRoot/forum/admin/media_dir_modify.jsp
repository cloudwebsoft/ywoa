...
MediaDirDb leaf = dir.getMediaDirDb(code);
name = leaf.getName();
...
String parent_name = "";
MediaDirDb pisdd = dir.getMediaDirDb(parent_code);
if (pisdd!=null)
      ...
      <TD align="center" style="PADDING-LEFT: 10px"><table class="frame_gray" width="48%" border="0" cellpadding="0" cellspacing="1">
        <tr>
          <td align="center"><table width="100%">
            <form name="form1" method="post" action="media_left.jsp?op=modify" target="leftFileFrame" onClick="return form1_onsubmit()">
              <tr>
...
<select name="parentCode">
  <%
									MediaDirDb rootlf = leaf.getMediaDirDb(MediaDirDb.ROOTCODE);
									MediaDirView dv = new MediaDirView(rootlf);
...
function del() {
	if (confirm("您确定要删除吗？目录下的所有文件也将会一起被删除！")) {
		window.parent.leftFileFrame.location.href = "media_left.jsp?op=del&code=" + "<%=StrUtil.UrlEncode(code)%>";
	}
}

function move(fx) {
	window.parent.leftFileFrame.location.href = "media_left.jsp?op=move&direction=" + fx + "&code=" + "<%=StrUtil.UrlEncode(code)%>";
}
</script>
</html>
