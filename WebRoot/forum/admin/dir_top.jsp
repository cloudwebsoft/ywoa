...
<table width='100%' cellpadding='0' cellspacing='0' >
  <tr>
    <td class="head"><lt:Label res="res.label.forum.admin.dir_bottom" key="manage"/>&nbsp;<%=root_name%></td>
  </tr>
</table>
<br>
...
border=0>
          <tbody>
            ...
            <td width="34%" align=right nowrap>>>&nbsp;<a href="dir_top.jsp?op=repair&root_code=<%=root_code%>"><lt:Label res="res.label.forum.admin.dir_bottom" key="repair"/></a>&nbsp;&nbsp;<a target="_parent" href="manager_m.jsp?boardcode=<%=root_code%>"><lt:Label res="res.label.forum.admin.dir_bottom" key="boardmanager_set"/></a>&nbsp;&nbsp;<%=root_name%>&nbsp;&nbsp;<a target="_parent" href="document_list_m.jsp?dir_code=<%=StrUtil.UrlEncode(root_code)%>&dir_name=<%=StrUtil.UrlEncode(root_name)%>"></a><a target=dirbottomFrame href="dir_bottom.jsp?parent_code=<%=StrUtil.UrlEncode(root_code, "utf-8")%>&parent_name=<%=StrUtil.UrlEncode(root_name, "utf-8")%>&op=AddChild"><lt:Label res="res.label.forum.admin.dir_bottom" key="add_dir"/></a>
			<%if (!root_code.equals(Leaf.CODE_ROOT)) {%>
				<a target="dirbottomFrame" href="dir_bottom.jsp?op=modify&code=<%=StrUtil.UrlEncode(root_code, "utf-8")%>&name=<%=StrUtil.UrlEncode(root_name,"utf-8")%>&description=<%=StrUtil.UrlEncode(root_description,"utf-8")%>"><lt:Label key="op_edit"/></a>
		    <%}%>
