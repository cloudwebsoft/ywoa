...
<jsp:useBean id="privilege" scope="page" class="com.redmoon.oa.pvg.Privilege"/><%
	...
	if (v.size()==0) {
		response.sendRedirect("pop3_setup.jsp");
		return;
	}
	...
	String sql = "select distinct a.id,a.sender,a.subject,a.mydate,a.is_readed,a.email_addr from email a left join email_attach b on a.id = b.emailId where  a.email_addr=" + StrUtil.sqlstr(email) + " and a.msg_type=" + MailMsgDb.TYPE_INBOX;
	...
	ResultIterator ir = jt.executeQuery(sql, Integer.parseInt(strcurpage), pagesize);
	...
	if(op.equals("updateTotal")){
		...
		String update_sql = "select distinct a.id,a.sender,a.subject,a.mydate,a.is_readed,a.email_addr from email a left join email_attach b on a.id = b.emailId where  a.email_addr=" + StrUtil.sqlstr(email) + " and a.msg_type=" + MailMsgDb.TYPE_INBOX;
...
<script>
	...
	function doGetMailStatus(response){
		...
		for (var i=0; i<items.length; i++) {
			var item = items[i];
			var count = item.getElementsByTagName("count")[0].firstChild.data;
			var storedCount = item.getElementsByTagName("storedCount")[0].firstChild.data;
			...
			var over = item.getElementsByTagName("over")[0].firstChild.data;
			var alertTitle = storedCount + "/" + count;
			//$.toaster({priority : 'info', message : '加载中...' });
			
			showPrompt("images/loading.gif", "邮件更新中...", alertTitle, 0);
			if (over=="true") {
				$.toaster({priority : 'info', message : "加载完成" });
	...
	function getMailStatus() {
		...
		var myAjax = new cwAjax.Request( 
			"pop3_getmail_ajax_status.jsp", 
			{ 
				method:"post",
				parameters:str,
				onComplete:doGetMailStatus,
				onError:errFunc
			}
		);
		timeoutid = window.setTimeout("getMailStatus()", "1000");
	}
	
	function doGetMail(response) {
	...
	function getMail() {
		...
		var myAjax = new cwAjax.Request( 
			...
			{ 
				method:"post",
				parameters:str,
				onComplete:doGetMail,
				onError:errFunc
			}
		);
	}
	
	function doGetInBox(response){
		var rsp = response.responseText.trim();
		$("#inBox").html(rsp);
	...
	function getInBox() {
		...
		var myAjax = new cwAjax.Request( 
			...
			{ 
				method:"post",
				parameters:str,
				onComplete:doGetInBox,
				onError:errFunc
			}
		);
	}
	function doGetInBox2(response){
		var rsp = response.responseText.trim();
		$("#inBox").html(rsp);
	}
	
	function getInBox2() {
		var str = "email=<%=email%>&CPages=<%=strcurpage%>&pageSize=<%=pagesize%>";
		var myAjax = new cwAjax.Request( 
			"in_box_ajax2.jsp", 
			{ 
				method:"post",
				parameters:str,
				onComplete:doGetInBox2,
				onError:errFunc
			}
		);
	}
	...
	function windowOnload() {
		setTimeout(function(){windowOnload();}, 300000);
		initDiv();
		getMailStatus();
		getMail();
		getInBox2();
	...
	function move(box) {
		...
		var str = "op=move&id=<%=id%>&box=" + box + "&ids=" + ids+"&sender=<%=epd.getEmail()%>";
		var myAjax = new cwAjax.Request( 
			"mail_move_ajax.jsp", 
			{ 
				method:"post",
  ...
  <div class="inbox-wrap">
   <div class="inbox-right">
     <div class="inbox-toolbar"> 
	     ...
	     <div class="inbox-right-btnbox" onclick="windowOnload()" title="刷新"><img src="images/inbox-refresh.png" width="20" height="20"/></div>
	...
	<script>
		$(function(){
		    ...
		    e = e || window.event; 
		    var dom =  e.srcElement|| e.target;
	    	if(dom.parentNode.id != "searchDiv" && dom.id != "searchDiv"){
...
<form id="formSearch" action="in_box.jsp" method="get">
...
function changeSort(sortname, sortorder) {
	window.location.href = "in_box.jsp?id=<%=id%>&pageSize=" + flex.getOptions().rp + "&orderBy=" + sortname + "&sort=" + sortorder;
}

function changePage(newp) {
	if (newp)
		window.location.href = "in_box.jsp?id=<%=id%>&CPages=" + newp + "&pageSize=" + flex.getOptions().rp + "&orderBy=<%=orderBy%>" + "&sort=<%=sort%>" + "&subMenu=<%=subMenu%>" + "&subMenuButton=<%=subMenuButton%>";
}

function rpChange(pageSize) {
	window.location.href = "in_box.jsp?id=<%=id%>&CPages=<%=curpage%>&pageSize=" + pageSize + "&orderBy=<%=orderBy%>" + "&sort=<%=sort%>" + "&subMenu=<%=subMenu%>" + "&subMenuButton=<%=subMenuButton%>";
}
...
function del(){
	...
	jConfirm("您确定要删除么？","提示",function(r){
		if(!r){return}
			...
			var str = "email=<%=email%>&ids=" + ids + "&op=del&isDustbin=true";
			var myAjax = new cwAjax.Request( 
				"mail_del_ajax.jsp", 
				{ 
					method:"post",
