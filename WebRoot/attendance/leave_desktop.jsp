...
<div id="drag_<%=id%>" dragTitle="<%=udsd.getTitle()%>" count="<%=udsd.getCount()%>" wordCount="<%=udsd.getWordCount()%>" class="portlet drag_div bor" >
    <div id="drag_<%=id%>_h" class="box">
    	...
    	<div class="titleimg">
        <!--<img src="images/desktop/leave.png" width="40" height="40" />-->
        <i class="fa <%=udsd.getIcon()%>"></i>
        &nbsp;&nbsp;</div>
        <div class="titletxt">&nbsp;&nbsp;<a href="attendance/leave_list.jsp"><%=udsd.getTitle()%></a></div>
    </div>
<div id="drag_<%=udsd.getId()%>_c" class="portlet_content">
<%
...
String sql = "select f.flowId from form_table_qjsqd f, flow fl where f.flowId=fl.id and " + now + ">=qjkssj" + " and " + now + "<=qjjssj and xjrq is null and f.flowTypeCode='qj' and f.result='1' and fl.status=" + WorkflowDb.STATUS_STARTED + " order by fl.mydate desc";
// out.print(sql);
...
while (ir.hasNext()) {
	...
	%>
    <li><%=um.getUserDb(wfd.getUserName()).getRealName()%>&nbsp;&nbsp;&nbsp;请假时间：<%=strBeginDate%>&nbsp;-&nbsp;<%=strEndDate%></li>
<%}%>
</ul>
<%}else{%>
