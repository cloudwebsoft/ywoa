...
String sql = "select f.flowId from form_table_qjsqd f, flow fl , users u  where f.flowId=fl.id and f.flowTypeCode='qj' and (fl.status=" + WorkflowDb.STATUS_STARTED + " or fl.status=" + WorkflowDb.STATUS_FINISHED + ")" + " and u.name = f.applier";
...
response.setHeader("Content-disposition","attachment; filename="+StrUtil.GBToUnicode("请假记录.xls"));  

OutputStream os = response.getOutputStream();

try {
	File file = new File(Global.getAppPath(request) + "attendance/leave_list_excel.xls");
	...
	while (ir.hasNext()) {
		...
		if (checker!=null&&!"".equals(checker))	
		...
		Label a2 = new Label(2, j, fdao.getFieldValue("jqlb"));
		Label a3 = new Label(3, j, fdao.getFieldValue("ts"));
		
		Label a4 = new Label(4, j, strBeginDate);
		Label a5 = new Label(5, j, strEndDate);
		Label a6 = new Label(6, j, xjrq);
		Label a7 = new Label(7, j, DateUtil.format(wfd.getMydate(), "yy-MM-dd HH:mm:ss"));
		Label a8 = new Label(8, j, fdao.getFieldValue("result").equals("1")?"通过":"不通过");
		Label a9 = new Label(9, j, wfd.getStatusDesc());
		Label a10 = new Label(10, j, checker);
