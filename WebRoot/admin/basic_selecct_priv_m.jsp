...
if (op.equals("add")) {
	...
	if (re) {
		out.print(StrUtil.jAlert_Redirect("添加成功！","提示", "flow_dir_priv_m.jsp?dirCode=" + StrUtil.UrlEncode(dirCode)));
	} else {
...
-->
<%if (!isAll.equals("y")) {%>
    &nbsp;&nbsp;<input name="button"  class="btn" type="button" onclick="javascript:location.href='flow_dir_priv_add.jsp?dirCode=<%=StrUtil.UrlEncode(leafPriv.getDirCode())%>';" value="添加" />
<%}%>
</form>
    </td>
  </tr>
...
while (ir.hasNext()) {
  ...
  <form id="form<%=i%>" name="form<%=i%>" action="?op=modify" method=post>
    <tr class="highlight">
      ...
      <td align="center" style="PADDING-LEFT: 10px"><input name="see" type="checkbox" <%=lp.getSee()==1?"checked":""%> value="1" />
      ...
      <td align="center" style="PADDING-LEFT: 10px"><input name="examine" type="checkbox" <%=lp.getExamine()==1?"checked":""%> value="1" /></td>
      <td><%=lp.getTypeDesc()%></td>
      <td><a href="flow_dir_priv_m.jsp?dirCode=<%=StrUtil.UrlEncode(lp.getDirCode())%>"><%=dirName%></a></td>
      <td align="center">
	  <input class="btn" type=submit value="修改">
&nbsp;<input class="btn" type=button onClick="jConfirm('您确定要删除吗?','提示',function(r){ if(!r){return;}else{window.location.href='flow_dir_priv_m.jsp?op=del&isAll=<%=isAll%>&dirCode=<%=StrUtil.UrlEncode(curCode)%>&id=<%=lp.getId()%>'}}) " style="cursor:pointer" value=删除></td>
    </tr></form>
<%}%>
  <tr>
  </tbody>
