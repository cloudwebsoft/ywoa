...
while (ir.hasNext()) {
    ...
    <tr id="tr_<%=wfd.getId()%>" class="highlight">
      ...
      <td><a href="javascript:;" onClick="addTab('<%=wfd.getTitle()%>', '<%=request.getContextPath()%>/flow_modify.jsp?flowId=<%=wfd.getId()%>')"><%=wfd.getTitle()%></a></td>
      <td>
	  <%
      ...
      <td align="center"><%=DateUtil.format(wfd.getBeginDate(), "yy-MM-dd HH:mm:ss")%> </td>
      <td><%=userRealName%></td>
      <td align="center" id="tdStatus<%=wfd.getId()%>"><%=wfd.getStatusDesc()%></td>
      <td align="center">
      <%
      ...
      %>
      <a href="javascript:;" onClick="addTab('<%=wfd.getTitle()%>', '<%=taburl%>')">查看</a>
		&nbsp;&nbsp;  <a href="javascript:;" onClick="addTab('<%=wfd.getTitle()%>', '<%=modifyUrl%>')">修改</a>
      <!--
      <%if (lp.canUserExamine(privilege.getUser(request))) {%>
      	&nbsp;&nbsp;
