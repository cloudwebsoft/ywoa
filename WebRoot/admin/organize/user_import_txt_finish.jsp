...
if (info!=null){
	...
	for(int i=0;i<info.length;i++){
		...
		if (realName==null || "".equals(realName)) {
			System.out.println("用户不存在：accountName=" + accountName + " dept=" + dept + " mobile=" + mobile);
			continue;
		}
		...
		//更改为支持多部门导入
		String depts_[] = StrUtil.split(dept, ",");
		int deptsLen = 0;
		if (depts_!=null) {
			deptsLen = depts_.length;
		}
		//存放多个deptcode
		String deptCodes = "";
		for(int ii = 0; ii<deptsLen; ii ++){
		...
		ad = ad.getAccountDb(accountName);
		if (ad.isLoaded()) {
			name = ad.getUserName();
		...
		boolean flag = false;
		System.out.println("import:" + i + "、" + accountName + "-" + name + "-" + realName + " 工号存在：" + ad.isLoaded() + " 用户存在：" + ud.isLoaded());
