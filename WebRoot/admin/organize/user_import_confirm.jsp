...
for(int i=0;i<info.length;i++){
	...
	// 帐号（必填） 姓名（必填）	工号	手机	性别	员工编号	部门（必填）	角色
	// 身份证号码 婚否	 出生日期 	E-mail	QQ	电话	入职日期	短号	兴趣爱好	地址
...
while (ir.hasNext()) {
	...
	for (int i=0; i<info.length; i++) {
		// 如果帐户及姓名均相等
		if (info[i][0].equals(ud.getName()) && info[i][1].equals(ud.getRealNameRaw())) {
			accountName = info[i][2];
			isFound = true;
			break;
		}
