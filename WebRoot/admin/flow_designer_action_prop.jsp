...
<head>
    ...
    <script language="JavaScript">
        ...
        function ModifyAction(isSubmit) {
            ...
            // 组合item2
            // var item2 = "{" + o("relateToAction").value + "," + o("ignoreType").value + "," + o("kind").value + "," + o("fieldHide").value.replaceAll(",", "|") + "}";
            ...
            var item2 = "{" + o("relateToAction").value + "," + o("ignoreType").value + "," + o("kind").value + "," + o("fieldHide").value.replaceAll(",", "|") + "," + isDelayed + "," + timeDelayedValue + "," + timeDelayedUnit + "," + canPrivUserModifyDelayDate + "," + o("formView").value + "}";
        ...
        function onload_win() {
            ...
            // 解析item2，格式{ relateToAction , ignoreType , kind , fieldHide , isDelayed , timeDelayedValue , timeDelayedUnit , canPrivUserModifyDelayDate, formView };
            if (item2.length >= 2) {
            ...
            if (o("msgProp").value.indexOf("<actions>") == 0) {
                $("#imgComb").show();
            }

            // alert(window.parent.getProps());
...
<table align="center" cellpadding="2" cellspacing="0" class="tabStyle_1" id="mainTable" style="padding:0px; margin:0px; margin-top:3px; width:100%">
    ...
    <tr>
        ...
        <td height="22" align="left" style="line-height:1.5">
            <span id="span_self"> <a title="自动转换为发起人" href="#" onclick="userName.value='$self';userRealName.value='本人';nodeMode.value='1';spanMode.innerHTML='用户';ModifyAction(false)">本人</a>&nbsp;&nbsp; </span> <a href="javascript:;" onclick="openWinUserRoles()">选择角色</a>&nbsp;&nbsp;&nbsp;<a href="#"
    ...
    <a title="自动转换为发起人" href="javascript:;" onclick="userName.value='$starter';userRealName.value='自动转换为发起人';nodeMode.value='1';spanMode.innerHTML='发起人';ModifyAction(false)">发起人</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a title="由前一用户自行指定" href="javascript:;"
    ...
    <tr id="nodeScriptTr" style="display:none">
        ...
        <td height="22">
            <%
                ...
                String url = "script_frame.jsp?formCode=" + flowlf.getFormCode();
            %>
            <textarea id="nodeScript" name="nodeScript" style="width:100%; height:200px"></textarea>
            <input type="button" style="margin-top:3px" value="设计器" class="btn" onclick="ideWin = openWin('<%=url%>', screen.width, screen.height);"/>
        </td>
    </tr>
    <tr>
    ...
    <tr id="trFieldHided">
        ...
        <td height="22"><textarea title="字段被隐藏后，在处理流程时将不可见" name="fieldHideText" rows="3" readonly="readonly" id="fieldHideText" style="width: 180px;background-color:#eeeeee"></textarea>
            ...
            <input name="fieldHide" type="hidden" id="fieldHide" value="<%=fieldHide%>"/></td>
    </tr>
    <tr>
    ...
    <tr id="trFlag">
        ...
        <td height="22"><input type=checkbox id="flagSel" name="flagSel" value="0" checked title="允许选择下一节点上的用户"/>
            ...
            删除附件
            <input onchange="ModifyAction(false)" type="checkbox" id="flagXorRadiate" name="flagXorRadiate" value="1" checked title="异或发散，发散节点有多条路径从其通过，如果置为异或，则只选取其中的一条继续，否则，所有路径都会继续"/>
            条件分支
            <br/>
            <input onchange="ModifyAction(false)" type="checkbox" id="flagXorAggregate" name="flagXorAggregate" value="1" checked title="聚合节点有多条路径汇合，如果置为多重，则只要有其中的一条到达，节点就会被激活，否则，只有当所有路径都到达后才会继续"/>
            多重聚合
    ...
    <tr>
        ...
        <td height="22">
            <select onchange="ModifyAction(false)" id="strategy" name="strategy" title="当满足条件的用户有多个时">
...
<script>
    ...
    function makeSubFlowProp() {
        ...
        if (o("kind").value == "<%=WorkflowActionDb.KIND_SUB_FLOW%>") {
            ...
            $("div[id^='pMapDiv']").each(function () {
                if (parentToSubMap == "") {
                    parentToSubMap = "{\"parentField\":\"" + $(this).attr("parentField") + "\", \"subField\":\"" + $(this).attr("subField") + "\", \"parentTitle\":\"" + $(this).attr("parentTitle") + "\", \"subTitle\":\"" + $(this).attr("subTitle") + "\"}";
                } else {
                    parentToSubMap += ",{\"parentField\":\"" + $(this).attr("parentField") + "\", \"subField\":\"" + $(this).attr("subField") + "\", \"parentTitle\":\"" + $(this).attr("parentTitle") + "\", \"subTitle\":\"" + $(this).attr("subTitle") + "\"}";
                }
            });
            ...
            $("div[id^='sMapDiv']").each(function () {
                if (subToParentMap == "") {
                    subToParentMap = "{\"parentField\":\"" + $(this).attr("parentField") + "\", \"subField\":\"" + $(this).attr("subField") + "\", \"parentTitle\":\"" + $(this).attr("parentTitle") + "\", \"subTitle\":\"" + $(this).attr("subTitle") + "\"}";
                } else {
                    subToParentMap += ",{\"parentField\":\"" + $(this).attr("parentField") + "\", \"subField\":\"" + $(this).attr("subField") + "\", \"parentTitle\":\"" + $(this).attr("parentTitle") + "\", \"subTitle\":\"" + $(this).attr("subTitle") + "\"}";
                }
            });
    ...
    function getSubFields() {
        $.get(
            "flow_designer_action_prop_ajax_fields.jsp",
