...
.workItemContent {
min-height:60px;
_height:expression(this.height>60 ? this.height : 60 + "px");
}
.workItemContent div {
width:210px;
...
.workItemContentKaoqin {
min-height:80px;
_height:expression(this.height>80 ? this.height : 80 + "px");
}
.workItemContentLog {
min-height:110px;
_height:expression(this.height>110 ? this.height : 110 + "px");
}
.workUser div p {
padding:0px;
margin:0px;
...
<!-- &nbsp;&nbsp;<a href="admin_dept_user_detail_style1.jsp?deptCode=<%=StrUtil.UrlEncode(deptCode)%>">- 横向显示</a> -->
<script>
...
%>
	...
	<div class="workItem"><a href="javascript:;" onclick="addTab('待办流程', '<%=request.getContextPath() %>/flow/flow_list.jsp?displayMode=1&userName=<%=StrUtil.UrlEncode(pu.getUserName())%>')" title="待办流程"><strong>待办流程</strong></a></div>
	<div class="workItemContent">
<%
MyActionDb mad = new MyActionDb();
sql = "select id from flow_my_action where (user_name=" + StrUtil.sqlstr(pu.getUserName()) + " or proxy=" + StrUtil.sqlstr(ud.getName()) + ") and is_checked=0 order by receive_date desc";
int count = 3;
...
%>
  	<div><a href="../flow_modify.jsp?flowId=<%=wfd.getId()%>" title="<%=wfd.getTitle()%> 类型：<%if (ft!=null) {%><%=ft.getName()%><%}%> 开始：<%=DateUtil.format(wfd.getBeginDate(), "yy-MM-dd HH:mm:ss")%> 到期：<%=DateUtil.format(mad.getExpireDate(), "yy-MM-dd HH:mm:ss")%> 发起人：<%=userRealName%>" target="_blank"><%=StrUtil.getLeft(wfd.getTitle(), 40)%></a></div>
<%}%>
	</div>
<div class="workItem"><a title="已办流程" href="javascript:;" onclick="addTab('<%=ud.getRealName() %>的已办流程', '<%=Global.getFullRootPath(request) %>/flow_list_done.jsp?userName=<%=StrUtil.UrlEncode(ud.getName())%>')"><strong>已办流程</strong></a></div>
	<div class="workItemContent">
<%
...
while (ir.hasNext()) {
	...
	%>
            <div>
			  <a href="../flow_modify.jsp?flowId=<%=wfd.getId()%>&amp;actionId=<%=mad.getActionId()%>" title="<%=wfd.getTitle()%> 类型：<%if (ft!=null) {%><%=ft.getName()%><%}%> 到达：<%=DateUtil.format(mad.getReceiveDate(), "yy-MM-dd HH:mm:ss")%> 处理：<%=DateUtil.format(mad.getCheckDate(), "yy-MM-dd HH:mm:ss")%> 到期：<%=DateUtil.format(mad.getExpireDate(), "yy-MM-dd HH:mm:ss")%> 绩效：<%=NumberUtil.round(mad.getPerformance(), 2)%> 发起人：<%=userRealName%>" target="_blank"><%=StrUtil.getLeft(wfd.getTitle(), 40)%></a>
            </div>
     <%}%>
	</div>
	<div class="workItem"><a title="工作报告" href="javascript:;" onclick="addTab('<%=ud.getRealName() %>的工作报告', '<%=Global.getFullRootPath(request) %>/ymoa/showWorkLogInfo.action?userName=<%=StrUtil.UrlEncode(ud.getName())%>')"><strong>工作报告</strong></a></div>
...
%>
		<div style="clear:both"><%=DateUtil.format(wld.getMyDate(), "yy-MM-dd HH:mm:ss")%></div>
        <div style="clear:both"><a href="../mywork/mywork_show.jsp?id=<%=wld.getId()%>&userName=<%=StrUtil.UrlEncode(pu.getUserName())%>" title="工作报告" target="_blank"><%=StrUtil.getAbstract(request, wld.getContent(), 38)%></a></div>
    <%}%>
	</div>
	<div class="workItem"><a title="日程安排" href="javascript:;" onclick="addTab('<%=ud.getRealName() %>的日程安排', '<%=Global.getFullRootPath(request) %>/plan/plan.jsp?userName=<%=StrUtil.UrlEncode(ud.getName())%>')"><strong>日程安排</strong></a></div>
	<div class="workItemContent">
		<%
...
<%}%>
	...
	<div class="workItem"><a title="工作计划" href="javascript:;" onclick="addTab('<%=ud.getRealName() %>的工作计划', '<%=Global.getFullRootPath(request) %>/workplan/workplan_list.jsp?userName=<%=StrUtil.UrlEncode(ud.getName())%>')"><strong>工作计划</strong></a></div>
	<div class="workItemContent">	
	<%
		...
		%>
          <div>
			<a title="<%=wpd.getTitle()%> 类型：<%=wptd.getWorkPlanTypeDb(wpd.getTypeId()).getName()%> 拟定者：<%=um.getUserDb(wpd.getAuthor()).getRealName()%> 进度：<%=wpd.getProgress()%>% 开始：<%=sbeginDate%> 结束：<%=sendDate%>" href="javascript:;" onclick="addTab('工作计划', 'workplan/workplan_show.jsp?id=<%=id%>')"><%=wpd.getTitle()%></a>
          </div>
          <%
		}
	%>
	...
	<div class="workItem"><a href="javascript:;" onclick="addTab('<%=ud.getRealName() %>的及时消息', '<%=Global.getFullRootPath(request) %>/lark/lark_msg_my_list.jsp?userName=<%=StrUtil.UrlEncode(ud.getName())%>')"><strong>及时消息</strong></a></div>
	<div class="workItemContent">	
	<%
		sql = "select id from oa_lark_msg where (from_user=" + StrUtil.sqlstr(pu.getUserName()) + " or to_user=" + StrUtil.sqlstr(pu.getUserName()) + ") order by create_date asc";
	...
	<div class="workItem"><a href="javascript:;" onclick="addTab('<%=ud.getRealName() %>的考勤', '<%=Global.getFullRootPath(request) %>/kaoqin.jsp?userName=<%=StrUtil.UrlEncode(ud.getName())%>')"><strong>考勤</strong></a></div>
	...
	<div class="workItem"><a href="javascript:;" onclick="addTab('<%=ud.getRealName() %>的请假', '<%=Global.getFullRootPath(request) %>/attendance/leave_list_user.jsp?userName=<%=StrUtil.UrlEncode(ud.getName())%>')"><strong>请假</strong></a></div>
	<div class="workItemContent">
<%
