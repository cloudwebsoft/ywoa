...
function action(com, grid) {
	if (com=="删除") {
		var selectedCount = jQuery(".cth input[type='checkbox'][value!='on'][checked=true]", grid.bDiv).length;
		if (selectedCount == 0) {
			jAlert('请选择一条记录!','提示');
			return;
		}
		...
		jConfirm("您确定要删除么？","提示",function(r){
			if(!r){return;}
				...
				var ids = "";
				jQuery(".cth input[type='checkbox'][value!='on'][checked=true]", grid.bDiv).each(function(i) {
					if (ids=="")
						ids = jQuery(this).val();
					else
						ids += "," + jQuery(this).val();
		...
		var id = jQuery(".cth input[type='checkbox'][value!='on'][checked=true]", grid.bDiv).val();		
