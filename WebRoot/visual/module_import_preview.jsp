...
%>
<form name="form1" action="?op=import&code=<%=code%>&formCode=<%=formCode%>" method="post"
    ...
    <div style="text-align:center">
        ...
        <input type="button" id="btnOk" class="btn" value="确定"
               onclick="o('btnOk').disabled=true; $('body').showLoading(); window.location.href='doImport.do?code=<%=code%>&formCode=<%=formCode%>&moduleCodeRelated=<%=moduleCodeRelated%>&parentId=<%=parentId%>&menuItem=<%=menuItem%>&templateId=<%=templateId%>&<%=params%>'"/>
        &nbsp;&nbsp;
        <%
            }
        %>
        <input type="button" class="btn" value="返回"
               onclick="window.location.href='module_import_excel.jsp?code=<%=code%>&formCode=<%=formCode%>&menuItem=<%=menuItem%>&parentId=<%=parentId%>'"/>
    </div>
</form>
</body>
</html>
