...
if (op.equals("set")) {
	...
	if (field.equals("")) {
		out.print(StrUtil.Alert_Back("请选择字段！"));
		return;
	}
	String[] ary = ParamUtil.getParameters(request, "field" + field);
	if (ary==null) {
		out.print(StrUtil.Alert_Back("请选择字段选项！"));
		return;
	}
	...
	aqd.setChartHistogram(pieDesc + ";" + opt + ";" + calcFieldCode + ";" + calcFunc);
		
	if (aqd.save()) {
		out.print(StrUtil.Alert_Redirect("操作成功！", "form_query_chart_histogram.jsp?id=" + id));
	}
	else
  ...
  <tr>
	...
	<table class="tabStyle_1 percent98" width="97%" border="0" align="center" cellpadding="2" cellspacing="0" >
          ...
          <tr>
			...
			while (ir.hasNext()) {
				...
				if (aqcd.getConditionType().equals("SELECTED")) {
					...
					if (isSeted && fieldCode.equals(fieldCodeDb))
						isChecked = true;
			%>
          ...
          <tr>
			...
			&nbsp;&nbsp;&nbsp;&nbsp;
			<input class="btn" type="button" value="清除设置" onclick="if (confirm('您确定要清除么？')) window.location.href='form_query_chart_histogram.jsp?op=clear&id=<%=id%>'" />
			&nbsp;&nbsp;&nbsp;&nbsp;
			<input class="btn" <%=!isSeted?"disabled":""%> type="button" value="预览报表" onclick="window.open('form_query_chart_histogram_show.jsp?id=<%=id%>')" />
