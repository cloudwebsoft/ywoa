...
<%
    ...
    if (op.equals("modifyName")) {
		...
		leaf.setName(name);
		leaf.setCode(code);
		leaf.setParentCode(parentCode);
		leaf.setLink(link);
		leaf.setType(0);
		leaf.setPreCode("");
		leaf.setWidth(0);
		leaf.setHasPath(false);
		leaf.setResource(false);
		...
		com.redmoon.oa.ui.menu.Leaf leafParent = new com.redmoon.oa.ui.menu.Leaf();
		leafParent = leafParent.getLeaf(parentCode);
		try {
			re = leafParent.AddChild(leaf);
...
function action(com, grid) {
	if (com=='设计')	{
		var selectedCount = $(".cth input[type='checkbox'][value!='on'][checked=true]", grid.bDiv).length;
