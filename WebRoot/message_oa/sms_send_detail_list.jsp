...
<br>
<form action="sms_send_detail_list.jsp?op=search" method="post">
...
<script type="text/javascript">
    ...
    <tr>
      <td height="24" colspan="4" align="center">
      	<input class="btn" type="submit" name="Submit" value=" 查 询 ">
  ...
  <%		
		...
		if (op.equals("del")) {
			...
			out.print(StrUtil.Alert_Redirect("操作完成！", "sms_send_detail_list.jsp?batch="+batch));
			return;
		}
		...
		if(op.equals("search")){
			...
			if (!con.equals("")) {
				con = " where batch=" + batch + " and " + con;
			}else{
				con = " where batch=" + batch;
			}
	
			sql += con;			
		}else{
			// sql = SMSSendRecordMgr.getSendSMSSQL();
			sql = "select id from sms_send_record where batch="+batch;
		}
...
<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" class="percent98">
  <tr>
    <td width="41%" height="30" align="left"><input name="button3" class="btn" type="button" onClick="selAllCheckBox('ids')" value="全选">
&nbsp;&nbsp;
...
<table width="98%" border="0" align="center" cellpadding="2" cellspacing="0" class="tabStyle_1 percent98">
  <tr>
    ...
    <td width="11%" align="center" class="tabStyle_1_title">发送者</td>
...
<%	
	  ...
	  %>
  <tr class="highlight">
    <td width="14%" align="center"><input name="ids" type="checkbox" value="<%=ssrd.getId()%>">
    <%=DateUtil.format(ssrd.getSendTime(), "yy-MM-dd HH:mm")%></td>
    <td width="11%" align="center"><%=userRealName%></td>
    <td width="11%" align="center"><%=ssrd.getReceiver()%></td>
    <td width="15%"><%=StrUtil.getNullStr(ssrd.getSendMobile())%></td>
...
<form name=formDel action="sms_send_detail_list.jsp?op=del" method=post><input name=ids type=hidden><input type="hidden" name="batch" id="batch" value="<%=batch%>" />
</form>
</body>
<script>
...
function doSort(orderBy) {
	...
	window.location.href = "sms_send_detail_list.jsp?batch=<%=batch%>&op=<%=op%>&userName=<%=userName%>&sendMobile=<%=sendMobile%>&strFromSendTime=<%=StrUtil.UrlEncode(strFromSendTime)%>&strToSendTime=<%=strToSendTime%>&orderBy=" + orderBy + "&sort=" + sort + "&msgText=<%=msgText%>";
}
</script>
<script>
