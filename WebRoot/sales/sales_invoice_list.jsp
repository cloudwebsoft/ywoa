...
function doSort(orderBy) {
	...
	window.location.href = "sales_invoice_list.jsp?op=<%=op%>&customer=<%=StrUtil.UrlEncode(customer)%>&beginDate=<%=strBeginDate%>&endDate=<%=strEndDate%>&orderBy=" + orderBy + "&sort=" + sort;
}
</script>
</head>
<body>
...
String sql = "select i.id from " + fd.getTableNameByForm() + " i , form_table_sales_order o, form_table_sales_customer c where i.cws_id=o.id and o.cws_id=c.id and i.unit_code=" + StrUtil.sqlstr(unitCode);
if (op.equals("search")) {
	sql = "select i.id from " + fd.getTableNameByForm() + " i, form_table_sales_order o, form_table_sales_customer c where i.cws_id=o.id and o.cws_id=c.id and c.unit_code=" + StrUtil.sqlstr(unitCode);
	if(!privilege.isUserPrivValid(request, "admin") && !privilege.isUserPrivValid(request, "sales") && !privilege.isUserPrivValid(request, "sales.manager") && privilege.isUserPrivValid(request, "sales.user") ){
		...
		if(!salesPerson.equals("")){
			sql += " and i.cws_creator in ("+StrUtil.sqlstr(salesPerson)+")";
		}
	}
	if (!customer.equals("")) {
...
%>
  <table id="searchTable" width="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <form action="sales_invoice_list.jsp" class="search-form">
...
o("dept").value = "<%=dept %>";
      ...
      <%
        ...
        <tr>
          ...
          <td align="center">
          <a href="../visual/module_mode1_show_relate.jsp?parentId=<%=fdao.getCwsId()%>&id=<%=fdao.getId()%>&formCodeRelated=sales_ord_invoice&formCode=sales_order&isShowNav=1">查看</a>
          </td>
        </tr>
      <%
		}
...
//out.print(paginator.getCurPageBlock("sales_invoice_list.jsp?" + querystr));
%>
</body>
<script>
...
function changeSort(sortname, sortorder) {
	window.location.href = "sales_invoice_list.jsp?<%=querystr%>&pagesize=" + flex.getOptions().rp + "&orderBy=" + sortname + "&sort=" + sortorder;
}

function changePage(newp) {
	if (newp){
		window.location.href = "sales_invoice_list.jsp?<%=querystr%>&CPages=" + newp + "&pagesize=" + flex.getOptions().rp;
		}
}

function rpChange(pagesize) {
	window.location.href = "sales_invoice_list.jsp?<%=querystr%>&CPages=<%=curpage%>&pagesize=" + pagesize;
}
