...
<script src="<%=Global.getRootPath()%>/js/jquery.js"></script>
...
Iterator ir = fdao.getAttachments().iterator();
  ...
  <tr>
	...
	<input class="btn" type="button" value=" 编辑 " onclick="window.location.href='customer_sales_chance_edit.jsp?customerId=<%=customerId%>&id=<%=id%>&formCodeRelated=<%=formCodeRelated%>&formCode=<%=formCode%>&isShowNav=1';" />
	<%}%>
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <input name="Submit" type="button" class="btn" onclick="showFormReport()" value="打印表单"/>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<input name="button" type="button" class="btn" onclick="exportToWord()" value="导出至Word"/></td>
  </tr>
</table>
