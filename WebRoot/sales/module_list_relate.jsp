...
<%
	...
	<td>
		<%if (mpd.canUserSee(privilege.getUser(request))) {%>
		<a href="module_show_relate.jsp?code=<%=moduleCode%>&customerId=<%=parentId %>&parentId=<%=parentId%>&id=<%=id%>&menuItem=<%=menuItem%>&formCodeRelated=<%=formCodeRelated%>&formCode=<%=formCode%>&isShowNav=<%=isShowNav%>">查看</a>
		<%}%>
...
<table width="98%" border="0" cellspacing="0" cellpadding="0" align="center" class="percent98">
  <tr>
    <td width="50%" height="23" align="left"><input class="btn" name="button2" type="button" onclick="selAllCheckBox('ids')" value="全选" />
	  ...
	  <%if (mpd.canUserSee(privilege.getUser(request))) {%>      
      &nbsp;&nbsp;
      <input name="button4" type="button" class="btn" onclick="javascript:window.open('<%=request.getContextPath() %>/visual/module_excel_relate.jsp?<%=querystr%>')" value="导出" />
      <%} %>
<%
