...
Iterator ir = fdao.getAttachments().iterator();
  ...
  <tr>
	...
	<input class="btn" type="button" value=" 编辑  " onclick="addTab('编辑', '<%=request.getContextPath() %>/sales/module_edit_relate.jsp?id=<%=id%>&customerId=<%=ParamUtil.getLong(request, "customerId", -1)%>&formCodeRelated=<%=formCodeRelated%>&formCode=<%=formCode%>&isShowNav=0')" />
	<%}%>
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<form name="formWord" target="_blank" action="<%=request.getContextPath() %>/visual/module_show_word.jsp" method="post">
