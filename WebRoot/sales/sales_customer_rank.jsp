...
<link type="text/css" rel="stylesheet" href="<%=request.getContextPath()%>/<%=skinPath%>/jquery-ui/jquery-ui.css" />
...
%>
<br />
<form action="sales_customer_rank.jsp" method="get">
...
while (irsd.hasNext()) {
	...
	strCategory += ",'" + sod.getName() + "'";
	
	strModel += "{name:'" + sod.getValue() + "',index:'" + sod.getValue() + "', width:40, align:'right', sortable:true, summaryType:'sum'},";
}
%>
jQuery("#mytable").jqGrid({
   	url:'sales_customer_rank_ajax.jsp?mode=<%=mode%>&preDate=' + o('preDate').value + '&beginDate=' + o('beginDate').value + '&endDate=' + o('endDate').value,
	datatype: "json",
	loadonce: true,
   	colNames:['序号','姓名', '部门', '客户数'<%=strCategory%>, '行动数','商机数'],
   	colModel:[
   		...
   		{name:'dept',index:'dept', width:150},
   		/*
		...
		*/
   		{name:'customerCount',index:'customerCount', width:50, align:"right", sortable:true, summaryType:'sum'},
		<%=strModel%>
   		{name:'actionCount',index:'actionCount', width:80, align:"right", sortable:true, sorttype:'number',formatter:'number',summaryType:'sum'},
   		{name:'chanceCount',index:'chanceCount', width:80, align:"right", sortable:true,sorttype:'number',formatter:'number',summaryType:'sum'}
   	],
   	rowNum:20,
...
jQuery("#group").change(function(){
	...
	if(vl == "") {
		...
		window.location.href = "sales_customer_rank.jsp?preDate=<%=StrUtil.UrlEncode(preDate)%>";
	} else {
		// jQuery("#mytable").jqGrid('groupingGroupBy',vl);
		window.location.href = "sales_customer_rank.jsp?mode=grouping&preDate=<%=StrUtil.UrlEncode(preDate)%>";
	}
});
