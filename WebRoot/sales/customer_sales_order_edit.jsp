...
<%}
	...
	out.print("<BR>");
%>
<form action="customer_sales_order_edit.jsp?op=saveformvalue&customerId=<%=customerId%>&parentId=<%=parentId%>&id=<%=id%>&formCodeRelated=<%=formCodeRelated%>&formCode=<%=StrUtil.UrlEncode(formCode)%>&isShowNav=<%=isShowNav%>&menuItem=<%=menuItem %>" method="post" enctype="multipart/form-data" name="visualForm" id="visualForm">
