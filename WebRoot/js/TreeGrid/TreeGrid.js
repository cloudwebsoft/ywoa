...
TreeGrid = function(_config){
	...
	drowRowData = function(_rows, _cols, _level, _pid){
		...
		for(var i=0;i<_rows.length;i++){
			...
			s += "<tr id='TR" + id + "' pid='" + ((_pid=="")?"":("TR"+_pid)) + "' folderOpen='Y' data=\"" + TreeGrid.json2str(row) + "\" rowIndex='" + rownum++ + "'>";
	...
	init = function(){
		...
		__root.find("img[folder='Y']").bind("click", function(){
			...
			var isOpen = __root.find("#" + trid).attr("folderOpen");
			isOpen = (isOpen == "Y") ? "N" : "Y";
			__root.find("#" + trid).attr("folderOpen", isOpen);
	...
	showSubs = function(_trid){
		var isOpen = __root.find("#" + _trid).attr("folderOpen");
		if(isOpen == "Y"){
