...
<head>
    ...
    <%
        ...
        if (!mad.isLoaded()) {
            ...
            out.print(SkinUtil.makeInfo(request, str));
            return;
...
// System.out.println(getClass() + " " + mad.getUserName() + " " + mad.getProxyUserName() + " " + myname);
        ...
        if (wa.getStatus() == WorkflowActionDb.STATE_DOING || wa.getStatus() == WorkflowActionDb.STATE_RETURN)
            ...
            if (!wfp.isReactive() && !wa.isXorAggregate()) {
                ...
                out.print(SkinUtil.makeErrMsg(request, str + wa.getStatus() + "，" + str1 + "，<a href='flow_modify.jsp?flowId=" + wf.getId() + "'>" + str2 + "</a>！", false, false));
                return;
            } else {
                if (wfp.isReactive()) {
...
if (wa.getStatus()!=wa.STATE_DOING && wa.getStatus()!=wa.STATE_RETURN) {
	...
	if (!wfp.isReactive()) {
		String str = LocalUtil.LoadString(request,"res.flow.Flow","processStatus");
		String str1 = LocalUtil.LoadString(request,"res.flow.Flow","mayHaveBeenProcess");
		out.print(SkinUtil.makeErrMsg(request, str + wa.getStatus() + "，"+str1));
		return;
	}
}
*/
...
// 置NestSheetCtl需要用到的formCode
    ...
    <script src="flow/form_js/form_js_<%=lf.getFormCode()%>.jsp?flowId=<%=flowId%>&myActionId=<%=myActionId%>&userName=<%=StrUtil.UrlEncode(privilege.getUser(request))%>"></script>
    ...
    <script language="javascript">
        ...
        function distributeDoc(flowId) {
            ...
            var str = "<lt:Label res='res.flow.Flow' key='tit'/>：<input name='paperTitle' value='<%=StrUtil.HtmlEncode(wf.getTitle())%>' /><BR />";
            str += "<lt:Label res='res.flow.Flow' key='seeFlow'/>：<input id='isFlowDisplay' name='isFlowDisplay' type='radio' value='1' /><lt:Label res='res.flow.Flow' key='yes'/><input id='isFlowDisplay' name='isFlowDisplay' type='radio' value='0' checked /><lt:Label res='res.flow.Flow' key='no'/><BR />";
            str += "<lt:Label res='res.flow.Flow' key='unit'/>：" + disNames;
            $("#dlg").html(str);
            $("#dlg").dialog({
                title: "<lt:Label res='res.flow.Flow' key='downDepartment'/>",
                modal: true,
                buttons: {
                    "<lt:Label res='res.flow.Flow' key='cancel'/>": function () {
                        $(this).dialog("close");
        ...
        function OpenModifyWin(internalName, actionId, isXor) {
            ...
            openWin('flow_action_modify.jsp?actionId=' + actionId + '&isXor=' + isXor, 800, 600)
        }
    ...
    <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="main">
      <tr>
        ...
        <%if (wf.getProjectId()!=-1) {
            ...
            %>
            <lt:Label res="res.flow.Flow" key="project"/>：<a href="javascript:;" onclick="addTab('<%=fdao.getFieldValue("name")%>', 'project/project_show.jsp?projectId=<%=wf.getProjectId()%>&formCode=project')"><%=fdao.getFieldValue("name")%></a>&nbsp;&nbsp;<a title="取消关联" href="javascript:;" onclick="unlinkProject()" style='font-size:16px; font-color:red'>×</a>
            <%
        }
        %>
        </span>
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr>
          <td align="center">
            ...
            <input name="cwsWorkflowLevel" type="radio" value="<%=WorkflowDb.LEVEL_NORMAL%>" checked /><img src="images/general.png" align="absmiddle" />&nbsp;<lt:Label res="res.flow.Flow" key="ordi"/>
            <input name="cwsWorkflowLevel" type="radio" value="<%=WorkflowDb.LEVEL_IMPORTANT%>" /><img src="images/important.png" align="absmiddle" />&nbsp;<lt:Label res="res.flow.Flow" key="impor"/>
            <input name="cwsWorkflowLevel" type="radio" value="<%=WorkflowDb.LEVEL_URGENT%>" /><img src="images/urgent.png" align="absmiddle" />&nbsp;<lt:Label res="res.flow.Flow" key="emergent"/>
            <script>
...
<input id="cwsWorkflowResult" name="cwsWorkflowResult" size="30" style="border:1px solid #cccccc; color:#888888;width:200px;" value="<%=StrUtil.HtmlEncode(mad.getResult())%>" />
      ...
      <table id="designerTable" width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <table id="processListTab" width="98%" class="tabStyle_1 percent98" style="display:none;background-color:#fff">
...
while (ir.hasNext()) {
	...
	%>
					  ...
					  %>
                        ...
                        <td align="center"><%=madPro.getResult()%></td>
                      </tr>
                      <%}%>
                    </tbody>
                  </table>
        ...
        <div id="formContent" style="margin-top:5px">
		<%
		// 判断是否需按选项卡的方式显示
		...
		Iterator irField = rd.getFieldsOfView().iterator();
                    ...
                    <tr>
                     	<td align="left" colspan="2">
                    		 <div id="myReplyTextarea<%=0%>" style="display:none; clear:both;position:relative;margin-bottom:40px">
                                    ...
                                    <span align="left" title="<lt:Label res='res.flow.Flow' key='othersHidden'/>" style="cursor:pointer;" onclick="chooseHideComment(this);"><img src="<%=SkinMgr.getSkinPath(request)%>/images/admin/functionManage/checkbox_not.png" />&nbsp;<lt:Label res='res.flow.Flow' key='needHidden'/>
...
<div class="sidebar">
	...
	<div class="subNav"><h6><input title="应用可写字段及隐藏字段" onclick="applyProps()" type="button" value="确定" /></h6></div>
...
while (irFields.hasNext()) {
	...
	%>
	<li><input name="fieldsWrite" type="checkbox" value="<%=ff.getName() %>" title="<%=ff.getTitle() %>" <%=isWrite?"checked":"" %> /><%=ff.getTitle() %></li>
	<%
}
		...
		// if (ff.getMacroType().equals("nest_table") || ff.getMacroType().equals("nest_sheet")) {
			...
			while (ir2.hasNext()) {
				...
				if (true || listField.indexOf("," + ff2.getName() + ",")!=-1) {
					...
					%>
					<li><input name="fieldsWrite" type="checkbox" value="nest.<%=ff2.getName() %>" title="<%=ff2.getTitle() %>" <%=isFinded?"checked":"" %> /><%=txt %></li>
					<%
				}
			}
			// break;
...
while (irFields.hasNext()) {
	...
	%>
	<li><input name="fieldsHide" type="checkbox" value="<%=ff.getName() %>" title="<%=ff.getTitle() %>" <%=isHide?"checked":"" %> /><%=ff.getTitle() %></li>
	<%
}
		...
		if (mu.getNestType() != MacroCtlUnit.NEST_TYPE_NONE) {
			...
			while (ir2.hasNext()) {
				...
				if (true || listField.indexOf("," + ff2.getName() + ",")!=-1) {
					...
					%>
					<li><input name="fieldsHide" type="checkbox" value="nest.<%=ff2.getName() %>" title="<%=ff2.getTitle() %>" <%=isFinded?"checked":"" %> /><%=txt %></li>
					<%
				}
			}
			// break;
...
%>
	...
	<ul class="navContent">
		<li><input type="button" value="运行" onclick="runValidateScript()" /></li>
		<li>
	...
	<ul class="navContent">
		<li><input type="button" value="运行" onclick="runFinishScript()" /></li>
		<li>
	...
	<ul class="navContent">
		<li><input type="button" value="运行" onclick="runDeliverScript()" /></li>
		<li>
...
function doRenameAtt(response) {
	...
	for (var i=0; i<items.length; i++){
		...
		jAlert(result,'<lt:Label res="res.flow.Flow" key="prompt"/>');
...
function showResponse(data)  {
	...
	else if (op=="editFormValue") {
		jAlert_Redirect(data.msg, '<lt:Label res="res.flow.Flow" key="prompt"/>', "flow_form_edit.jsp?flowId=<%=wf.getId()%>");
		return;
	}
...
function toolbarSubmit() {
	...
	<%if (mad.getCheckStatus()!=MyActionDb.CHECK_STATUS_SUSPEND) {%>
		/*
		if (!isDocDistributed && confirm("该文件尚未分发，是否分发？")) {
...
function initToolbar(toolbarId) {
  ...
  toolbar = new Toolbar({
	...
	<%if (!isReadOnly) {%>
		...
		,'-', {
		  ...
		  handler : function(){
			  /*
			  <%
			  if (!isReadOnly && wf.isStarted() && wa.isArchiveGov() && mad.getCheckStatus()!=MyActionDb.CHECK_STATUS_SUSPEND) {
			  %>
				$.ajax({
					type: "post",
					url: "flow_dispose_do.jsp",
					data: {
						myop: "isArchiveGovDone",
						flowId: <%=wf.getId()%>
					},
					dataType: "html",
					beforeSend: function(XMLHttpRequest){
					...
					success: function(data, status){
						...
						if (data.ret=="1") {
							...
							jConfirm("该文件尚未存档，是否存档？",'<lt:Label res="res.flow.Flow" key="prompt"/>',function(r){
								if(!r){return;}
								else{
	...
	<%}%>
	<%}}%>
	<%//if (!isReadOnly && wf.isStarted() && flag.length()>=3 && flag.substring(2, 3).equals("1")) {%>
	<%
...
function selTemplate(doc_id, file_id) {
	...
	$.ajax({
		...
		success: function(data, status){
			...
			$("#dlg").html(data);
			hideDesigner();
			$("#dlg").dialog({title:'<lt:Label res="res.flow.Flow" key="template"/>', modal: true,
								buttons: {
									'<lt:Label res="res.flow.Flow" key="cancel"/>':function() {
										$("#dlg").html("");
										$(this).dialog("close");
...
<%
// 如果下一节点上设置为“表单中的人员”，则绑定change事件，以重新匹配人员
