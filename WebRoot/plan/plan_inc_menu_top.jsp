...
<div id="tabs1">
  <ul>
	...
	if (userNameTop.equals("") || new com.redmoon.oa.pvg.Privilege().getUser(request).equals(userNameTop)) {
		%>
        <li id="menu5"><a class="black" href="plan_list.jsp?mode=iMake&userName=<%=StrUtil.UrlEncode(userNameTop)%>"><span title="我制定的其他人员的日程">我制定的日程</span></a></li>
		<%
	}
	%>
    <li id="menu4"><a class="black" href="plan_add.jsp?userName=<%=StrUtil.UrlEncode(userNameTop)%>"><span>添加日程</span></a></li>
    <%} %>
  </ul>
</div>
