...
int cooperateId = ParamUtil.getInt(request, "cooperateId",0);
if (cooperateId ==0){
	String cooperateString = ParamUtil.get(request, "cooperateString");
	if(cooperateString.equals("")){
		cooperateId = cm.getPrivId(dir_code,privilege.getUser(request));
	}
}
//String cooperate = ParamUtil.get(request, "cooperate");   //判断是否是网络硬盘的入口
//if(cooperate.equals("cooperate") && !dir_code.equals(privilege.getUser(request))){
	...
	if (isSucceed) {
		if (op=="edit")
		{
			if (work=="modify")
				...
				location.href = "?op=editarticle&dir_code=<%=StrUtil.UrlEncode(dir_code)%>&userName=<%= StrUtil.UrlEncode(userName)%>&mode=<%=mode%>&cooperateId=<%=cooperateId%>";
			else
				//location.href = "clouddisk_list.jsp?userName=<%=StrUtil.UrlEncode(userName) %>&op=editarticle&dir_code=<%=StrUtil.UrlEncode(dir_code)%>&mode=<%=mode%>";
				location.href = "?op=editarticle&dir_code=<%=StrUtil.UrlEncode(dir_code)%>&userName=<%= StrUtil.UrlEncode(userName)%>&mode=<%=mode%>&cooperateId=<%=cooperateId%>";
		}
...
//javascript去空格函数 
	...
	$(function(){
		...
		$('.clfT').live("click",function(){
			$("#ctlTable").css({"display":"block","top":"150px","z-index":"1000"});
	...
	<body oncontextmenu="return false" onload="upload_none()">
		&nbsp;
		<%
if (!privilege.isUserLogin(request))
{
				...
				<div class="rHead2" style="background: #f7f7f7;display:block">
					<%	CooperateBean cb = new CooperateBean();
						...
						if(select_sort.equals("select_one")) {
							showPath = "<a href='clouddisk_network_neighborhood_list.jsp?op=editarticle&dir_code="+StrUtil.UrlEncode(cb.getDirCode())+"&cooperateId="+cooperateId+"'>所有文件</a>    >    “" + text_content + "”的搜索结果";
						...
						<%}else{
							if (dir_code.equals(cb.getDirCode()) || select_file.equals("select_file")) {
								backFlag = false;
								showPath = "<a href='clouddisk_network_neighborhood_list.jsp?op=editarticle&dir_code="+StrUtil.UrlEncode(cb.getDirCode())+"&cooperateId="+cooperateId+"'>所有文件</a>";
							}  else if (leaf.getParentCode().equals(cb.getDirCode())) {
									showPath = "<a href='clouddisk_network_neighborhood_list.jsp?op=editarticle&dir_code="+StrUtil.UrlEncode(cb.getDirCode())+"&cooperateId="+cooperateId+"'>所有文件</a>    >>    <a href='clouddisk_network_neighborhood_list.jsp?op=editarticle&dir_code="+StrUtil.UrlEncode(dir_code)+"&cooperateId="+cooperateId+"'>" + dir_name + "</a>";
							} else {
								Leaf rootLeaf = new Leaf(cb.getDirCode());
								Leaf pleaf = new Leaf(leaf.getParentCode());
								if ((leaf.getLayer() - rootLeaf.getLayer()) == 2) {
									showPath = "<a href='clouddisk_network_neighborhood_list.jsp?op=editarticle&dir_code="+StrUtil.UrlEncode(cb.getDirCode())+"&cooperateId="+cooperateId+"'>所有文件</a>    >>    <a href='clouddisk_network_neighborhood_list.jsp?op=editarticle&dir_code="+StrUtil.UrlEncode(pleaf.getCode())+"&cooperateId="+cooperateId+"'>" + pleaf.getName() + "</a>    >>   <a href='clouddisk_network_neighborhood_list.jsp?op=editarticle&dir_code="+StrUtil.UrlEncode(dir_code)+"&cooperateId="+cooperateId+"'>"+ dir_name + "</a>";
								} else {
									showPath = "<a href='clouddisk_network_neighborhood_list.jsp?op=editarticle&dir_code="+StrUtil.UrlEncode(cb.getDirCode())+"&cooperateId="+cooperateId+"'>所有文件</a>    >>    ...>>    <a href='clouddisk_network_neighborhood_list.jsp?op=editarticle&dir_code="+StrUtil.UrlEncode(pleaf.getCode())+"&cooperateId="+cooperateId+"'>" + pleaf.getName() + "</a>    >>     <a href='clouddisk_network_neighborhood_list.jsp?op=editarticle&dir_code="+StrUtil.UrlEncode(dir_code)+"&cooperateId="+cooperateId+"'>" + dir_name + "</a>";
								}
							}
						 %>
						 <%if(backFlag) {%>
						 <div style="float:left;margin-left:15px;margin-top:4px;"><a href="clouddisk_network_neighborhood_list.jsp?op=editarticle&cooperateId=<%=cooperateId %>&dir_code=<%=StrUtil.UrlEncode(leaf.getParentCode()) %>"><img src="images/clouddisk/back.png"/></a></div>
						 <%}
						}%>
			   ...
			   // 防XSS
					...
					<dd class="fileTitle">
						<div class="fileNameDetailTitle">
							...
							<input id="filename_input" name="checkbox" class="title_cbox"
								type="checkbox" />
							<span class="fnameTitle" onClick="doSort('name')">文件名</span>
						...
						<div class="colTitle dateT" >
							<span onClick="doSort('version_date')">修改日期</span>
				...
				<form name="addform" action="fwebedit_do.jsp" method="post"
				   ...
				   <input type="hidden" value="<%=Leaf.getAutoCode()%>" id="code" />
				    ...
				    <input type="hidden" value="<%=cooperateId%>" id="cooperateId" />
				   <!-- lzm添加 角色模板 -->
				   <!-- lzm SwfUpload文件批量上传 -->
				   	<input type="hidden" value="netdisk_office_upload.jsp;jsessionid=<%=session.getId() %>?userName=<%=StrUtil.UrlEncode(cb.getShareUser()) %>&dirCode=<%=StrUtil.UrlEncode(dir_code) %>" id="uploadUrl" />
			...
			<% 
			 if(!select_sort.trim().equals("select_one") && !select_file.trim().equals("select_file")){
				...
				<dl>
					<dd class="fileGroup" id="folder<%=clf.getCode()%>" type='file'>
						<div class="fileNameDetail">
							...
							<%}else{ %>
								<img src="images/folder_netdisk_share.gif" class="extImg" />
							<%} %>
							<span class="fname floderNameInfo" id="span<%=clf.getCode()%>"
								dirCode="<%=clf.getCode()%>" name="span<%=clf.getCode()%>">
								<a title="<%=clf.getName()%>"
								href="clouddisk_network_neighborhood_list.jsp?userName=<%=cb.getShareUser() %>&op=editarticle&cooperateId=<%=cooperateId %>&dir_code=<%=StrUtil.UrlEncode(clf.getCode())%>&mode=<%=mode%>"><%=clf.getName()%></a>
							</span>
				...
				<%
   				...
   				Vector vs = cmg.getShareDirBySearch(dir_code,text_content);
   				Iterator irch = vs.iterator();
   				while (irch.hasNext()) {
   					Leaf clf = (Leaf)irch.next();
   					Leaf parentLeaf = new Leaf(clf.getParentCode());
           %>
           		<dl>
					<dd class="fileGroup" id="folder<%=clf.getCode()%>" type='file'>
						...
						<div class="col catalogT"  style="display:none;">
							<span ><a href="?mode=<%=mode %>&op=editarticle&userName=<%=StrUtil.UrlEncode(userName) %>&dir_code=<%=clf.getParentCode() %>&cooperateId=<%=cooperateId %>"><%=parentLeaf.getName()%></a></span>
						</div>
					</dd>
				</dl>
			...
			//}
			  if (doc!=null) {
				  ...
				  if (select_file.equals("select_file")){
						...
						CooperateMgr cmg = new CooperateMgr();
						attachments = cmg.getShareAttByType(dir_code,sb.toString());   //用新方法将数组重新赋值
				  }else if (select_sort.equals("select_one")) {
						...
						//sql += orderBy + " " + sort;
						CooperateMgr cmg = new CooperateMgr();
						attachments = cmg.getShareAttBySearch(dir_code,text_content);   //用新方法将数组重新赋值
				  }
				...
				<dl id="tree<%=am.getId()%>" class="fileDl">
					<dd class="fileGroup">
						...
						<div class='col catalogT' style="display:none;">
							<span><a href="?mode=<%=mode %>&op=editarticle&userName=<%=StrUtil.UrlEncode(userName) %>&dir_code=<%=am.getCodeByDocId() %>&cooperateId=<%=cooperateId %>"><%=am.getParentDir() %></a></span>
						</div>
					</dd>
				</dl>
				<%}
			...
			<tr>	
				<td>
					...
					<tr><td class='SD_upload'></td>
						...
						<div id='SD_button'><div class='SD_button'>
				...
				<td align="center">
					<div style="width: 400px; height: 43px;">
						<object id="redmoonoffice"
							classid="CLSID:D01B1EDF-E803-46FB-B4DC-90F585BC7EEE" width="316"
...
function doSort(orderBy) {
	if (orderBy==curOrderBy)
		if (sort=="asc")
			...
			sort = "asc";
	window.location.href = "?mode=<%=mode%>&cooperateId=<%=cb.getId()%>&dir_code=<%=StrUtil.UrlEncode(dir_code)%>&op=editarticle&orderBy=" + orderBy + "&sort=" + sort;
}
...
function select_con(){
	...
	if(content == "请输入文件名搜索..." || $.trim(content) == ""){
		...
		window.location.href="?mode=<%=mode%>&userName=<%=StrUtil.UrlEncode(cb.getShareUser()) %>&cooperateId=<%=cooperateId%>&dir_code=<%=dir_code%>&select_sort=select_one&select_content="+content;
	}
}

function select_file(which){
	window.location.href="clouddisk_network_neighborhood_list.jsp?mode=<%=mode%>&userName=<%=StrUtil.UrlEncode(cb.getShareUser()) %>&cooperateId=<%=cooperateId%>&dir_code=<%=dir_code%>&select_file=select_file&select_which="+which;
}
...
function showDigFile(){
		...
		upload += "<object classid='CLSID:DE757F80-F499-48D5-BF39-90BC8BA54D8C' width=400 style='height:180px' align='middle' id='webedit'> <param name='Encode' value='utf-8'> <param name='MaxSize' value='<%=Global.MaxSize%>'><!--上传字节-->";
