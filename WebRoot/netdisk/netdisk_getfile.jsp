...
if (ext==null)
	ext = "";
String vpath = StrUtil.UrlEncode(att.getVisualPath());
vpath = vpath.replaceAll("%2F", "/");
	
// System.out.println(getClass() + " att.getExt()=" + att.getExt());
if (StrUtil.isImage(ext)) {
	%>
	<img src="<%=request.getContextPath()%>/<%=file_netdisk%>/<%=vpath%>/<%=StrUtil.UrlEncode(att.getDiskName())%>" />
	<%
	return;
}
...
<param name="SRC" value="<%=Global.getFullRootPath(request) + "/" + file_netdisk + "/" + vpath + "/" + att.getDiskName()%>">
...
<object classid="clsid:22D6F312-B0F6-11D0-94AB-0080C74C7E95" id="MediaPlayer1" width="428" height="330">
        ...
        <param name="Filename" value="<%=Global.getFullRootPath(request) + "/" + file_netdisk + "/" + vpath + "/" + URLEncoder.encode(att.getDiskName(), "UTF-8")%>">
	...
	if (MIMEMap.get(att.getExt())==null) {
