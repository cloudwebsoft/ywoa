...
swfobject.embedSWF(
  ...
  {"data-file":"mywork_stat_data.jsp<%=StrUtil.UrlEncode("?userName=" + StrUtil.UrlEncode(userName) + "&showyear=" + showyear + "&showmonth=" + showmonth)%>"} );
</script>
</head>
<body>
<%
...
for (int i=1; i<=12; i++) {
	if (showmonth==i)
		out.print("<a href='mywork_stat.jsp?userName="+StrUtil.UrlEncode(userName)+"&showyear="+showyear+"&showmonth="+i+"'><font color=red>"+i+"月</font></a>&nbsp;");
	else
		out.print("<a href='mywork_stat.jsp?userName="+StrUtil.UrlEncode(userName)+"&showyear="+showyear+"&showmonth="+i+"'>"+i+"月</a>&nbsp;");
