...
<table cellspacing="0" cellpadding="0" width="100%">
  <tbody>
    <tr>
      <td class="tdStyle_1"><span class="thead" style="PADDING-LEFT: 10px"><%=name %>增加或修改</span></td>
    </tr>
  </tbody>
</table>
<form action="" method="post" name="form1" target="dirhidFrame" id="form1" onsubmit="return form1_onsubmit()">
	<table align="center" class="tabStyle_1 percent80" >
		...
		<tr>
			<%
				SubjectDb sjd = new SubjectDb();
				if(tsd.getParentCode().equals(root_code)){%>
				<td>科目：
					<select size="1" name="subject">
			        <option selected="selected" value="" >--选择科目--</option>
			        <%
					String ssql = "select id from oa_exam_subject";
					Iterator ir2 = sjd.list(ssql).iterator();
					while (ir2.hasNext()) {
					  sjd = (SubjectDb)ir2.next();
			        %>
			        <option value="<%=sjd.getId()%>"><%=sjd.getSubject()%></option>
			        <%}%>
			      </select> 
				</td>
				<%
				}else{
					sjd =sjd.getSubjectDb(Integer.parseInt(tsd.getDescription()));
				%>
				<td >科目：
					<select size="1" name="subject" >
				        <option selected="selected" value="<%=sjd.getId() %>" ><%=sjd.getSubject() %></option>
				    </select>
				</td>
				<%}
			 %>
...
function mysubmit(){
	...
	ajaxPost('../question/questionKindEdit.do',formData,function(data){
		data = $.parseJSON(data);
		if(data.ret=="1"){
			var framec = window.parent.document.getElementById("exanMajorMenu");
			framec.src="major_priv_left.jsp";
			jAlert_Redirect(data.msg,"提示","major_priv_manage.jsp?majorCode="+code);
		}else if(data.ret=="0"){
			jAlert(data.msg,"提示");
		}
	});
