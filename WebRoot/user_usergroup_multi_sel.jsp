  ...
  <%}%>
	...
	<div id="resultTable">
	  <table width="100%" border="0" cellpadding="4" cellspacing="0" class="tabStyle_1_subTab">
        <tr>
          <td width="98" align="left" class="tabStyle_1_subTab_title"><lt:Label res='res.flow.Flow' key='rank'/></td>
          <td width="91" align="left" class="tabStyle_1_subTab_title"><lt:Label res='res.flow.Flow' key='staffMember'/></td>
	  ...
	  <tr>
	    <td>
            <%
                // com.redmoon.oa.basic.RankDb.getRankName(ud.getRankCode())
            %></td>
	    <td><a onClick="selPerson('', '', '<%=StrUtil.toHtml(ud.getName())%>', '<%=StrUtil.toHtml(ud.getRealName())%>')" href="#"><%=ud.getRealName()%></a></td>
	    <td>[<a onClick="cancelSelPerson('', '', '<%=StrUtil.toHtml(ud.getName())%>')" href="#"><lt:Label res='res.flow.Flow' key='deselect'/></a>]</td>
	  </tr>
	  <%}%>
      </tbody>
    </table>
	</div><div><input class="btn" type="button" value="<lt:Label res='res.flow.Flow' key='selectGroupUsers'/>" onClick="selAllUserOfDept()"></div></td>
  </tr>
  <tr align="center">
